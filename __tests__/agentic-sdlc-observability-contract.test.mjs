...
test("verified, delivery_ready, and deployed remain distinct non-promoting claims", () => {
  ...
  assert.match(facts, /does not yet claim a runtime-ready `agentic-sdlc-run\/v1` parity projection/);
});

test("economics and deployment boundaries stay exact and the planning row is complete", () => {
  assert.match(runtime, /exact zeros for network calls, model calls, prompt tokens, completion tokens, and estimated cost/);
  assert.match(runtime, /does not claim current-guideline, protected Knowgrph, cross-device, Prod, or Cloudflare runtime parity/);
