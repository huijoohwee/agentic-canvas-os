...
test('canonical lifecycle delegates one guarded lane state machine to the pinned ADLC harness', () => {
  ...
  assert.match(lifecycle, /Publication\s+is create-only: one evidence\s+ref names one one-parent child/)
  assert.match(lifecycle, /separate zero-bypass rules forbid update,\s+deletion, and\s+non-fast-forward movement after creation/)
