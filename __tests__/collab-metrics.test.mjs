...
test("recordRoomFanOut rejects impossible dispatch accounting without throwing", () => {
  const initial = createRoomMetrics();
  const invalid = recordRoomFanOut(initial, {
    attempted: 2,
    dispatched: 2,
  ...
  assert.equal(invalid.observabilityFailures, 1);
  assert.equal(invalid.fanOutOperations, 0);
  assert.equal(initial.observabilityFailures, 0);
});

test("recordRoomFanOut settles malformed and hostile telemetry as an observability failure", () => {
  const initial = createRoomMetrics();
  for (const malformed of [null, [], "secret-shaped-telemetry"]) {
    const result = recordRoomFanOut(initial, malformed);
    assert.equal(result.observabilityFailures, 1);
    assert.equal(result.fanOutOperations, 0);
  }

  const hostile = new Proxy({}, {
    get() {
      throw new Error("raw telemetry secret");
    },
  });
  const result = recordRoomFanOut(initial, hostile);
  assert.equal(result.observabilityFailures, 1);
  assert.equal(result.fanOutOperations, 0);
  assert.equal(JSON.stringify(result).includes("raw telemetry secret"), false);
});
...
test("roomLogRecord keeps reserved envelope fields authoritative", () => {
  const record = roomLogRecord({
    room: "room_safe",
    event: "fanout_degraded",
    level: "warn",
    now: 0,
    fields: {
      ts: "forged",
      level: "info",
      room: "raw-secret-room",
      event: "healthy",
      attempted: 3,
    },
  });
  ...
  assert.equal(JSON.stringify(record).includes("raw-secret-room"), false);
});
