...
import {
  ACOS_ADMISSION_TEST_SECRET,
  createAdmissionAuthFixture,
} from "./lib/commerce-admission-auth-fixture.mjs";

const NOW = 1_800_000_000_000;
const OPERATOR_REF = "operator://agentic-graph/commerce-adapter-admission/2026-09-03";
const URL = `https://acos-admission.internal${COMMERCE_ADMISSION_PATH}`;
const AUTH_SECRET = ACOS_ADMISSION_TEST_SECRET;
const { authenticatedPost, readyRequest } = createAdmissionAuthFixture(URL);
...
const DEPLOYMENT_IDENTITY = Object.freeze({
  ...
  versionTag: productionVersionTag(CANDIDATE_DIGEST),
...
function registrationBody(agentId = "agent-flight", overrides = {}) {
...
function requestFor(body, permit, { headers = {}, rawBody } = {}) {
...
function createRuntime(namespace, { projectionFail, now = () => NOW } = {}) {
  const registry = createAgentDefinitionRegistry();
  const resolver = createCommerceOperatorInstructionResolver(OPERATOR_REF);
  ...
  return {
    ...
    provider: createCommerceAdmissionProvider({
      store, registrationInterface, deploymentIdentity: DEPLOYMENT_IDENTITY, now,
...
function assertEcho(response, permit) {
...
function assertNoFenceEcho(response) {
...
test("HMAC authentication gates readiness and rejects a forged high-epoch permit before durable writes", async () => {
  ...
  malformedHeaders.set("x-acos-admission-auth-signature", "A".repeat(64));
...
test("the Worker entrypoint wires the configured private service-binding route", async () => {
  const env = {
    ...
    ACOS_ADMISSION_OPERATOR_INSTRUCTION_REF: OPERATOR_REF,
    ACOS_SOURCE_REVISION: SOURCE_REVISION,
    ACOS_CANDIDATE_DIGEST: CANDIDATE_DIGEST,
    CF_VERSION_METADATA: {
  ...
  assert.equal(readyBody.contract, "commerce.acos-admission-provider/v3");
  assert.equal(readyBody.productionReady, true);
  const publicRoute = await handleCloudflareRequest(new Request(
    `https://airvio.co${COMMERCE_ADMISSION_PATH}/readyz`,
  ), { ...env, ASSETS: { fetch: () => Response.json({ leaked: true }) } });
  assert.equal(publicRoute.status, 404);
  assert.deepEqual(await publicRoute.json(), { error: "not found" });
});

test("the resolver and unavailable runtime fail closed without broadening authority", async () => {
  const broadened = createCommerceOperatorInstructionResolver(`${OPERATOR_REF},operator://unowned`);
  assert.equal(broadened.configured, false);
  assert.equal((await broadened.resolveOperatorInstruction(OPERATOR_REF)).resolved, false);
