...
test("replay ledger reader does not treat the full ledger blob as the replay byte subject", () => {
  const largeLedger = {
    ...LEDGER,
    padding: "x".repeat((8 * 1024 * 1024) + 1),
  };
  const encoded = Buffer.from(JSON.stringify(largeLedger), "utf8").toString("base64");
  const execFile = (command, args) => {
    assert.equal(command, "gh");
    const path = args.at(1);
    if (path.endsWith("/git/ref/heads/agentic%2Fcollaboration-ledger")) {
  ...
  assert.equal(snapshot.revision, SHA("1"));
  assert.equal(snapshot.ledger.padding.length, largeLedger.padding.length);
});
