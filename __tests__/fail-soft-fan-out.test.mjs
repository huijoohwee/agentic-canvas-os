...
test("cancellation after setup but before queued dispatch prevents every branch call", async () => {
  const controller = new AbortController();
  let dispatches = 0;
  const run = failSoftFanOut(["duplicate", "duplicate", "third"], () => {
    dispatches += 1;
    return "must-not-run";
  }, { signal: controller.signal });
  ...
  assert.equal(result.dispatched, 0);
  assert.equal(result.canceledBeforeDispatch, 3);
  assert.equal(result.canceled, 3);
  assert.equal(result.failed, 3);
  assert.ok(result.dispatched + result.canceledBeforeDispatch <= result.attempted);
  assert.deepEqual(result.auditTrail.map((entry) => entry.branchId), [
    "branch-1",
    "branch-2",
    "branch-3",
  ]);
  assert.equal(new Set(result.auditTrail.map((entry) => entry.branchId)).size, 3);
});
...
test("rejects timer values that hosts would overflow and clamp", async () => {
  const accepted = await failSoftFanOut([], () => {}, { timeoutMs: MAX_FAN_OUT_TIMEOUT_MS });
  assert.equal(accepted.status, "empty", "the exact host-safe maximum remains portable");
