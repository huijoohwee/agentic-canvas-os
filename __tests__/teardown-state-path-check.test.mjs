...
test("checks hook destinations for copy, move, tee, and redirection", () => {
  const sources = [
    "cp inside ../copied", "mv inside ../moved", "tee ../written",
    "printf value > ../redirected",
  ];
  for (const source of sources) {
    const [finding] = analyzeSource({ source, file: ".githooks/pre-push", repositoryRoot: root });
    assert.ok(finding, source);
    assert.equal(finding.target.startsWith(path.dirname(root)), true);
  }
});
