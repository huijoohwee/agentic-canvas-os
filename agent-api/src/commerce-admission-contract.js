...
export const COMMERCE_ADMISSION_PROVIDER_CONTRACT = "commerce.acos-admission-provider/v3";
export const COMMERCE_ADMISSION_PATH = "/internal/v2/adapter-registrations";
export const COMMERCE_ADMISSION_RECEIPT_SCHEMA = "acos-adapter-registration/v2";
export const COMMERCE_ADMISSION_OPERATOR_INSTRUCTION_REF = "operator://agentic-graph/commerce-adapter-admission/2026-09-03";
export const AUTHORING_MUTATION_PERMIT_SCHEMA = "agentic-graph-authoring-mutation-permit/v2";
export const AUTHORING_OPERATION_SCHEMA = "agentic-graph-authoring-operation/v1";
export const COMMERCE_ADMISSION_SCOPE = "operator-registry";
export const COMMERCE_ADMISSION_WRITE_TARGET = "registry";
export const COMMERCE_ADMISSION_AUTH_SCHEMA = "commerce-acos-admission-auth/v1";
...
export async function commerceAdmissionAuthHeaders({ method, url, bodyDigest, headers, secret }) {
  if (!isCommerceAdmissionAuthSecret(secret)) throw new TypeError("ACOS admission authentication is unconfigured.");
  const input = createCommerceAdmissionAuthInput({ method, url, bodyDigest, headers });
  if (!input) throw new TypeError("ACOS admission authentication input is malformed.");
...
export function readCommerceAdmissionReadyEnvelope(value) {
  ...
  return deploymentIdentity ? Object.freeze({ ...value, deploymentIdentity }) : null;
}
