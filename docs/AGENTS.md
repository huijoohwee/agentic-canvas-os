...
- Permit simultaneous cross-device implementation only for different semantic scopes with distinct draft ownership pull requests. Same-scope work waits for a parked, exact-SHA handoff and a newer fencing epoch; only the delivering session may reclaim its exact remote SHA to revise a failed protected check.
...
- Require automatic challenge-bound attestation from the application-root identity owner through the dedicated authenticated room. The room, verifier, WebMCP reader, and Settings projection must not build, select, mutate, persist, or synchronize identity; `Copy diagnostic JSON` is optional troubleshooting only.
- Run `npm run collaboration:gate` as the sole collaboration compliance entry point. The command creates isolated local owner and guest browser contexts plus the storage worker; do not require physical devices, clipboard transfer, or runtime-identity JSON files.
- Require distinct session-bound device principals issued by the authenticated storage boundary; client-provided device labels, runtime ids, or additional tabs cannot establish a second device. Reset the bounded reconnect budget only after a stable connected window.
...
- Treat repository-owned immutable publication as an object lane, not a Dev checkout: it may validate and push one exact commit without switching or touching the canonical worktree, but it must emit and round-trip an exact app/docs/catalog manifest and cannot run or deploy the application.
...
- Treat `TODO.md` plus the active `../todo/YYYY-MM.md` shard as the sole planning authority for Knowgrph and every participating repository. Remove repository-local todo files and append one strict task row to the active shard.
