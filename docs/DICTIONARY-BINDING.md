...
| `@knowledge-graph` | One immutable digest-addressed local graph snapshot plus its source, parser, omission, and artifact manifest. | Knowgrph local MCP knowledge-graph artifact owner under the selected `@working-directory`. | The binding is not a database credential, global index, vector store, approval, or executable graph; stale digests fail closed, source files stay authored SSOT, and query or explanation cannot select or mutate another snapshot. |
...
| `/knowledge.graph.ingest #knowledge-graph @working-directory @knowledge-graph @operator @runtime-proof` | Bind one explicit workspace to one generated digest-addressed local graph snapshot. |
| `/knowledge.graph.query #knowledge-graph @knowledge-graph @runtime-proof` | Read one exact snapshot through bounded deterministic graph operations. |
| `/knowledge.graph.explain #knowledge-graph @knowledge-graph @runtime-proof` | Read one exact stored edge explanation and its canonical source evidence. |
...
| `@knowledge-graph` | `FACTS.md` direct-resolution entry for one immutable digest-bound local graph snapshot. |
