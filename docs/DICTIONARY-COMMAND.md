...
| `/knowgrph.probe-tree` | Generate bounded editable next-question branches from the selected Widget Card through the shared Probe-Tree runtime. | `@knowgrph.probe-tree`, `@source.frontmatter`, `@runtime-proof` | `#knowgrph.probe-tree`, `#runtime-ready`, `#token-economics` | MCP-first generation returns two to four forward-only Type 2 cards whose distinct selected-child focuses, numbered multi-select clarification suggestions, and verbatim context anchors derive from the selected child plus bounded lineage. The selected child owns continuation even when a same-ID root alias exists; bare focus fragments, repeated questions, reused choice labels, and reused or subset/superset choice sets fail closed, while Other, canonical Output, depth, token, provider approval, graph persistence, and zero-spend input-derived evidence remain explicit. |
