...
| `/pmf-agent` | Evaluate product-market-fit hypotheses from cited workspace evidence and convert problem-to-solution gaps into bounded experiments. | `@pmf-agent`, `@source.frontmatter`, `@source.body`, `@cost-log`, `@runtime-proof` | `#pmf-agent`, `#token-economics`, `#approval-gate`, `#vcc` | Output separates segments, jobs, pains, alternatives, promises, adoption and retention signals, willingness to pay, confidence, freshness, and unknowns; ranks gaps; defines falsifiable experiments with thresholds and stop conditions; and returns continue, iterate, pivot, or insufficient-evidence without inventing demand. |
