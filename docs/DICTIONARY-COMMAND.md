...
| `/knowgrph.probe-tree` | Generate bounded editable next-question branches from the selected Widget Card through the shared Probe-Tree runtime. | `@knowgrph.probe-tree`, `@source.frontmatter`, `@runtime-proof` | `#knowgrph.probe-tree`, `#runtime-ready`, `#token-economics` | MCP-first generation returns two to four forward-only branch cards plus one Rich Media ledger; depth, token, provider-approval, graph-persistence, and zero-spend fallback evidence remain explicit. |
