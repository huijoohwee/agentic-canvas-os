...
| `/media.immersive` | Inspect or control bounded immersive-media projection on the existing shared Canvas. | exactly `@canvas`; add `@media-url` only when setting a source | exactly `#canvas-media` | Dev-only browser-local WebMCP tools `knowgrph.inspect_local_immersive_media` and `knowgrph.control_local_immersive_media` validate source, view, navigation, transition, annotation, layer, overlay, and capture operations; Knowgrph retains the single renderer, camera/input, media, and persistence owners, while this dictionary adds no dependency, network, storage, Prod, or Cloudflare authority. |
