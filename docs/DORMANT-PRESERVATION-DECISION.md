...
- A planned journal remains immutable when protected `main` advances. Continuation may accept only a clean monotonic descendant whose exact changed-path set is disjoint from the planned manifest. Controller path, origin, clean state, and device-script digest remain historical-static identity; controller head, local and remote protected refs, and tree are protected-dynamic state and must all resolve to that same freshly verified descendant and its Git-derived tree. The candidate claim, lease, fence, single-parent lineage, manifest files, task identity, and attributed peer authorities remain exact; a non-descendant, overlapping path, mixed protected refs, tree mismatch, controller identity drift, or peer-authority drift fails before lease annotation. `verifyProtectedAdvance` and local peer verification bind the same current protected revision rather than mixing the historical base with the live canonical lane.
- A selected lane that already carries a repository-verified `retired-preserved` owner remains valid only when it is still `disjoint-attributed` and every selected worktree field matches the fresh operation-derived dormant-preservation receipt. Its null dormant-receipt projection is not owner loss: the verified retirement receipt remains authoritative. Other retired, released, unselected, structurally ambiguous, or projection-drifted lanes remain fail-closed.
