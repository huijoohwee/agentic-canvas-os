...
- A planned journal remains immutable when protected `main` advances. Continuation may accept only a clean monotonic descendant whose exact changed-path set is disjoint from the planned manifest. The candidate claim, lease, fence, single-parent lineage, controller, manifest files, task identity, and attributed peer authorities remain exact; a non-descendant, overlapping path, canonical identity drift, or peer-authority drift fails before lease annotation. `verifyProtectedAdvance` and local peer verification bind the same current protected revision rather than mixing the historical base with the live canonical lane.
