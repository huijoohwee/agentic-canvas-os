...
| `@knowledge-graph` | `DICTIONARY-BINDING.md` | Binding for one immutable digest-addressed Knowgrph local graph snapshot. | Snapshot identity is read-only context for query and explanation, carries no approval or credential, and cannot silently select or mutate another graph. |
...
| Deterministic codebase knowledge graph | Agentic Canvas OS owns exact invocation and proof semantics; Knowgrph local MCP owns bounded local parsing, immutable snapshot artifacts, deterministic query, and stored edge explanation, with source evidence for every edge and no model, embedding, vector store, external graph service, or Graphify runtime dependency. Dictionary lookup remains metadata-only. | `/knowledge.graph.ingest`, `/knowledge.graph.query`, `/knowledge.graph.explain`, `#knowledge-graph`, `@knowledge-graph`, `KNOWLEDGE-GRAPH.md`. |
