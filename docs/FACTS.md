...
runtime_claim: "source-backed facts and invocation definitions; native runtimes remain in typed source owners with no duplicate command runner, memory store, provider panel, or deploy claim"
...
| `/knowledge.graph.ingest` | `DICTIONARY-COMMAND.md` | Route from one explicit bounded workspace to `agentic_canvas_os.knowledge_graph.ingest`. | Build one deterministic digest-bound local snapshot from supported code, docs, SQL, configs, PDFs, and inert custom grammar artifacts; preserve sources and return typed omissions without model, network, embedding, vector, or external parser fallback. |
| `/knowledge.graph.query` | `DICTIONARY-COMMAND.md` | Route from one exact graph snapshot to `agentic_canvas_os.knowledge_graph.query`. | Return a bounded deterministically ordered subgraph with stored explanations and evidence or a typed empty result; reject stale snapshot identity. |
| `/knowledge.graph.explain` | `DICTIONARY-COMMAND.md` | Route from one exact edge identity to `agentic_canvas_os.knowledge_graph.explain`. | Return stored endpoints, relationship kind, deterministic explanation, parser/source span and excerpt, and extraction rule without reparsing, inference, generation, or mutation. |
| `#knowledge-graph` | `DICTIONARY-SEMANTIC.md` | Semantic route for native deterministic source-backed knowledge graphs. | Require stable graph identity, parser/source evidence, explained edges, deterministic operations, and absence of models, embeddings, vector stores, external parsers, and external graph services. |
| `@knowledge-graph` | `DICTIONARY-BINDING.md` | Binding for one exact digest-fenced native local graph snapshot view. | Snapshot identity is read-only context for query and explanation, carries no approval or credential, and becomes stale after an atomic pointer replacement. |
