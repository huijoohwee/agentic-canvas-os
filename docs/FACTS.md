...
source_revision_contract: {canonical_repository: "$GITHUB_ROOT/agentic-canvas-os", canonical_docs_root: "$GITHUB_ROOT/agentic-canvas-os/docs", canonical_ref: "origin/main", consumer_repository: "$GITHUB_ROOT/knowgrph", runtime_guard: "$GITHUB_ROOT/knowgrph/scripts/dev-source-consistency.mjs", canonical_rule: "the Agentic Canvas OS local supervisor may serve fixed local ports only from a clean registered main worktree at the accepted product source revision with required product checks; ADLC separately owns remote refresh, protected integration, and canonical synchronization; task worktrees never own canonical runtime ports", task_rule: "Knowgrph automatically selects task mode on contract-valid agent branches; explicit AG_DEV_SOURCE_MODE values remain expert overrides and task mode never relaxes the Agentic Canvas OS docs revision", parity_rule: "cross-device parity requires identical 40-character Knowgrph and Agentic Canvas OS commit SHAs; matching branch names, ports, or labels are informational and never sufficient", runtime_identity_rule: "every served runtime must visibly expose its Knowgrph revision, Agentic Canvas OS docs revision, catalog revision, deterministic catalog digest, catalog hydration state, and catalog entry counts", runtime_identity_owner_rule: "one application-root runtime owns the canonical Knowgrph identity snapshot globally; route catalogs, panels, and settings views may publish or project facets but must not instantiate competing identity owners", runtime_identity_projection_rule: "MainPanel Settings exposes Cross-device Identity Gate as a normal section inside the Settings body using the shared Key-Type-Value row layout; it must not render above or replace the KTV header", runtime_identity_attestation_rule: "the repository-owned collaboration gate creates two isolated authenticated runtime peers, relays short-lived digest-bound canonical snapshots without owning or persisting identity, and passes only on exact SHA, catalog-digest, catalog-count, active-peer, and common-digest parity; physical devices and manual JSON exports are not required", catalog_freshness_rule: "catalog hydration is keyed by the Agentic Canvas OS docs revision; every sigil response must agree on exact full-catalog counts and one deterministic SHA-256 digest, the browser must recompute that digest after all three source-backed slices replace prior state, and a revision change invalidates the prior catalog with at most two explicit refresh attempts before returning a visible blocked or stale result", production_owner_rule: "Knowgrph remains the sole production and mirror owner for airvio.co product surfaces; the protected ACOS controller owns only the agentic-canvas-os Worker and its private admission provider after exact GitHub production authorization", production_attestation_rule: "both public surfaces expose byte-identical agenticgraph-production-runtime-readiness/v2 JSON binding exact app tree, docs, catalog, manifest, artifact, mirror, and surface identities; the separate ACOS Worker exposes its exact Cloudflare deployment identity", workspace_seed_authority_rule: "knowgrph/docs/workspace-seeds is the only authored workspace-seed source; agentic-canvas-os/docs/workspace-seeds may contain an exact-byte default-storage projection, while publish repositories must contain no editable workspace-seed copy"}
...
| Cross-device parity | Devices are in parity only when their visible runtime identities report identical exact Knowgrph and Agentic Canvas OS commit SHAs. A shared branch name, port, route, or device label is not revision proof. | `source_revision_contract` above, the pinned ADLC start workflow, and `VALIDATION-RUNBOOK.md`. |
...
| Deployment boundary | Knowgrph owns airvio.co deployment and mirror publication. The separate protected ACOS controller may mutate only the agentic-canvas-os Worker after exact protected-main, first-attempt GitHub production-environment authorization and exact Cloudflare identity readback; the pinned ADLC release workflow separately owns repository integration, retirement, synchronization, and cleanup. Any candidate, source, authorization, baseline, binding, readiness, or recovery drift fails closed. | `PROTECTED-ACOS-PRODUCTION-RELEASE.md`, `scripts/acos-production-release-controller.mjs`, the pinned ADLC release workflow, `AGENTS.md`, and `RUNTIME-READINESS.md`. |
