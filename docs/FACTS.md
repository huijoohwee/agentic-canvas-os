...
source_revision_contract:
  ...
  canonical_rule: "every normal local Dev port uses the clean registered main worktree whose HEAD equals fetched origin/main; distinct semantic scopes may mutate concurrently in registered branch-exclusive task worktrees, while npm run dev:latest remains the explicit clean-main-only, two-phase, fast-forward-only refresh path"
