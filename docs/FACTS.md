...
| Agent orchestration, Sandbox Agents, and long-horizon SuperAgent inspiration | External orchestration, handoff, sandbox, and SuperAgent systems are pattern references only. This repo owns explicit branch answer ownership, a neutral sandbox control plane, and independently authored Docker, policy, state, and verification owners. | `AGENT-ORCHESTRATION.md`, `SANDBOX-AGENTS.md`, plus cited official sources. |
...
| External pattern boundary | Official OpenAI guides, Docker documentation, LangGraph, DeerFlow, Kimi Agent Swarm, NVIDIA NeMo Agent Toolkit, CrewAI, Tool Gateway, Tool Search, and Repomix may inform abstract semantics or platform usage only; local docs and runtime must not copy or depend on external code, APIs, schemas, tests, examples, prompts, vocabulary sets, provider configs, runtime layouts, defaults, algorithms, services, packages, binaries, or prose. | `AGENT-RUNTIME-COMPOSITION.md`, `AGENT-ORCHESTRATION.md`, `AGENT-TEAM.md`, `AGENT-SWARM.md`, `AGENT-TOOLKIT.md`, `SANDBOX-AGENTS.md`, `TOOL-SEARCH.md`, `REPOSITORY-PACKING.md`, `VALIDATION-RUNBOOK.md`, `HARNESS-CONTRACTS.md`. |
## Resolution Rules
