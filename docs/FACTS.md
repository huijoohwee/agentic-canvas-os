...
direct_resolution:
  ...
  "/pmf-agent": "DICTIONARY-COMMAND.md#/pmf-agent"
  ...
  "#pmf-agent": "DICTIONARY-SEMANTIC.md##pmf-agent"
  ...
  "@pmf-agent": "DICTIONARY-BINDING.md#@pmf-agent"
...
| `/pmf-agent` | `DICTIONARY-COMMAND.md` | Product-market-fit hypothesis evaluation route derived from the HackaMap problem-to-solution gap concept. | Use cited evidence, keep assumptions and unknowns explicit, rank gaps, define falsifiable experiments and thresholds, and never infer PMF from hackathon attention alone. |
| `#pmf-agent` | `DICTIONARY-SEMANTIC.md` | Semantic route for evidence-led PMF analysis and gap mapping. | Keep segment, problem, alternative, solution, adoption, retention, willingness-to-pay, confidence, and freshness signals distinct. |
| `@pmf-agent` | `DICTIONARY-BINDING.md` | Binding for the PMF analysis role and centralized preset. | Analysis may propose experiments but cannot invent customer evidence, contact users, spend, mutate product state, persist results, or declare PMF without evidence and approval. |
...
| Planning authority | `TODO.md` and the active `todo/YYYY-MM.md` shard are the sole live planning SSOT for Knowgrph and every participating repository; repository-local ledgers are forbidden. | `START-WORKFLOW.md`, `RELEASE-WORKFLOW.md`, `VALIDATION-RUNBOOK.md`. |
