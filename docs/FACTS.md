...
| Implementation-run observation | The pinned `agentic-os` ADLC harness owns repository lifecycle. ACOS retains only the read-only, non-promoting `agentic-graph.adlc.observe` projection through existing KGC, GraphData, and Canvas owners. Its native `adlc-ledger-receipt/v1` preserves original source schema; bounded historical reads retain exact provenance. Missing evaluators fail closed before projection. It never repairs or grades its source and grants no lifecycle authority. `verified`, `delivery_ready`, and `deployed` remain distinct externally owned claims and are never inferred. | `/adlc.observe`, `#adlc-observability`, `@implementation-run`, `@canvas`, `@runtime-proof`, `IMPLEMENTATION-RUN-OBSERVATION.md`. |
