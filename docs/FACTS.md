...
source_revision_contract: {canonical_repository: "$GITHUB_ROOT/agentic-canvas-os", canonical_docs_root: "$GITHUB_ROOT/agentic-canvas-os/docs", canonical_ref: "origin/main", consumer_repository: "$GITHUB_ROOT/agentic-graph", runtime_guard: "$GITHUB_ROOT/agentic-graph/scripts/dev-source-consistency.mjs", canonical_rule: "the Agentic Canvas OS local supervisor may serve fixed local ports only from a clean registered main worktree at the accepted product source revision with required product checks; ADLC separately owns remote refresh, protected integration, and canonical synchronization; task worktrees never own canonical runtime ports", task_rule: "agentic-graph automatically selects task mode on contract-valid agent branches; explicit AG_DEV_SOURCE_MODE values remain expert overrides and task mode never relaxes the Agentic Canvas OS docs revision", parity_rule: "cross-device parity requires identical 40-character agentic-graph and Agentic Canvas OS commit SHAs; matching branch names, ports, or labels are informational and never sufficient", runtime_identity_rule: "every served runtime must visibly expose its agentic-graph revision, Agentic Canvas OS docs revision, catalog revision, deterministic catalog digest, catalog hydration state, and catalog entry counts", runtime_identity_owner_rule: "one application-root runtime owns the canonical agentic-graph identity snapshot globally; route catalogs, panels, and settings views may publish or project facets but must not instantiate competing identity owners", runtime_identity_projection_rule: "MainPanel Settings exposes Cross-device Identity Gate as a normal section inside the Settings body using the shared Key-Type-Value row layout; it must not render above or replace the KTV header", runtime_identity_attestation_rule: "the repository-owned collaboration gate creates two isolated authenticated runtime peers, relays short-lived digest-bound canonical snapshots without owning or persisting identity, and passes only on exact SHA, catalog-digest, catalog-count, active-peer, and common-digest parity; physical devices and manual JSON exports are not required", catalog_freshness_rule: "catalog hydration is keyed by the Agentic Canvas OS docs revision; every sigil response must agree on exact full-catalog counts and one deterministic SHA-256 digest, the browser must recompute that digest after all three source-backed slices replace prior state, and a revision change invalidates the prior catalog with at most two explicit refresh attempts before returning a visible blocked or stale result", production_owner_rule: "agentic-graph is the sole production build, Cloudflare deploy, live-fidelity, rollback, and mirror-publication owner for airvio.co; Agentic Canvas OS has no production deploy workflow", production_attestation_rule: "both public surfaces expose byte-identical agentic-os-production-runtime-readiness/v2 JSON binding exact app tree, docs, catalog, manifest, artifact, mirror, and surface identities", workspace_seed_authority_rule: "agentic-graph/docs/workspace-seeds is the only authored workspace-seed source; agentic-canvas-os/docs/workspace-seeds may contain an exact-byte default-storage projection, while publish repositories must contain no editable workspace-seed copy"}
layer_contract: {soul: "durable agent identity and voice", facts: "shared truth and precedence", memory: "persistence, routing memory, and reusable local context", planning: "bounded private index plus immutable Context records and generated Kanban view", user: "bounded user preferences, communication style, and expectations", skills: "on-demand procedural knowledge and progressive disclosure contracts", agents: "agent roles, editing rules, and operational behavior", dictionaries: "direct token definitions for /, #, and @ invocation grammar"}
...
prompt_preset_contract: {selection_alias_suffix: "-prompt-preset", selection_alias_authority: "PROMPT-PRESETS.md", runtime_command_authority: "SKILLS.md and DICTIONARY-COMMAND.md", execution_boundary: "selection resolves and loads the source-backed runtime prompt without submit; Send remains the execution boundary", semantic_contract_authority: "PROBE-TREE.md", invocation_routes: "llm-chat-response uses the active Chat provider, endpoint, and model; native-chat-response uses the named shared runtime; mcp-invocation resolves the matching runtime command through agentic-graph.agentic_canvas_os.docs.invoke without executing it"}
external_pattern_sources: [ "https://github.com/NousResearch/hermes-agent", "https://hermes-agent.nousresearch.com/docs/", "https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/features/personality.md", "https://hermes-agent.nousresearch.com/docs/guides/use-soul-with-hermes", "https://hermes-agent.nousresearch.com/docs/user-guide/features/memory", "https://hermes-agent.nousresearch.com/docs/user-guide/features/skills", "https://hermes-agent.nousresearch.com/docs/user-guide/features/context-files", "https://hermes-agent.nousresearch.com/docs/user-guide/features/context-references", "https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway", "https://hermes-agent.nousresearch.com/docs/user-guide/features/tools", "https://developers.openai.com/api/docs/guides/tools-tool-search", "https://developers.openai.com/api/docs/guides/agents/guardrails-approvals", "https://agentskills.io/specification", "https://learn.chatgpt.com/docs/customization/overview", "https://hermes-agent.nousresearch.com/docs/user-guide/features/mixture-of-agents", "https://github.com/NousResearch/hermes-agent-self-evolution", "https://github.com/langchain-ai/langgraph", "https://docs.langchain.com/oss/python/langgraph/overview", "https://docs.langchain.com/oss/python/langgraph/graph-api", "https://github.com/bytedance/deer-flow", "https://github.com/NVIDIA/openshell", "https://www.kimi.com/help/agent/agent-swarm", "https://github.com/NVIDIA/NeMo-Agent-Toolkit", "https://docs.nvidia.com/nemo/agent-toolkit/latest/", "https://docs.nvidia.com/nemo/agent-toolkit/latest/improve-workflows/profiler.html", "https://docs.nvidia.com/nemo/agent-toolkit/latest/improve-workflows/optimizer.html", "https://github.com/jamiepine/voicebox", "https://github.com/yamadashy/repomix" ]
direct_resolution:
  "/python.learning": "DICTIONARY-COMMAND.md#/python.learning"
  "/adlc.observe": "DICTIONARY-COMMAND.md#/adlc.observe"
  "/application.compose": "DICTIONARY-COMMAND.md#/application.compose"
  ...
  "/ecs.world-tick": "DICTIONARY-COMMAND.md#/ecs.world-tick"
  "/ecs.decision-persist": "DICTIONARY-COMMAND.md#/ecs.decision-persist"
  ...
  "/voice.studio": "DICTIONARY-COMMAND.md#/voice.studio"
  ...
  "/launch-copilot": "DICTIONARY-COMMAND.md#/launch-copilot"
  "/ingest-url": "DICTIONARY-COMMAND.md#/ingest-url"
  "/image.to-threejs": "DICTIONARY-COMMAND.md#/image.to-threejs"
  "/image.to-glb": "DICTIONARY-COMMAND.md#/image.to-glb"
  "/asset.create": "DICTIONARY-COMMAND.md#/asset.create"
  "/camera.select": "DICTIONARY-COMMAND.md#/camera.select"
  "/camera.frame": "DICTIONARY-COMMAND.md#/camera.frame"
  "/camera.animate": "DICTIONARY-COMMAND.md#/camera.animate"
  "/camera.play": "DICTIONARY-COMMAND.md#/camera.play"
  "/camera.scrub": "DICTIONARY-COMMAND.md#/camera.scrub"
  "/xr.stage": "DICTIONARY-COMMAND.md#/xr.stage"
  "/xr.place": "DICTIONARY-COMMAND.md#/xr.place"
  "/xr.transform": "DICTIONARY-COMMAND.md#/xr.transform"
  "/xr.label": "DICTIONARY-COMMAND.md#/xr.label"
  "/xr.remove": "DICTIONARY-COMMAND.md#/xr.remove"
  "/xr.physics": "DICTIONARY-COMMAND.md#/xr.physics"
  "/xr.present": "DICTIONARY-COMMAND.md#/xr.present"
  "/animation.control": "DICTIONARY-COMMAND.md#/animation.control"
  "/motion.control": "DICTIONARY-COMMAND.md#/motion.control"
  "/game.mode": "DICTIONARY-COMMAND.md#/game.mode"
  "/agentic-graph.probe-tree": "DICTIONARY-COMMAND.md#/agentic-graph.probe-tree"
  ...
  "/instruction.quality-evaluate": "DICTIONARY-COMMAND.md#/instruction.quality-evaluate"
  ...
  "/git.run": "DICTIONARY-COMMAND.md#/git.run"
  "/file.sync": "DICTIONARY-COMMAND.md#/file.sync"
  ...
  "#instruction-quality": "DICTIONARY-SEMANTIC.md##instruction-quality"
  "#image-to-threejs": "DICTIONARY-SEMANTIC.md##image-to-threejs"
  "#image-to-glb": "DICTIONARY-SEMANTIC.md##image-to-glb"
  "#procedural-asset": "DICTIONARY-SEMANTIC.md##procedural-asset"
  "#agentic-graph.probe-tree": "DICTIONARY-SEMANTIC.md##agentic-graph.probe-tree"
  "#camera": "DICTIONARY-SEMANTIC.md##camera"
  "#camera-shot": "DICTIONARY-SEMANTIC.md##camera-shot"
  "#camera-motion": "DICTIONARY-SEMANTIC.md##camera-motion"
  "#character-motion": "DICTIONARY-SEMANTIC.md##character-motion"
  "#pose": "DICTIONARY-SEMANTIC.md##pose"
  "#gameplay": "DICTIONARY-SEMANTIC.md##gameplay"
  "#action-path": "DICTIONARY-SEMANTIC.md##action-path"
  "#transform": "DICTIONARY-SEMANTIC.md##transform"
  "#world": "DICTIONARY-SEMANTIC.md##world"
  "#body": "DICTIONARY-SEMANTIC.md##body"
  "#impulse": "DICTIONARY-SEMANTIC.md##impulse"
  "#controller": "DICTIONARY-SEMANTIC.md##controller"
  "#reticle": "DICTIONARY-SEMANTIC.md##reticle"
  ...
  "#adlc-observability": "DICTIONARY-SEMANTIC.md##adlc-observability"
  "#application-composition": "DICTIONARY-SEMANTIC.md##application-composition"
  ...
  "#agentic-ecs": "DICTIONARY-SEMANTIC.md##agentic-ecs"
  ...
  "#voice-clone": "DICTIONARY-SEMANTIC.md##voice-clone"
  "#speech-to-text": "DICTIONARY-SEMANTIC.md##speech-to-text"
  ...
  "#git-remote": "DICTIONARY-SEMANTIC.md##git-remote"
  "#multi-provider-file-sync": "DICTIONARY-SEMANTIC.md##multi-provider-file-sync"
  "#repository-packing": "DICTIONARY-SEMANTIC.md##repository-packing"
  ...
  "@agentic-graph.probe-tree": "DICTIONARY-BINDING.md#@agentic-graph.probe-tree"
  "@scene": "DICTIONARY-BINDING.md#@scene"
  "@canvas": "DICTIONARY-BINDING.md#@canvas"
  "@camera": "DICTIONARY-BINDING.md#@camera"
  "@selected-actor": "DICTIONARY-BINDING.md#@selected-actor"
  ...
  "@repository-root": "DICTIONARY-BINDING.md#@repository-root"
  ...
  "@local-git-repository": "DICTIONARY-BINDING.md#@local-git-repository"
  "@git-remote": "DICTIONARY-BINDING.md#@git-remote"
  "@persisted-cache": "DICTIONARY-BINDING.md#@persisted-cache"
  "@file-sync-provider": "DICTIONARY-BINDING.md#@file-sync-provider"
  ...
  "@implementation-run": "DICTIONARY-BINDING.md#@implementation-run"
  "@application-manifest": "DICTIONARY-BINDING.md#@application-manifest"
  "@component-catalog": "DICTIONARY-BINDING.md#@component-catalog"
  "@integration-profile": "DICTIONARY-BINDING.md#@integration-profile"
  ...
  "@ecs-session": "DICTIONARY-BINDING.md#@ecs-session"
...
truth_tokens:
  commands: ["/launch-copilot", "/application.compose", "/agentic.graph.ingest", "/agentic.graph.parser.generate", "/agentic.graph.query", "/agentic.graph.explain", "/ecs.session-start", "/ecs.world-tick", "/ecs.decision-persist", "/query", "/soul.load", "/personality.overlay", "/moa", "/video-agent", "/voice.studio", "/sme-care-agent", "/investment-research-agent", "/crawler-agent", "/ingest-url", "/image.to-threejs", "/image.to-glb", "/asset.create", "/agentic-graph.probe-tree", "/camera.select", "/xr.stage", "/xr.place", "/xr.transform", "/xr.label", "/xr.remove", "/xr.physics", "/xr.present", "/animation.control", "/motion.control", "/game.mode", "/memory.seed", "/memory.write", "/memory.compact", "/memory.search", "/session.search", "/user.profile", "/skill.discover", "/skill.load", "/skill.bundle", "/skill.manage", "/instruction.audit", "/instruction.quality-evaluate", "/context.discover", "/context.load", "/context.audit", "/reference.expand", "/reference.audit", "/kanban.task", "/kanban.handoff", "/kanban.sync", "/tool.catalog", "/tool.route", "/tool.provider.select", "/tool.gateway.audit", "/toolset.enable", "/toolset.disable", "/tool.search", "/tool.describe", "/tool.call", "/experience.capture", "/skill.propose", "/skill.evolve", "/identity.reflect", "/orchestration.graph", "/agent.team", "/agent.swarm", "/agent.toolkit", "/state.checkpoint", "/human.review", "/stream.trace", "/superagent.run", "/sandbox.policy.validate", "/sandbox.policy.authorize", "/git.run", "/file.sync", "/repository.pack", "/prd-tad.create", "/runtime-ready.check", "/adlc.observe", "/release.complete", "/deploy.guard"]
  semantics: ["#agentic-ecs", "#truth", "#frontmatter", "#harness", "#token-economics", "#spec.low", "#spec.medium", "#spec.high", "#thinking.type.enabled", "#thinking.type.disabled", "#thinking.type.auto", "#token-cap.low", "#token-cap.medium", "#token-cap.high", "#vcc", "#no-hardcode", "#soul", "#primary-identity", "#personality-overlay", "#mixture-of-agents", "#reference-agents", "#aggregator-agent", "#learning-loop", "#persistent-memory", "#user-profile", "#frozen-snapshot", "#memory-capacity", "#session-search", "#skill-system", "#instruction-audit", "#instruction-quality", "#image-to-threejs", "#image-to-glb", "#procedural-asset", "#agentic-graph.probe-tree", "#character-motion", "#pose", "#gameplay", "#action-path", "#transform", "#world", "#body", "#impulse", "#controller", "#reticle", "#progressive-disclosure", "#skill-bundle", "#agentskills-compatible", "#skill-security", "#context-file", "#project-context", "#cwd-discovery", "#context-reference", "#inline-context", "#attached-context", "#kanban-board", "#task-row", "#profile-handoff", "#worker-process", "#multi-agent-collaboration", "#adlc-observability", "#application-composition", "#agentic-graph", "#parser-generation", "#role-based-agent-team", "#tool-gateway", "#tool-routing", "#tool-function", "#toolset", "#platform-toolset", "#tool-search", "#deferred-tool-schema", "#bridge-tool", "#web-search", "#image-generation", "#voice-clone", "#speech-to-text", "#text-to-speech", "#cloud-browser", "#skill-evolution", "#memory-search", "#identity-model", "#orchestration-graph", "#agent-swarm", "#agent-toolkit", "#stateful-agent", "#durable-execution", "#human-in-loop", "#long-horizon-harness", "#sandboxed-workspace", "#agent-sandbox-policy", "#message-gateway", "#git-remote", "#multi-provider-file-sync", "#repository-packing"]
  bindings: ["@ecs-session", "@agent", "@operator", "@source.frontmatter", "@source.body", "@local-harness", "@runtime-proof", "@instruction-source", "@instruction-eval-suite", "@soul-profile", "@identity-slot", "@personality-overlay", "@moa-preset", "@reference-agents", "@aggregator-agent", "@video-generation-demo-script", "@provider.byteplus", "@provider.openai", "@text", "@image", "@image-to-threejs", "@image-to-glb", "@agentic-graph.probe-tree", "@scene", "@canvas", "@audio", "@voice-profile", "@video", "@experience", "@memory-store", "@memory-entry", "@memory-snapshot", "@memory-policy", "@user-profile", "@session-index", "@skill-index", "@skill-source", "@skill-reference", "@skill-bundle", "@skill-policy", "@context-file", "@working-directory", "@repository-root", "@context-policy", "@file:", "@folder:", "@diff", "@staged", "@git:", "@local-git-repository", "@git-remote", "@persisted-cache", "@file-sync-provider", "@url:", "@reference-policy", "@attached-context", "@kanban-board", "@task-row", "@implementation-run", "@application-manifest", "@component-catalog", "@integration-profile", "@agentic-graph", "@parser-specification", "@agent-team", "@handoff-row", "@agent-profile", "@worker-process", "@tool-gateway", "@tool-provider", "@tool-function", "@toolset", "@platform-surface", "@deferred-tool-catalog", "@bridge-tool", "@web-search-tool", "@image-tool", "@tts-tool", "@browser-tool", "@tool-policy", "@skill-catalog", "@identity-model", "@orchestration-graph", "@swarm-run", "@agent-toolkit-observer", "@state-store", "@checkpoint-store", "@human-review", "@sandbox-workspace", "@sandbox-policy", "@message-gateway"]
---
# Agentic OS Facts
...
| Prompt preset selection | FloatingPanel Prompt Presets expose catalog-owned selection aliases while executable runtime routes remain owned by `SKILLS.md` and `DICTIONARY-COMMAND.md`; selection loads without submit, LLM responses use the active Chat tuple, and MCP resolves the same runtime token read-only through `agentic-graph.agentic_canvas_os.docs.invoke`. | `PROMPT-PRESETS.md`, `SKILLS.md`, `DICTIONARY-COMMAND.md`. |
...
| Canonical local runtime | The Agentic Canvas OS supervisor serves agentic-graph only when both canonical worktrees are clean at the accepted product source revisions with required product checks. Its private token owns fixed Apex `5173` and storage `8787`; unmanaged listeners fail closed. Remote refresh and repository synchronization remain ADLC-owned. | `source_revision_contract` above, `scripts/local-runtime.mjs`, and `docs/schemas/local-runtime-readiness.v1.schema.json`. |
| Cross-device parity | Devices are in parity only when their visible runtime identities report identical exact agentic-graph and Agentic Canvas OS commit SHAs. A shared branch name, port, route, or device label is not revision proof. | `source_revision_contract` above, the pinned ADLC start workflow, and `VALIDATION-RUNBOOK.md`. |
| Visible runtime identity | The running surface must expose exact `agenticGraphRevision`, `agenticCanvasOsRevision`, `catalogRevision`, `catalogDigest`, `catalogHydration.status`, `catalogHydration.attempts`, and `/`, `#`, `@` counts. Hidden build metadata or terminal-only branch output is insufficient. | `VALIDATION-RUNBOOK.md` runtime identity schema and `RUNTIME-PROOF.md`. |
...
| Settings identity projection | `Cross-device Identity Gate` is a normal collapsible MainPanel Settings body section that uses the shared KTV row contract. A standalone gate above the KTV header, in Skills & Commands, or rendered through a custom non-KTV identity table fails compliance. | `npm run collaboration:gate` and the pinned ADLC start workflow. |
| Automatic identity attestation | The application-root reporter reads the canonical identity store and answers short-lived authenticated-room challenges. The room relays but never builds, mutates, selects, synchronizes, or persists identity. At least two distinct authenticated peer and runtime identities, valid TTL/digests, fresh bounded hydration, exact app/docs/catalog SHAs, exact catalog SHA-256 digest, and exact `/`, `#`, `@` counts are required. | `npm run collaboration:gate` and `VALIDATION-RUNBOOK.md`. |
| Repository-owned collaboration gate | `npm run collaboration:gate` in Agentic Canvas OS allocates run-scoped ports, credentials, external persistence, proof, and diagnostics before delegating to agentic-graph's canonical readiness owner; concurrent proofs cannot reuse or stop canonical `5173`/`8787`, and cleanup is allocation-owned. A nonzero result blocks parity, review, integration, and release but does not revoke an otherwise valid admitted ADLC lane. | `scripts/collaboration-gate.mjs`, the pinned ADLC harness, and `VALIDATION-RUNBOOK.md`. |
| ADLC lane grammar | `agent/<device>/<semantic-scope>` is the single task-branch identity. `agentic-os` validates it and selects the verified protected base before creating a registered worktree; ACOS has no second identity or authority system. | The pinned `agentic-os` ADLC guideline/start workflow and package. |
| Multi-user, multi-device lifecycle authority | The remotely addressable ADLC branch and pull request are the shared claim. Provider ordering, exact-head checks, integration proof, and exact clean retirement are distinct receipts. Dirty, untracked, ambiguous, or concurrently owned bytes are preserved; no local marker, queue state, or projection grants protected integration, Production, deployment, or cleanup authority. | The pinned `agentic-os` ADLC guideline/release workflow and `__tests__/adlc-compatibility.test.mjs`. |
...
| Catalog freshness | `catalogRevision` must equal `agenticCanvasOsRevision`; every `/`, `#`, and `@` MCP slice must report the same full-catalog counts and `catalogDigest`; and the assembled replacement catalog must recompute to that SHA-256 digest. Hydration caches use the revision as part of their key, invalidate on revision change, and stop after no more than two explicit refresh attempts with a visible `fresh`, `blocked`, or `stale` result. | `VALIDATION-RUNBOOK.md` and the pinned ADLC release workflow. |
...
| Deployment boundary | The ACOS product contract binds the reviewed source, artifact, and immutable manifest to one candidate and one fresh human Production decision. agentic-graph owns deployment, live verification, rollback, and mirror publication; the pinned ADLC release workflow separately owns repository integration, retirement, synchronization, and cleanup. Any candidate, source, product-policy, artifact, manifest, authorization, target, or observed-result drift fails closed. | `scripts/production-release-authorization-contract.mjs`, the pinned ADLC release workflow, `AGENTS.md`, and `RUNTIME-READINESS.md`. |
| Private commerce admission | `agentic-os-admission.internal` is a private admission provider, never a public fallback route. New writes require caller HMAC plus one signed, exact, unexpired local configuration envelope that binds the admission intent, inputs, and permit; persisted records retain only its normalized non-secret projection. This ACOS component neither issues authenticated agentic-graph release authority nor owns a Cloudflare deployment action. | `agent-api/src/commerce-admission-authority.js`, `worker/index.js`, `RUNTIME-PROOF.md`, and the production-owner rule in this frontmatter. |
...
| Kanban collaboration inspiration | Context-reference patterns may inform how agents attach board rows or context refs, but the private workspace owns immutable Context records and projects `docs/kanban.md` through its shared utilities. | This file, dictionaries, `huijoohwee/.workspace/.todo/docs/kanban.md`, and `HARNESS-CONTRACTS.md`. |
| Tool gateway, toolset, and search inspiration | External gateway, registry, and Tool Search systems are pattern references only. This repo may adopt neutral per-tool routing, callable-function grouping, platform scope, and session-scoped definition deferral while preserving real gateway policy. | Official Hermes Agent and OpenAI guides listed in frontmatter. |
...
| Stateful and long-horizon orchestration inspiration | External graph, handoff, sandbox, and SuperAgent systems are pattern references only. This repo may adopt neutral state, checkpoint, trace, review, ownership, sandbox, and verification contracts through independently authored owners. | `AGENT-ORCHESTRATION.md`, `SANDBOX-AGENTS.md`, plus cited official sources. |
...
| `/git.run`<br>`/file.sync` | `DICTIONARY-COMMAND.md` | Browser Git repository and remote-transport route.<br>Bidirectional browser file or directory synchronization route. | Execute only through browser WebMCP with active persistence; keep remote credentials in the authenticated Dev Worker relay and make local stdio a typed handoff only.<br>Return per-file outcomes through the provider-neutral browser owner; keep credentials and upstream resource identities behind the authenticated Dev Worker relay. |
| `#git-remote`<br>`#multi-provider-file-sync` | `DICTIONARY-SEMANTIC.md` | Semantic route for browser Git object/ref and remote operations.<br>Semantic route for provider-neutral pull and push. | Prove exact hashes, bounds, authority checks, typed conflicts, and Worker-only credentials.<br>Prove capability, pagination, hash behavior, conflict detection, bounds, offline FIFO, and secret-free browser payloads. |
| `@local-git-repository`, `@git-remote`<br>`@persisted-cache`, `@file-sync-provider` | `DICTIONARY-BINDING.md` | Browser Git state and opaque remote alias bindings.<br>Browser binary cache and opaque provider alias bindings. | Keep local repository state browser-owned and remote transport authenticated, allowlisted, and Worker-owned.<br>Require active persistence and provider-capability proof without browser credentials or upstream resource ids. |
...
| `/memory.write`<br>`/memory.compact` | `DICTIONARY-COMMAND.md` | Route for adding, replacing, or removing bounded memory entries.<br>Route for consolidating memory when bounded capacity would overflow. | Require target, action, evidence, capacity check, scan, and typed result before persistence.<br>Merge or remove stale entries deliberately; never silently drop content. |
...
| `/skill.discover`<br>`/skill.load` | `DICTIONARY-COMMAND.md` | Route for zero-spend skill metadata discovery.<br>Route for loading one on-demand skill and optional referenced resource. | Return name, description, category, source, trust, and compatibility without loading full skill bodies.<br>Load full source only after selection; load referenced files only when required; reject deep reference chains. |
| `/skill.bundle`<br>`/skill.manage` | `DICTIONARY-COMMAND.md` | Route for grouping several installed skills behind one command.<br>Route for create, patch, edit, delete, or supporting-file changes. | Resolve all listed skills, report skipped missing skills, and keep bundle instructions bounded.<br>Require source evidence, scan, validation, operator review when gated, and no direct auto-commit. |
...
| `/instruction.quality-evaluate` | `DICTIONARY-COMMAND.md` | Route for provenance-bound final-answer task-quality screening. | Score every registered case, fail on unsafe or missing behavior, and retain human review. |
| `/context.discover`<br>`/context.load` | `DICTIONARY-COMMAND.md` | Route for discovering project-local context files from a working directory.<br>Route for loading one scanned and bounded context file. | Use scoped startup and touched-path discovery, first-match project context, zero model spend, and no mutation.<br>Scan for injection and secrets, truncate within policy, and return loaded or blocked state. |
...
| `/reference.expand`<br>`/reference.audit` | `DICTIONARY-COMMAND.md` | Route for expanding explicit inline context references into appended context packets.<br>Route for inspecting context-reference expansion source, size, warnings, and safety. | Resolve only approved forms, preserve original message text, scan and bound content, and return typed warnings or refusals before injection.<br>Report supported platform, refused targets, truncation, source metadata, and proof without fetching or mutating extra content. |
| `/kanban.task` | `DICTIONARY-COMMAND.md` | Route for recording a task Context through [TODO.md](TODO.md); the private workspace generates `docs/kanban.md`. | Validate row schema through shared table/Kanban utilities; require owner profile, status, evidence, acceptance, and next action. |
...
| `/kanban.sync` | `DICTIONARY-COMMAND.md` | Route for reconciling board rows across worker processes. | Preserve immutable Context records as source, regenerate the private workspace board, and surface conflicts. |
...
| `#instruction-quality` | `DICTIONARY-SEMANTIC.md` | Semantic route for observable instruction task-quality evidence. | Evaluate final answers under a named revision without reading private reasoning or claiming general model quality. |
...
| `#kanban-board` | `DICTIONARY-SEMANTIC.md` | Semantic route for the durable shared task board. | Board rows stay in `huijoohwee/.workspace/.todo/docs/kanban.md` and render through existing multi-dimensional table/Kanban utilities. |
...
| `#profile-handoff` | `DICTIONARY-SEMANTIC.md` | Semantic route for profile-to-profile transfer. | Handoffs are owner-validated successor Context records that authorized profiles can read, not private chat context. |
...
| `@instruction-eval-suite` | `DICTIONARY-BINDING.md` | Binding for the repository-owned instruction-quality scenario suite. | Read-only rubric input; model execution, approval, and deployment remain separate. |
...
| `@url:` | `DICTIONARY-BINDING.md` | Binding form for bounded URL content context or source import. | Requires approved egress, no embedded credentials, citation/cache metadata, and size bounds. |
...
| `@kanban-board` | `DICTIONARY-BINDING.md` | Binding for `huijoohwee/.workspace/.todo/docs/kanban.md`. | Board is a derived read view; writes resolve through TODO.md to immutable Context records. |
...
| `#tool-gateway` | `DICTIONARY-SEMANTIC.md` | Semantic route for routing tool calls through existing `agentic-graph` infrastructure. | Use local MCP, Pages HTTP MCP, Browser WebMCP, or Cloudflare control-plane owners already present; do not add a new proxy. |
...
| `@tool-gateway` | `DICTIONARY-BINDING.md` | Binding for the selected `agentic-graph` tool routing surface. | Must be existing infrastructure; no new gateway service or deploy claim without approval. |
...
| Fact | Rule | Proof or source |
|---|---|---|
| Tool gateway routing | Tool calls route through existing `agentic-graph` infrastructure surfaces: local MCP, Pages HTTP MCP, Browser WebMCP, or approved Cloudflare control-plane owners. | `/tool.route`, `#tool-gateway`, `@tool-gateway`. |
...
| URL intake and context references | Explicit `@file:`, `@folder:`, `@diff`, `@staged`, `@git:`, and `@url:` references may append bounded context on supported surfaces; `/ingest-url @url:https://example.com @reference-policy #canvas` is the exact four-token source-import invocation and delegates only to `agentic-graph.control_local_import_url`, while `agentic-graph.agentic_canvas_os.docs.invoke` remains read-only discovery. | `/ingest-url`, `/reference.expand`, `#canvas`, `#context-reference`, `@url:`, `@reference-policy`. |
| Reference safety | Reference expansion is workspace-scoped where applicable, blocks sensitive paths, rejects binary content, enforces soft warnings and hard refusals, and preserves raw text on unsupported surfaces. Missing, invalid, unsupported, over-limit, or unsafe references return typed warnings or refusals without silent mutation or downstream parser patches. | `@attached-context`, `#inline-context`, `VALIDATION-RUNBOOK.md`. |
| Kanban board | `huijoohwee/.workspace/.todo/docs/kanban.md` is the derived shared task board over private immutable Context records. | `/kanban.task`, `#kanban-board`, `@kanban-board`. |
...
| Cross-repository planning index | `huijoohwee/.workspace/.todo/docs/TODO.md` owns routing; immutable `huijoohwee/.workspace/.todo/todo/YYYY-MM/<context>.md` records own new task plans, while flat monthly shards preserve legacy history. | `huijoohwee/.workspace/.todo/docs/TODO.md`, `todo-context-record/v3`, `VALIDATION-RUNBOOK.md`. |
| Planning record lifecycle | Each stable Context owns one immutable record; exact Context retrieval precedes deterministic date/Context/source projection, and legacy monthly shards never change. | `huijoohwee/.workspace/.todo/docs/TODO.md` lifecycle, retrieval, and projection contracts. |
| Planning authority | `huijoohwee/.workspace/.todo/docs/TODO.md` and independently claimed Context records are the sole live planning SSOT for participating repositories; shared writable planning indexes and repository-local todo files are forbidden. | The pinned ADLC start/release workflows and `VALIDATION-RUNBOOK.md`. |
| Planning update compliance | A task creates one base-absent Context record under the current owner schema with matching dates; no legacy or extra record path may change. | `huijoohwee/.workspace/.todo/docs/TODO.md` and `planning-context-record-contract.mjs`. |
...
| Planning history | Flat `huijoohwee/.workspace/.todo/todo/YYYY-MM.md` files preserve immutable legacy rows; new `huijoohwee/.workspace/.todo/todo/YYYY-MM/<context>.md` files store immutable task records; `huijoohwee/.workspace/.todo/docs/TODO.md` owns routing and validation. | `huijoohwee/.workspace/.todo/docs/TODO.md` and the pinned ADLC start/release workflows. |
...
| Durable execution | Long-running product agents require checkpoint and resume contracts before runtime-ready promotion. Lifecycle waiting and rechecks belong to the pinned `agentic-os` prompt and never turn silence, expiry, or a resume signal into authority. | `/state.checkpoint`, `#durable-execution`, `@checkpoint-store`. |
...
| Implementation-run observation | The pinned `agentic-os` ADLC harness owns repository lifecycle. ACOS retains only the read-only, non-promoting `agentic-graph.adlc.observe` projection through existing Agentic OS, GraphData, and Canvas owners. Its native `adlc-ledger-receipt/v1` preserves original source schema; bounded historical reads retain exact provenance. Missing evaluators fail closed before projection. It never repairs or grades its source and grants no lifecycle authority. `verified`, `delivery_ready`, and `deployed` remain distinct externally owned claims and are never inferred. | `/adlc.observe`, `#adlc-observability`, `@implementation-run`, `@canvas`, `@runtime-proof`, `IMPLEMENTATION-RUN-OBSERVATION.md`. |
| Application composition | Agentic Canvas OS owns the exact invocation, interface, plan-digest, and owner-separation contract; agentic-graph catalogs and plans exact components, then bounded execution delegates dependency-ready work to existing agent, model, tool, integration, policy, and persistence owners without a new loop, proxy, silent upgrade, retry, migration, or deploy. | `/application.compose`, `#application-composition`, `@application-manifest`, `@component-catalog`, `@integration-profile`, `APPLICATION-COMPOSITION.md`. |
| Role-based Agent Team | `/agent.team #role-based-agent-team @agent-team` composes exact Agent Definition and Agent Orchestration revisions; agentic-graph owns durable MCP plan/start/list/control state, while roles, goals, and personas grant no authority, delegate intermediates stay private, and the agent named by the registered branch's final-answer ownership field owns the final answer. | `AGENT-TEAM.md`, `agentic-graph.agent_team.plan`, `agentic-graph.agent_team.start`, `agentic-graph.agent_team.list`, `agentic-graph.agent_team.control`. |
| Repository packing | `/repository.pack #repository-packing @repository-root @runtime-proof` resolves one exact local Git worktree to `agentic-graph.repository.pack`; agentic-graph publishes a deterministic bounded content-addressed Markdown artifact with typed omissions and zero network, model, token, cost, Prod, or Cloudflare activity. | `REPOSITORY-PACKING.md`, `MCP-GATEWAY.md`, `RUNTIME-PROOF.md`. |
| AI Voice Studio invocation | `/voice.studio` selects exactly one metadata route for `clone`, `dictate`, or `create`; consent, recording rights, permitted use, revocation, disclosure, source provenance, approval, cost, and bounds fail closed before work. agentic-graph alone owns execution through `agentic-graph.voice.studio`; the Voicebox reference is inspiration only and is not copied or required. | `VOICE-STUDIO.md`, `#voice-clone`, `#speech-to-text`, `#text-to-speech`, `@voice-profile`. |
| Agentic ECS | Three exact Dev-only MCP routes hydrate Agentic OS state, advance a private transactional world, and atomically persist only pending `EcsDecision` nodes; Agentic Canvas OS owns invocation truth, agentic-graph owns execution and Canvas projection, and no bitECS artifact or dependency is copied. | `/ecs.session-start`, `/ecs.world-tick`, `/ecs.decision-persist`, `#agentic-ecs`, `@ecs-session`. |
| Game Mode invocation catalog | `/game.mode @canvas #gameplay` resolves one Dev-only browser-local Game Mode inspection or control handoff; agentic-graph WebMCP owns deterministic Agentic ECS gameplay, scored `hold`, `alert`, `engage`, or `flee` decisions, the existing Canvas and XR Mode renderer, Motion Control input reuse, local HUD errors, and validated Decision-only persistence without a model, network dependency, second renderer, Prod mutation, or Cloudflare deployment. | `/game.mode`, `@canvas`, `#gameplay`, `agentic-graph.inspect_local_game_mode`, `agentic-graph.control_local_game_mode`. |
| Camera and XR scene invocation catalog | Camera source selection plus XR stage, asset, transform, physics, and immersive-placement grammar resolve canonically in the dictionaries; browser-local WebMCP tools `agentic-graph.control_local_camera` and `agentic-graph.control_local_xr_scene` remain the execution and persistence owners, while Agentic ECS stays a separate three-tool stdio/Agentic OS composition lane with no renderer or physics ownership. `/media.immersive @canvas #canvas-media` resolves one Dev-only immersive-media handoff with optional `@media-url`; `/flight.sim @canvas #flight` resolves the existing deterministic flight runtime; Motion Control names its executable browser-local tools. agentic-graph retains the existing Canvas, Camera/input, media, ECS, persistence, capture, and runtime-proof owners without a copied artifact, external dependency, second renderer, Prod mutation, or Cloudflare deployment. | `/camera.select`, `/xr.stage`, `/xr.place`, `/xr.transform`, `/xr.label`, `/xr.remove`, `/xr.physics`, `/xr.present`, `/media.immersive`, `/flight.sim`, `/motion.control`, `@scene`, `@canvas`, `@media-url`, `#transform`, `#world`, `#body`, `#impulse`, `#controller`, `#reticle`, `#canvas-media`, `#flight`, `#pose`, `agentic-graph.control_local_camera`, `agentic-graph.control_local_xr_scene`, `agentic-graph.inspect_local_immersive_media`, `agentic-graph.control_local_immersive_media`, `agentic-graph.inspect_local_flight_sim`, `agentic-graph.control_local_flight_sim`, `agentic-graph.inspect_local_motion_control`, `agentic-graph.control_local_motion_control`. |
...
| External pattern boundary | Official OpenAI guides, Podman documentation, LangGraph, DeerFlow, Kimi Agent Swarm, NVIDIA NeMo Agent Toolkit, CrewAI, Hermes Agent memory, SOUL, and MoA patterns, Tool Gateway, Tool Search, Voicebox, and Repomix may inform abstract semantics or platform usage only; local docs and runtime remain provider-neutral and must not copy or depend on external code, APIs, provider code, search implementation, schemas, prompts, examples, tests, fixtures, tables, model lists, vocabulary sets, provider configs, runtime layouts, defaults, algorithms, services, packages, binaries, generated artifacts, or prose. | `AGENT-RUNTIME-COMPOSITION.md`, `AGENT-ORCHESTRATION.md`, `AGENT-TEAM.md`, `AGENT-SWARM.md`, `AGENT-TOOLKIT.md`, `MEMORY.md`, `SOUL.md`, `SANDBOX-AGENTS.md`, `TOOL-SEARCH.md`, `MCP-GATEWAY.md`, `VOICE-STUDIO.md`, `REPOSITORY-PACKING.md`, `VALIDATION-RUNBOOK.md`, `HARNESS-CONTRACTS.md`. |
## Resolution Rules
...
| A dictionary token is missing or a UI label looks invocable | Resolve through dictionaries; add one canonical token to the correct `DICTIONARY-*` file and cross-link it here only when missing. |
...
| VCC | Check |
|---|---|
| Facts and dictionaries resolve | Frontmatter parses as YAML, `direct_resolution` includes command, semantic, and binding truth tokens, and each dictionary contains its matching token. |
| Roles and memory retain their boundaries | `AGENTS.md` points here for truth, while `MEMORY.md` persists context without overriding facts. |
| Exact revisions establish parity | Two runtime identity records pass only when their agentic-graph and Agentic Canvas OS SHAs are valid, exact, and equal across devices; branch names are ignored. |
...
| Catalog revision is current | Every runtime identity reports `catalogRevision == agenticCanvasOsRevision`, a valid exact `catalogDigest`, matching source counts, a bounded hydration attempt count, and a digest-verified non-stale successful state before parity or release is claimed. |
