...
direct_resolution:
  ...
  "/knowgrph.probe-tree": "DICTIONARY-COMMAND.md#/knowgrph.probe-tree"
  ...
  "@knowgrph.probe-tree": "DICTIONARY-BINDING.md#@knowgrph.probe-tree"
...
| `#reference-agents` | `DICTIONARY-SEMANTIC.md` | Semantic route for advisory model or agent calls used before aggregation. | References are no-tool, bounded, private-context producers; every independent call settles as a typed outcome, and one failed reference never aborts successful siblings. |
...
| Failure handling | Reference fan-out is fail-soft: every independent call settles into a typed ledger entry, the aggregator continues with available results, and an all-failed ledger produces a typed fallback without erasing branch failures or fabricating advisory evidence. | `HARNESS-CONTRACTS.md`, `SKILLS.md`. |
...
| Repository packing | `/repository.pack #repository-packing @repository-root @runtime-proof` resolves one exact local Git worktree to `knowgrph.repository.pack`; Knowgrph publishes a deterministic bounded content-addressed Markdown artifact with typed omissions and zero network, model, token, cost, Prod, or Cloudflare activity. | `REPOSITORY-PACKING.md`, `MCP-GATEWAY.md`, `RUNTIME-PROOF.md`. |
| AI Voice Studio invocation | `/voice.studio` selects exactly one metadata route for `clone`, `dictate`, or `create`; consent, recording rights, permitted use, revocation, disclosure, source provenance, approval, cost, and bounds fail closed before work. Knowgrph alone owns execution through `knowgrph.voice.studio`; the Voicebox reference is inspiration only and is not copied or required. | `VOICE-STUDIO.md`, `#voice-clone`, `#speech-to-text`, `#text-to-speech`, `@voice-profile`. |
