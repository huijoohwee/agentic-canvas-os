...
| `/git.run` | `DICTIONARY-COMMAND.md` | Browser Git repository and remote-transport route. | Execute only through browser WebMCP with active persistence; keep remote credentials in the authenticated Dev Worker relay and make local stdio a typed handoff only. |
| `/file.sync` | `DICTIONARY-COMMAND.md` | Bidirectional browser file or directory synchronization route. | Return per-file outcomes through the provider-neutral browser owner; keep credentials and upstream resource identities behind the authenticated Dev Worker relay. |
| `#git-remote` | `DICTIONARY-SEMANTIC.md` | Semantic route for browser Git object/ref and remote operations. | Prove exact hashes, bounds, authority checks, typed conflicts, and Worker-only credentials. |
| `#multi-provider-file-sync` | `DICTIONARY-SEMANTIC.md` | Semantic route for provider-neutral pull and push. | Prove capability, pagination, hash behavior, conflict detection, bounds, offline FIFO, and secret-free browser payloads. |
| `@local-git-repository`, `@git-remote` | `DICTIONARY-BINDING.md` | Browser Git state and opaque remote alias bindings. | Keep local repository state browser-owned and remote transport authenticated, allowlisted, and Worker-owned. |
| `@persisted-cache`, `@file-sync-provider` | `DICTIONARY-BINDING.md` | Browser binary cache and opaque provider alias bindings. | Require active persistence and provider-capability proof without browser credentials or upstream resource ids. |
