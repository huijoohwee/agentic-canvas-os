...
| Managed implementation run | Agentic Canvas OS owns canonical invocation and the fenced worktree/lease/PR lifecycle; Knowgrph owns the durable local MCP supervisor and team projection; the default managed run stops `delivery_ready` when ACOS reports `review_ready`. Only an immutable opt-in terminal implementation completion may authorize exact-head protected auto-delivery; ordinary message, chat, session, or thread endings never do, and no path grants deployment authority. | `/implementation.run`, `#managed-implementation-run`, `@work-item`, `@implementation-run`, `MANAGED-IMPLEMENTATION-RUNS.md`. |
