...
| Multi-worktree writer lease | Same-device and cross-device mutation may run in parallel for distinct semantic scopes. Each registered task worktree has one branch-bound session lease and pull request in the shared Git-common-directory registry; same-worktree, same-branch, and same-scope work serialize by epoch and claim-commit fence. Post-baseline untracked files with that durable attribution remain `owned-untracked` in their physical lane, reject cleanup, and block only their semantic scope; canonical or unattributed dirt fails closed. | `START-WORKFLOW.md`, `scripts/writer-lease-lib.mjs`, `scripts/worktree-lifecycle-lib.mjs`, and focused tests. |
...
| Deployment boundary | Dev, Prod mirror, and Cloudflare are separate. A protected green `main` merge proves Dev integration only. `turn:end` records exact localhost review, and forward Production remains blocked until an authenticated human authorizes the exact build-once candidate digest in the protected Production environment; every local or alternate mutation path remains forbidden. | `CANONICAL-LIFECYCLE.md`, `AGENTS.md`, `RUNTIME-READINESS.md`. |
