...
| Cross-repository planning index | `TODO.md` owns routing; immutable `todo/YYYY-MM/<context>.md` records own new task plans, while flat monthly shards preserve legacy history. | `TODO.md`, `todo-context-record/v2`, `VALIDATION-RUNBOOK.md`. |
| Planning record lifecycle | Each stable Context owns one immutable record; exact Context retrieval precedes deterministic date/Context/source projection, and legacy monthly shards never change. | `TODO.md` lifecycle, retrieval, and projection contracts. |
| Planning authority | `TODO.md` and independently claimed Context records are the sole live planning SSOT for participating repositories; shared writable planning indexes and repository-local todo files are forbidden. | The pinned ADLC start/release workflows and `VALIDATION-RUNBOOK.md`. |
| Planning update compliance | A task creates one base-absent Context record with one canonical 11-cell row, a Directive of at most 50 words, and matching dates; no legacy or extra record path may change. | `TODO.md` and `planning-context-record-contract.mjs`. |
...
| Planning history | Flat `todo/YYYY-MM.md` files preserve immutable legacy rows; new `todo/YYYY-MM/<context>.md` files store immutable task records; `TODO.md` owns routing and validation. | `TODO.md` and the pinned ADLC start/release workflows. |
