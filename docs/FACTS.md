...
| Multi-chat writer lease | Read-only work may run in parallel, but mutation requires one unexpired session lease in the explicitly targeted canonical repository; different devices may write different semantic scopes through distinct draft PRs, while same-scope work serializes by epoch and claim-commit fence. A delivered branch may be revised only by its original session from the exact remote SHA. | `START-WORKFLOW.md`, `scripts/writer-lease-lib.mjs`, and focused tests. |
