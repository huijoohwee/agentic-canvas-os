...
| Sandbox Agents and long-horizon SuperAgent inspiration | External sandbox and SuperAgent harnesses are pattern references only. This repo owns a neutral control plane plus an independently authored Docker adapter, deny-first authorizer, atomic state store, and separate bounded verifier for files, commands, packages, ports, snapshots, and resume. | `SANDBOX-AGENTS.md` plus the DeerFlow repository listed in frontmatter. |
