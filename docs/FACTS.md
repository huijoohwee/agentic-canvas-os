...
| Multi-worktree writer lease | Same-device and cross-device mutation may run in parallel for distinct semantic scopes. Each registered task worktree has one branch-bound unexpired session lease in the shared Git-common-directory registry; same-worktree, same-branch, and same-scope work serialize by epoch and claim-commit fence. A delivered branch may be revised only by its original session from the exact remote SHA. | `START-WORKFLOW.md`, `scripts/writer-lease-lib.mjs`, `scripts/repository-guards.mjs`, and focused tests. |
