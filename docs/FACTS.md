...
| Deterministic codebase knowledge graph | Agentic Canvas OS owns exact invocation and proof semantics; Knowgrph local MCP owns bounded local parsing, atomic digest-fenced graph artifacts, deterministic query, and stored edge explanation, with source evidence for every edge and no model, embedding, vector store, external graph service, or Graphify runtime dependency. Dictionary lookup remains metadata-only. | `/knowledge.graph.ingest`, `/knowledge.graph.query`, `/knowledge.graph.explain`, `#knowledge-graph`, `@knowledge-graph`, `KNOWLEDGE-GRAPH.md`. |
