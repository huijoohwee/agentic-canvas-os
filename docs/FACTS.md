...
source_revision_contract:
  ...
  canonical_rule: "every normal local Dev port uses the clean registered main worktree whose HEAD equals fetched origin/main; an explicit reviewed-runtime command may serve one exact clean reviewHeadSha from its leased task worktree on a reserved noncanonical port, while npm run dev:latest remains the explicit clean-main-only, two-phase, fast-forward-only refresh path"
  ...
  workspace_seed_authority_rule: "knowgrph/docs/workspace-seeds is the only authored workspace-seed source; agentic-canvas-os/docs/workspace-seeds may contain an exact-byte default-storage projection, while publish repositories must contain no editable workspace-seed copy"
layer_contract:
...
| Canonical docs revision | Every Knowgrph Dev runtime, including an explicit reviewed-head server, consumes `$GITHUB_ROOT/agentic-canvas-os/docs` from one clean local checkout whose `HEAD` equals fetched `origin/main`. Port numbers, reviewed app heads, and Knowgrph task mode cannot select or relax that docs revision. | `source_revision_contract` above, the Knowgrph runtime guard, and `scripts/reviewed-runtime.mjs`. |
