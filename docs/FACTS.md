...
source_revision_contract: {canonical_repository: "$GITHUB_ROOT/agentic-canvas-os", canonical_docs_root: "$GITHUB_ROOT/agentic-canvas-os/docs", canonical_ref: "origin/main", consumer_repository: "$GITHUB_ROOT/knowgrph", runtime_guard: "$GITHUB_ROOT/knowgrph/scripts/dev-source-consistency.mjs", canonical_rule: "the Agentic Canvas OS local supervisor may serve fixed local ports only from clean registered main worktrees whose HEAD equals fetched origin/main and whose required protected checks succeeded; task worktrees never own canonical runtime ports", task_rule: "Knowgrph automatically selects task mode on contract-valid agent branches; explicit AG_DEV_SOURCE_MODE values remain expert overrides and task mode never relaxes the Agentic Canvas OS docs revision", parity_rule: "cross-device parity requires identical 40-character Knowgrph and Agentic Canvas OS commit SHAs; matching branch names, ports, or labels are informational and never sufficient", runtime_identity_rule: "every served runtime must visibly expose its Knowgrph revision, Agentic Canvas OS docs revision, catalog revision, deterministic catalog digest, catalog hydration state, and catalog entry counts", runtime_identity_owner_rule: "one application-root runtime owns the canonical Knowgrph identity snapshot globally; route catalogs, panels, and settings views may publish or project facets but must not instantiate competing identity owners", runtime_identity_projection_rule: "MainPanel Settings exposes Cross-device Identity Gate as a normal section inside the Settings body using the shared Key-Type-Value row layout; it must not render above or replace the KTV header", runtime_identity_attestation_rule: "the repository-owned collaboration gate creates two isolated authenticated runtime peers, relays short-lived digest-bound canonical snapshots without owning or persisting identity, and passes only on exact SHA, catalog-digest, catalog-count, active-peer, and common-digest parity; physical devices and manual JSON exports are not required", catalog_freshness_rule: "catalog hydration is keyed by the Agentic Canvas OS docs revision; every sigil response must agree on exact full-catalog counts and one deterministic SHA-256 digest, the browser must recompute that digest after all three source-backed slices replace prior state, and a revision change invalidates the prior catalog with at most two explicit refresh attempts before returning a visible blocked or stale result", production_owner_rule: "Knowgrph is the sole production build, Cloudflare deploy, live-fidelity, rollback, and mirror-publication owner for airvio.co; Agentic Canvas OS has no production deploy workflow", production_attestation_rule: "both public surfaces expose byte-identical agenticgraph-production-runtime-readiness/v2 JSON binding exact app tree, docs, catalog, manifest, artifact, mirror, and surface identities", workspace_seed_authority_rule: "knowgrph/docs/workspace-seeds is the only authored workspace-seed source; agentic-canvas-os/docs/workspace-seeds may contain an exact-byte default-storage projection, while publish repositories must contain no editable workspace-seed copy"}
...
direct_resolution:
  ...
  "/implementation.run": "DICTIONARY-COMMAND.md#/implementation.run"
  ...
  "/agenticgraph.probe-tree": "DICTIONARY-COMMAND.md#/agenticgraph.probe-tree"
  ...
  "#agenticgraph.probe-tree": "DICTIONARY-SEMANTIC.md##agenticgraph.probe-tree"
  ...
  "#managed-implementation-run": "DICTIONARY-SEMANTIC.md##managed-implementation-run"
  ...
  "@agenticgraph.probe-tree": "DICTIONARY-BINDING.md#@agenticgraph.probe-tree"
  ...
  "@work-item": "DICTIONARY-BINDING.md#@work-item"
...
| Prompt preset selection | FloatingPanel Prompt Presets expose catalog-owned selection aliases while executable runtime routes remain owned by `SKILLS.md` and `DICTIONARY-COMMAND.md`; selection loads without submit, LLM responses use the active Chat tuple, and MCP resolves the same runtime token read-only through `agenticgraph.agentic_canvas_os.docs.invoke`. | `PROMPT-PRESETS.md`, `SKILLS.md`, `DICTIONARY-COMMAND.md`. |
...
| Canonical local runtime | The Agentic Canvas OS supervisor serves Knowgrph only when both canonical worktrees are clean exact fetched `origin/main` revisions with required protected checks. Its private token owns fixed Apex `5173` and storage `8787`; unmanaged listeners fail closed. | `source_revision_contract` above, `scripts/local-runtime.mjs`, and `docs/schemas/local-runtime-readiness.v1.schema.json`. |
...
| Repository-owned collaboration gate | `npm run collaboration:gate` in Agentic Canvas OS allocates run-scoped ports, credentials, external persistence, proof, and diagnostics before delegating to Knowgrph's canonical readiness owner; concurrent proofs cannot reuse or stop canonical `5173`/`8787`, and cleanup is allocation-owned. A nonzero result blocks parity, review, integration, and release but does not revoke an otherwise valid unique leased authoring lane. | `scripts/collaboration-gate.mjs`, `scripts/session-start-policy.mjs`, and `VALIDATION-RUNBOOK.md`. |
| Device task branch grammar | `agent/<device>/<semantic-scope>` preserves lowercase hostname device identities with alphanumeric boundaries and interior `.`, `_`, or `-`, including `.local`; semantic scope uses lowercase alphanumerics with interior hyphens only. Generation and parsing share this grammar and reject invalid identity before checkout mutation. | `START-WORKFLOW.md`, `scripts/writer-lease-lib.mjs`, `scripts/device-branch-lib.mjs`, and focused tests. |
| Multi-user, multi-device cloud authority | The provider-neutral mutation roots are exactly `claim(scope)`, `continue(claim)`, `integrate(candidate)`, and `retire(claim)`. Unlimited pairwise-disjoint claims may be current, but an overlapping declared write set has exactly one current writer and any newcomer is a non-writing waiting successor. Expiry derives overlap-preserving `dormant-preserved`; authenticated recovery is independent of an expired local lease. Ordinary active integration revalidates its exact claim through an injected continuation adapter before validation, immediately before committing validated bytes, and before publication, retaining sufficient expiry or renewing monotonically while preserving bytes, claim, branch, session, scope, and admitted write-set digest; expired or drifted authority still fails closed. Immutable review/revision identities, authenticated actors, monotonic CAS, and typed receipts are authoritative; worktrees, leases, branches, pull requests, device/session data, provider metadata, and execution placement are replaceable projections. `workspace:cloud:bootstrap` reports only the current canonical-checkout readiness boundary with `mutationAuthority: false`; it cannot admit, claim, integrate, release, deploy, or clean a lane. A cross-repository task is a dependency-ordered DAG of immutable per-repository units, each with its own repository, branch, worktree, scope, claim, epoch, fence, PR, checks, and handoff. Exceptional local cleanup is a separate exact-authorized operation: it may unregister only one completely clean attached task worktree with no current local writer after an external complete bundle and atomic full-checkout snapshot are preserved; every local/remote branch, cloud claim, and provider object remains unchanged. The pinned dependency is JH guideline/checker revision `8a2e5e0711f7193535b9aac2aee285e0ee705111`, tree `63c13dcfb3ce01aa60213f4f6fa214bfa0e76778`, guideline digest `ff4f0dc41209bdacb05001b6fd5a450883736118f89fcff6fab331cedca8c2bd`, companion digest `c8831f6c6642f89c3e5f51af55523e1e4db1ed08b118840daa0d4f28289806e5` -> ACOS coordination/runtime/registration. None of these states grants protected integration, Production, publication, deployment, or cleanup authority. | `CLOUD-COLLABORATION.md`, `CLOUD-WORKSPACE.md`, `INTEGRATION-ORDER.md`, `WORKSPACE-PARALLELISM.md`, `START-WORKFLOW.md`, `scripts/cloud-workspace.mjs`, `scripts/device-integrate-lib.mjs`, `scripts/recoverable-lane-cleanup-{contract,controller,recovery-store,repository-adapter}.mjs`, and focused tests. |
...
| Deployment boundary | The provider-neutral lifecycle joins Integration, Runtime Review, Candidate, Human Authorization, Live Verification, and Publication receipts. Protected integration proves Dev only; review and candidate preparation create no forward-deployment authority. One authenticated human decision for the exact candidate and target opens one idempotent, target-fenced deployment attempt. Any source, transitive dependency, policy, target, artifact, manifest, receipt, authorization, or controller drift fails closed. GitHub, `main`, `turn:end`, localhost, the Production environment, Cloudflare, and the generated mirror are reference adapters only. | `CANONICAL-LIFECYCLE.md`, `RELEASE-WORKFLOW.md`, `AGENTS.md`, `RUNTIME-READINESS.md`. |
...
| Planning authority | `TODO.md` and independently claimed Context records are the sole live planning SSOT for participating repositories; shared writable planning indexes and repository-local todo files are forbidden. | `START-WORKFLOW.md`, `RELEASE-WORKFLOW.md`, `VALIDATION-RUNBOOK.md`. |
...
| Planning history | Flat `todo/YYYY-MM.md` files preserve immutable legacy rows; new `todo/YYYY-MM/<context>.md` files store immutable task records; `TODO.md` owns routing and validation. | `TODO.md`, `START-WORKFLOW.md`, `RELEASE-WORKFLOW.md`. |
...
| Managed implementation run and observation | Agentic Canvas OS owns canonical invocation, fenced worktree/lease/PR lifecycle, and the pure Agentic SDLC conformance evaluator; Knowgrph owns the durable local MCP supervisor, its `delivery_ready` handoff, and the read-only `agenticgraph.agentic_sdlc.observe` projection through existing KGC, GraphData, and Canvas owners. Observation requires an immutable `agentic-sdlc-ledger-receipt/v1` and never repairs or grades its source. Knowgrph does not yet claim a runtime-ready `agentic-sdlc-run/v1` parity projection because the current supervisor does not persist the full VCC/grant/bound/role packet before dispatch or stable receipt and consumption identities in Implementer output. Only the named independent Evaluator may set a canonical task `verified`; `delivery_ready` remains a review handoff, while `deployed` requires joined release-lifecycle evidence and is never inferred. Exact-head protected auto-delivery remains an immutable opt-in for terminal completion, never an inferred Operator decision, and no execution task or observation grants deployment authority. | `/implementation.run`, `/sdlc.observe`, `#managed-implementation-run`, `#agentic-sdlc-observability`, `@work-item`, `@implementation-run`, `@canvas`, `@runtime-proof`, `MANAGED-IMPLEMENTATION-RUNS.md`, `AGENTIC-SDLC-RUNTIME.md`. |
...
| Role-based Agent Team | `/agent.team #role-based-agent-team @agent-team` composes exact Agent Definition and Agent Orchestration revisions; Knowgrph owns durable MCP plan/start/list/control state, while roles, goals, and personas grant no authority, delegate intermediates stay private, and the agent named by the registered branch's final-answer ownership field owns the final answer. | `AGENT-TEAM.md`, `agenticgraph.agent_team.plan`, `agenticgraph.agent_team.start`, `agenticgraph.agent_team.list`, `agenticgraph.agent_team.control`. |
| Repository packing | `/repository.pack #repository-packing @repository-root @runtime-proof` resolves one exact local Git worktree to `agenticgraph.repository.pack`; Knowgrph publishes a deterministic bounded content-addressed Markdown artifact with typed omissions and zero network, model, token, cost, Prod, or Cloudflare activity. | `REPOSITORY-PACKING.md`, `MCP-GATEWAY.md`, `RUNTIME-PROOF.md`. |
| AI Voice Studio invocation | `/voice.studio` selects exactly one metadata route for `clone`, `dictate`, or `create`; consent, recording rights, permitted use, revocation, disclosure, source provenance, approval, cost, and bounds fail closed before work. Knowgrph alone owns execution through `agenticgraph.voice.studio`; the Voicebox reference is inspiration only and is not copied or required. | `VOICE-STUDIO.md`, `#voice-clone`, `#speech-to-text`, `#text-to-speech`, `@voice-profile`. |
