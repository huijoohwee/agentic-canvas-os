...
| `/knowledge.graph.ingest` | `DICTIONARY-COMMAND.md` | Route from one explicit bounded workspace to `knowgrph.knowledge_graph.ingest`. | Build one deterministic digest-bound local snapshot from supported code, docs, SQL, configs, and text-bearing PDFs; preserve source files and return typed omissions without model, network, vector, or Graphify fallback. |
| `/knowledge.graph.query` | `DICTIONARY-COMMAND.md` | Route from one exact graph snapshot to `knowgrph.knowledge_graph.query`. | Return a bounded deterministically ordered subgraph with explanations and evidence or a typed empty/unsupported result; reject stale snapshot identity. |
| `/knowledge.graph.explain` | `DICTIONARY-COMMAND.md` | Route from one exact edge identity to `knowgrph.knowledge_graph.explain_edge`. | Return stored endpoints, edge label, evidence kind, explanation, parser/source span and excerpt, confidence, premises, and candidate count without reparsing, generation, or mutation. |
| `#knowledge-graph` | `DICTIONARY-SEMANTIC.md` | Semantic route for local deterministic source-backed knowledge graphs. | Require stable graph identity, parser/source evidence, explained edges, deterministic operations, and absence of models, embeddings, vector stores, external graph services, and Graphify runtime dependency. |
| `@knowledge-graph` | `DICTIONARY-BINDING.md` | Binding for one exact digest-fenced Knowgrph local graph snapshot view. | Snapshot identity is read-only context for query and explanation, carries no approval or credential, and becomes stale after an atomic artifact replacement. |
