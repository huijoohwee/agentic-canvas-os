...
| Multi-user, multi-device writer lease | Parallel actors, devices, sessions, and registered task worktrees are permitted only for disjoint semantic and write scopes. The protected `agentic/collaboration-ledger` ref is cross-device authority through append-only compare-and-swap transitions; local lease files and pull-request markers are projections only. Authenticated actor and repository ids, device, session, worktree digest, branch, scope, write-set digest, monotonic epoch, claim digest, and ledger revision join at integration. `review-ready` closes authoring; a separate idempotent `delivery-authorized` receipt binds the unchanged reviewed head and explicit protected-integration intent without granting deployment authority. Same-worktree, same-branch, same-scope, and overlapping-path work serialize. Offline authoring may remain local, but shared push, review, integration, and runtime readiness require live cloud verification. | `CLOUD-COLLABORATION.md`, `START-WORKFLOW.md`, `scripts/cloud-collaboration-contract.mjs`, `scripts/github-cloud-collaboration-adapter.mjs`, and focused tests. |
