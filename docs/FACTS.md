...
| ADLC lane grammar | `agent/<device>/<semantic-scope>` is the single task-branch identity. `agentic-os` validates it before creating a registered worktree from fetched `origin/main`; ACOS compatibility shims do not create a second identity or authority system. | `CANONICAL-LIFECYCLE.md`, `START-WORKFLOW.md`, and the pinned `agentic-os` package. |
| Multi-user, multi-device lifecycle authority | The remotely addressable ADLC branch and pull request are the shared claim. Provider ordering, exact-head checks, integration proof, and exact clean retirement are distinct receipts. Dirty, untracked, ambiguous, or concurrently owned bytes are preserved; no local lease, marker, queue state, or compatibility projection grants protected integration, Production, deployment, or cleanup authority. | `CANONICAL-LIFECYCLE.md`, `RELEASE-WORKFLOW.md`, `WORKSPACE-PARALLELISM.md`, and `__tests__/adlc-compatibility.test.mjs`. |
