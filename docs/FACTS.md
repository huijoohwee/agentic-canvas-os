...
| Immersive media and Flight Sim invocation catalog | `/media.immersive @canvas #canvas-media` resolves one Dev-only browser-local immersive-media inspection or control handoff with optional `@media-url`; `/flight.sim @canvas #flight` resolves the existing deterministic flight runtime; Motion Control truth now names its executable browser-local tools. Knowgrph retains the existing Canvas, Camera/input, media, ECS, persistence, capture, and runtime-proof owners without a copied artifact, external dependency, second renderer, Prod mutation, or Cloudflare deployment. | `/media.immersive`, `/flight.sim`, `/motion.control`, `@canvas`, `@media-url`, `#canvas-media`, `#flight`, `#pose`, `knowgrph.inspect_local_immersive_media`, `knowgrph.control_local_immersive_media`, `knowgrph.inspect_local_flight_sim`, `knowgrph.control_local_flight_sim`, `knowgrph.inspect_local_motion_control`, `knowgrph.control_local_motion_control`. |
