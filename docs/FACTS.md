...
direct_resolution:
  "/sdlc.observe": "DICTIONARY-COMMAND.md#/sdlc.observe"
  ...
  "#agentic-sdlc-observability": "DICTIONARY-SEMANTIC.md##agentic-sdlc-observability"
...
| Implementation-run observation | The pinned `agentic-os` ADLC harness owns repository lifecycle. ACOS retains only the read-only, non-promoting `agentic-graph.agentic_sdlc.observe` compatibility projection through existing KGC, GraphData, and Canvas owners. It requires an immutable `agentic-sdlc-ledger-receipt/v1`, never repairs or grades its source, and grants no lifecycle authority. `verified`, `delivery_ready`, and `deployed` remain distinct externally owned claims and are never inferred. | `/sdlc.observe`, `#agentic-sdlc-observability`, `@implementation-run`, `@canvas`, `@runtime-proof`, `IMPLEMENTATION-RUN-OBSERVATION.md`. |
