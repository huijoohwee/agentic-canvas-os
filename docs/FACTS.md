...
| Managed implementation run | Agentic Canvas OS owns canonical invocation, fenced worktree/lease/PR lifecycle, and the pure Agentic SDLC conformance evaluator; Knowgrph owns the durable local MCP supervisor and its `delivery_ready` handoff. Knowgrph does not yet claim a runtime-ready `agentic-sdlc-run/v1` projection because the current supervisor does not persist the full VCC/grant/bound/role packet before dispatch or stable receipt and consumption identities in Implementer output. Only the named independent Evaluator may set a canonical task `verified`. Exact-head protected auto-delivery remains an immutable opt-in for terminal completion, never an inferred Operator decision, and no execution task grants deployment authority. | `/implementation.run`, `#managed-implementation-run`, `@work-item`, `@implementation-run`, `MANAGED-IMPLEMENTATION-RUNS.md`, `AGENTIC-SDLC-RUNTIME.md`. |
