...
| External pattern boundary | Official OpenAI guides, Docker documentation, LangGraph, DeerFlow, Kimi Agent Swarm, NVIDIA NeMo Agent Toolkit, CrewAI, Hermes Agent memory, SOUL, and MoA patterns, Tool Gateway, Tool Search, Voicebox, and Repomix may inform abstract semantics or platform usage only; local docs and runtime remain provider-neutral and must not copy or depend on external code, APIs, schemas, tests, examples, prompts, vocabulary sets, provider configs, runtime layouts, defaults, algorithms, services, packages, binaries, or prose. | `AGENT-RUNTIME-COMPOSITION.md`, `AGENT-ORCHESTRATION.md`, `AGENT-TEAM.md`, `AGENT-SWARM.md`, `AGENT-TOOLKIT.md`, `MEMORY.md`, `SOUL.md`, `SANDBOX-AGENTS.md`, `TOOL-SEARCH.md`, `VOICE-STUDIO.md`, `REPOSITORY-PACKING.md`, `VALIDATION-RUNBOOK.md`, `HARNESS-CONTRACTS.md`. |
## Resolution Rules
