...
| Multi-user, multi-device writer lease | Parallel actors, devices, sessions, and registered task worktrees are permitted only for disjoint semantic and write scopes. A device-local Git-common-directory lease registry owns worktrees and branches; the shared remote pull-request set owns cross-user and cross-device scope exclusion. Authenticated actor, device, session, worktree, branch, scope, monotonic epoch, and claim-commit fence identities join at integration. Same-worktree, same-branch, same-scope, and overlapping-path work serialize. Post-baseline authored state remains in its attributed lane; canonical or unattributed dirt fails closed. | `START-WORKFLOW.md`, `CANONICAL-LIFECYCLE.md`, `scripts/writer-lease-lib.mjs`, `scripts/worktree-lifecycle-lib.mjs`, and focused tests. |
