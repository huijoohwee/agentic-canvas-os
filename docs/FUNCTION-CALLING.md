...
runtime_claim: "direct calls, durable reviewed continuation, and one native deduplicated run-note mutation are runtime-ready in Dev with one bounded reviewed provider proof; Prod remains gated"
runtime_owner: "../agent-api/src/function-calling.js; ../agent-api/src/function-calling-manager.js; ../agent-api/src/function-execution-receipts.js; ../agent-api/src/openai-responses-function-adapter.js; ../agent-api/src/knowgrph-function-gateway.js; ../agent-api/src/knowgrph-function-tools.js; ../scripts/live-function-run-note-proof.mjs"
runtime_proof: "../__tests__/function-calling.test.mjs; ../__tests__/function-calling-continuation.test.mjs; ../__tests__/function-execution-receipts.test.mjs; ../__tests__/openai-function-gateway.test.mjs; ../__tests__/live-function-run-note-proof.test.mjs; ../scripts/function-gateway-run-note-proof.mjs; ../scripts/live-function-run-note-proof.mjs"
...
- Given configured offline transports, when OpenAI returns reasoning plus a strict function call, then the application calls only `agenticgraph.os.status`, replays the opaque reasoning and same-id output with the prior response id, and returns final text plus actual model usage and zero-cost gateway evidence.
