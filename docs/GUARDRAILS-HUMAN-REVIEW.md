...
- Given a Function Calling review pause, when a fresh manager claims the exact per-run checkpoint, then the gateway resolves the stored review before one idempotent call and the provider continues from the stored response without replay.
