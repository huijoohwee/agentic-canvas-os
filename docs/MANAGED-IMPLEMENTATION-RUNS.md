...
An expired cloud-admitted `active` lane that still contains owned modified or untracked bytes is not a resume or handoff subject. Its exact recorded source session may run `active-owned-dirt-recovery.mjs plan`, then `execute` with the returned typed plan authorization. The controller admits only the unchanged attached branch, draft PR, remote fence, admission manifest, dormant same claim, and in-scope dirt. It first pins a content-addressed Git snapshot containing exact HEAD/index/worktree path, type, mode, blob, staged, unstaged, untracked, deletion, and symlink evidence; then it recovers the same cloud claim, atomically advances the local epoch for the same session/device, and replaces only the hidden PR lease marker. A durable registry intent makes interruption replay single-effect and blocks heartbeat from crossing the transition. The command does not touch authored bytes, the real index, HEAD, branch/remote refs, PR draft state, or delivery controls, and it never transfers dirt. The managed run resumes authoring only from the returned mutation-authority receipt, then must become clean and use ordinary `device:review` to reach `review_ready`.
