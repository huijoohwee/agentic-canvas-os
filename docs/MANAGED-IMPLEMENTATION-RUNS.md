...
For runtime-relevant `delivery_ready` handoff, `device:serve-reviewed` starts only the exact clean `reviewHeadSha` on reserved loopback port `5176`, and `device:runtime-status` re-proves worktree, remote, PR, listener, HTTP, and canonical docs identity. `device:runtime-stop` stops only its recorded process group after the recorded listener PID still owns that port. Canonical port `5173` requires an explicit flag, occupied ports always fail, and no command merges, deploys, accepts arbitrary argv, or kills an unrelated server. Non-runtime, active, parked, blocked, failed, canceled, and read-only turns do not manufacture reviewed-server proof.
