...
advanced from its base projection at transition `t1` to the same claim's fence
projection at transition `t2`, while the local writer lease retained the `t1`
...
same-claim `t1` to `t2` projection. A later transition, foreign claim, rewritten
...
registry. The target lease adopts the already-authoritative `t2` fence while
...
- Exact `t1` source: project the `t2` fence once.
- Exact `t2` target plus the operation receipt: adopt the result without a
  second write.
