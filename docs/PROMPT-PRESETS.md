...
| Knowgrph Probe-Tree | `/knowgrph-probe-tree-prompt-preset` | `/knowgrph.probe-tree` | Load the source-backed prompt in Chat or insert its slash invocation into the selected Widget Card. The `@` and `#` aliases remain independently authorable. | Use the active Chat provider, endpoint, and model to materialize 2-4 Type 2 child cards with distinct selected-child decision variables, semantic clarification choices, 2-6 verbatim child-or-lineage anchors, and Other; keep the selected child as continuation owner, preserve every user-authored output edge, reject inferred output edges and zero-model fallback cards, and stop at depth 8. |
