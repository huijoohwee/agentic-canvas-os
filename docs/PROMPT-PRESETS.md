...
| Knowgrph Probe-Tree | `/knowgrph-probe-tree-prompt-preset` | `/knowgrph.probe-tree` | Load the source-backed prompt in Chat or insert its slash invocation into the selected Widget Card. The `@` and `#` aliases remain independently authorable. | Materialize 2-4 Type 2 child cards with distinct selected-child focuses, context-relevant scope, priority, and constraint suggestions, 2-6 verbatim child-or-lineage anchors, Other, and a separate Rich Media branch ledger through the native zero-cost path; keep the selected child as continuation owner and stop at depth 8. |
