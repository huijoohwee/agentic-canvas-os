...
| Runtime revision identity contract | Facts, startup, validation, and release contracts require one application-root canonical identity owner, a MainPanel Settings KTV projection, automatic authenticated challenge-bound attestation, visible exact Knowgrph, Agentic Canvas OS, and catalog revisions, one common verification digest, and revision-keyed catalog hydration limited to two explicit refresh attempts. | A claim of cross-device parity before at least two distinct live devices pass the automatic gate; `Copy diagnostic JSON` remains optional troubleshooting only. |
...
| Runtime revision identity architecture holds | Knowgrph `50dc3d5c154ce599f413e8d00c5786a49d8a009f` mounts one application-root identity runtime, copies point-in-time canonical snapshots into authenticated 60-second room challenges, rejects replay, duplicate authenticated sessions/devices/runtimes, socket identity mutation, and identity-room persistence, renders twelve shared KTV rows inside MainPanel Settings, and exposes the canonical identity plus automatic gate through read-only WebMCP. Five attestation/room tests, Settings and WebMCP tests, TypeScript, targeted lint, hygiene, Worker compile, and fail-closed browser proof passed. A live common digest from two separately authenticated devices remains required before parity or release. | Automatic architecture passed; live two-device parity gated |
