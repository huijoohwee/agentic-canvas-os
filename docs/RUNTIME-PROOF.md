...
| Todo planning-ledger compliance | Startup pins the exact fetched `todo-log.md`; release requires one changed row for the declared Context with 11 filled cells, a directive of at most 50 words, matching section/date, and byte-identical non-target baseline rows. | Missing, unchanged, duplicate, empty, overlong, misdated, or destructive planning updates. |
...
| Todo-log compliance holds | The startup command pins the fetched baseline; the release command validates the current task row and rejects unchanged rows or deleted historical Contexts. | Passed |
