...
| Probe-Tree clarification contract | The source-backed preset declares RECOMMEND, COMPARE, ASSESS, and PLAN as semantic, case-insensitive clarification topics producing 2-4 cards through the active Chat tuple; dictionaries preserve selected-child ownership and the runtime-recognized terminal boundary. | Query-specific hardcoding, action-verb terminal inference, stale card-local routing, zero-model fallback cards, or live provider execution. |
...
| Probe-Tree clarification contract holds | `npm run probe-tree-contract:check` validates the structured action-topic set, 2-4 bounds, semantic classification, runtime-recognized selected-child terminal boundary, active Chat route, and fail-closed no-hardcode/no-zero-model policy across the preset and all three dictionaries. | Passed without provider execution |
