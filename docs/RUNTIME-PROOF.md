...
| Function calling is executable | `npm run function-gateway:check` reports 18/18 passing: twelve controller checks plus six OpenAI and Knowgrph gateway checks for strict translation, one-shot forced selection, reasoning replay, prior-response and call identity, usage, allowlisting, automatic tool guardrails, caller-approval rejection, exact signed review, provider-error redaction, and no secret return. `read_agentic_os_status` remains the sole mapping to `knowgrph.os.status`. | Runtime-ready in Dev; reviewed status path proven offline and live provider proof gated |
