...
| Function calling is executable | The focused gateway suite covers the controller, durable continuation, execution-receipt, MCP transport, OpenAI adapter, guarded gateway, Dev deployment, and sanitized live-proof composition owners. `npm run function-gateway:cross-repo-proof -- --knowgrph-root=<checkout>` additionally runs the real Agentic receipt owner against Knowgrph's native run-note Durable Object: a dropped first receipt blocks retryably, the second call returns `replayed`, the manifest remains revision 1, and a terminal Agentic replay makes no third MCP call. `read_agentic_os_status` stays read-only; `update_agent_run_note` is intrinsically review-required and maps only to `knowgrph.run_manifest.note.update`. The Dev live runner is fail-closed until exact Worker secrets and approval are present; source readiness alone is not a live-provider claim. | Runtime-ready offline in Dev for one native mutation; bounded Dev deployment and provider proof are separately evidenced when executed; Prod remains gated |
