...
| Running agents runtime | Agent-loop transitions, exclusive continuation strategies, same-loop streaming, pause and resume, serialization, bounds, cost evidence, and fail-closed readiness are executable locally; one approved proof also covers previous-response continuation, final settlement, and handoff usage. | Copied provider agent code, SDK schemas, prompts, examples, event fixtures, tests, prose, fabricated adapter evidence, default provider wiring, or unbounded live execution. |
| Guardrails and human review runtime | Ordered input, output, tool-input, and tool-output checks plus same-turn approve, reject, or edit interruption state are executable locally with single consumption, expiry, capacity, edit revalidation, and sanitized readiness. | Copied SDK code, schemas, examples, prompts, tests, fixtures, inferred approval, duplicate side effects, fabricated durable recovery, live provider execution, or deployment. |
...
| Running agents is executable | `npm run running-agents:check` reports 7/7 passing for model/tool/handoff progression, application-history replay, session/conversation/previous-response exclusivity, incremental same-loop streaming, same-turn pause resume, active serialization, recent-run fencing, step and event bounds, timeout, unconfigured behavior, and honest cost. App and Worker tests keep adapter configuration false. | Runtime-ready in Dev; continuation, handoff, settlement, and usage passed bounded live proof |
| Guardrails and human review are executable | `npm run guardrails-human-review:check` reports 5/5 passing for ordered input/output transformation, tool-input policy rejection, fail-closed missing evaluators, inspectable bounded interruptions, opaque resume state, same-turn approval resume, approve/reject/edit, edit revalidation, single consumption, expiry, and capacity. Agent Definition and Runtime Composition checks prove four-stage references plus automatic pre-adapter and pre-return execution; app and Worker tests expose sanitized unconfigured evaluator state. | Runtime-ready in Dev for same-process review; durable recovery and live provider/tool approval remain gated |
...
| Agent runtime composition is executable | `npm run agent-runtime-composition:check` reports 10/10 passing for real source hashing, exact source preparation, provider, model, transport, feature, and definition selection, one Running Agents lifecycle, previous-response continuation, output rejection and reset, drift blocks, explicit ownership, hidden intermediate output, and honest cost. App and Worker tests keep the default adapter unconfigured. | Runtime-ready in Dev; one explicitly approved composed live proof passed |
