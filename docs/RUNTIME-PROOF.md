...
| Function calling is executable | `npm run function-calling:check` reports 11/11 passing for strict nested schemas, direct gateway metadata, auto/required/none/forced/allowed selection, exact call-id outputs, active reasoning replay, bounded parallel calls, typed approval blocks, validators, result and turn limits, duplicate serialization, timeout, and honest model and gateway costs. App and Worker readiness tests expose only sanitized policy while provider execution remains `unverified`. | Runtime-ready in Dev; live adapter and gateway proof remain gated |
