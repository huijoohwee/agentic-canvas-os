...
| Function calling is executable | `npm run function-gateway:check` reports 21/21 passing: twelve controller checks, three durable continuation and HTTP-boundary checks, and six OpenAI and Knowgrph gateway checks. Proof covers strict translation, one-shot forced selection, reasoning replay, exact prior-response and call identity across a fresh manager, atomic Durable Object claims, invalid-review retry without execution, definition-drift fencing, usage, allowlisting, automatic tool guardrails, raw-approval rejection, exact signed review, provider-error redaction, and no secret return. `read_agentic_os_status` remains the sole mapping to `knowgrph.os.status`. | Runtime-ready in Dev; durable reviewed status continuation proven offline and live provider proof gated |
