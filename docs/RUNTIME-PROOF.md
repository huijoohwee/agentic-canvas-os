...
| Writer lease and fencing contract | For the explicit canonical target repository, `device:start` claims one atomic Git-metadata lease, increments its epoch, records a claim-commit fence, and opens a draft ownership pull request before authoring; `device:heartbeat` renews the bounded TTL. | Same-device competing sessions, duplicate semantic-scope pull requests, expired leases, session mismatch, ambiguous target repositories, and non-ancestral fences fail closed. |
...
| Multi-chat writer coordination is executable | Focused Node tests exercise atomic local serialization, TTL renewal, expired-session epoch takeover, session fencing, pull-request metadata round-trip, different-scope PR coexistence, and duplicate-scope rejection. | Dev branch proof passed: 140 tests, web build, and 26-file docs contract; protected integration pending |
