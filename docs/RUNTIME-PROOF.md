...
| Function calling is executable | `npm run function-gateway:check` reports 34/34 passing across the controller, durable continuation, execution-receipt, MCP transport, OpenAI adapter, and guarded gateway owners. `npm run function-gateway:cross-repo-proof -- --knowgrph-root=<checkout>` additionally runs the real Agentic receipt owner against Knowgrph's native run-note Durable Object: a dropped first receipt blocks retryably, the second call returns `replayed`, the manifest remains revision 1, and a terminal Agentic replay makes no third MCP call. `read_agentic_os_status` stays read-only; `update_agent_run_note` is intrinsically review-required and maps only to `knowgrph.run_manifest.note.update`. | Runtime-ready offline in Dev for one native mutation; live provider, remote Worker, Prod, and Cloudflare remain gated |
