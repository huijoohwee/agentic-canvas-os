...
| Function calling is executable | `npm run function-gateway:check` reports 33/33 passing across the controller, durable continuation, execution-receipt, MCP transport, OpenAI adapter, and guarded gateway owners. Proof covers strict translation, exact prior-response and call identity across a fresh manager, atomic Durable Object claims, invalid-review retry, definition-drift fencing, pre-side-effect receipt reservation, one execution claimant, stable-key retry after an uncertain mutation result, required native receipt echo, fresh-isolate terminal replay, usage, allowlisting, guardrails, raw-approval rejection, signed review, redaction, and no secret return. `read_agentic_os_status` remains the sole enabled mapping to `knowgrph.os.status`; mutating mappings remain gated. | Runtime-ready in Dev; durable reviewed execution fencing proven offline and live provider or mutating-tool proof gated |
...
| Function calling Dev runtime | Runtime-ready in Dev | The local controller is strict, typed, bounded, selection-aware, call-id-preserving, reasoning-continuous, cost-aware, gateway-owned, receipt-fenced, fail-closed, and no-copy; a live model call and a native mutating Knowgrph receipt remain separately gated. |
