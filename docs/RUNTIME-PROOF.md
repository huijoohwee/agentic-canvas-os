...
| Multi-chat writer coordination is executable | Focused Node tests exercise atomic local serialization, TTL renewal, expired-session epoch takeover, same-session delivery revision, session fencing, pull-request metadata round-trip, different-scope PR coexistence, duplicate-scope rejection, and redacted command output. | Runtime-ready in Dev; protected CI is required for every revision |
