...
| Sandbox Agents runtime | Container-provider capability and revision fencing, application authorization, safe workspace inputs, serialized operations, opaque snapshot seeding, cross-controller resume, output redaction, bounds, cost, and honest containment evidence are executable locally. | Copied provider SDK or client code, APIs, examples, prompts, schemas, fixtures, tests, model defaults, fabricated containment, host process fallback, public ports, or live provider execution. |
...
| Agent definitions are executable | `npm run agent-definitions:check` reports 7/7 passing for immutable minimal packets, authorized optional behavior, reference-only capability denial, handoff target verification, revision conflict and stale fencing, unknown-field and capacity rejection, explicit bounds, and text or structured output validation. App and Worker tests expose sanitized registry policy while no definitions are registered and provider execution remains `unverified`. | Runtime-ready in Dev; downstream agent execution and live provider proof remain gated |
...
| Sandbox Agents is executable | `npm run sandbox-agents:check` reports 7/7 passing for fresh workspace creation, safe files, argument-vector commands, packages, private or preview ports, exact capabilities, application approvals, provider attestations, per-sandbox serialization, opaque snapshot seeding, external resume state across controller instances, secret rejection, provider-error redaction, costs, and cleanup. App and Worker tests expose sanitized unconfigured state while independent containment remains `unverified`. | Runtime-ready in Dev; real container provider and independent containment proof remain gated |
...
| Sandbox Agents Dev runtime | Runtime-ready in Dev | The local controller is typed, bounded, provider-neutral, capability-fenced, approval-gated, serialized, secret-safe, snapshot- and resume-aware, cost-honest, fail-closed, and no-copy; a real container provider, external state store deployment, and independent containment proof remain separately gated. |
