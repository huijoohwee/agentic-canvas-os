---
title: "agentic-graph Agentic Canvas OS Runtime Readiness"
graphId: "md:agentic-graph-agentic-canvas-os-runtime-readiness"
...
external_runtime_policy: "agentic-graph runtime, Prod mirror, and Cloudflare remain gated until explicit proof or approval"
...
Runtime-ready means the claim can be proven from surfaced output. This file marks the Agentic Canvas OS docs control surface runtime-ready. It does not claim that every external `agentic-graph`, Prod mirror, or Cloudflare runtime is already executed.
...
| Implementation-run observation | One immutable implementation-run receipt has a specified deterministic projection through existing Agentic OS, GraphData, and Canvas owners without grading or promotion. | `IMPLEMENTATION-RUN-OBSERVATION.md` and its focused source-contract test. | Spec-complete compatibility contract only; lifecycle remains owned by pinned ADLC, while live source authenticity, tool execution, and rendering remain unproven |
| Cache context runtime | Stable prefixes precede dynamic tails; local reuse cannot establish provider hits. | Pinned `agentic-os/context/prefix`, app/Worker injection, `CACHE-CONTEXT.md`, OS behavior tests and Canvas integration tests. | Local source evidence; live provider hit gated |
| Reasoning continuity runtime | Stable invariants permit compatible continuation; drift resets requested reasoning context. | Pinned `agentic-os/context/continuity`, app/Worker injection, `REASONING-CONTINUITY.md`, OS behavior tests and Canvas integration tests. | Local source evidence; live provider confirmation gated |
| Function calling runtime | Models may request strict application functions while exact call identity, guarded gateway execution, signed review, reasoning continuity, durable continuation, execution receipts, and separate costs remain application-owned. | `agent-api/src/{function-calling.js,function-calling-manager.js,function-calling-continuation.js,function-execution-receipts.js,openai-responses-function-adapter.js,agentic-graph-function-gateway.js,agentic-graph-function-tools.js,function-calling-handler.js,durable-object-state-store.js}`, the route-free `env.dev` Worker wiring, `scripts/live-function-run-note-proof.mjs`, `FUNCTION-CALLING.md`, `LIVE-REVIEWED-FUNCTION-PROOF.md`, focused tests, the explicit cross-repository proof, and one accepted provider run establish immutable review, native atomic receipt echo, one-revision recovery, terminal replay, strict schemas, redaction, returned usage, and persisted read-back. | Verified bounded live in Dev; 1 logical run, 2 provider requests, 1 reviewed call, 546 input tokens, 55 output tokens, USD 0.000876; Prod remains gated |
| Agent definitions runtime | Each specialist binds an application-verified source URI and digest to its exact model route, ordered instructions, and optional reference-only behavior without acquiring execution authority. | `agent-api/src/agent-definitions.js`, isolate-scoped Worker injection, `AGENT-DEFINITIONS.md`, sanitized readiness, and eight focused offline tests prove exact source verification, immutable registration, revision fencing, capability authorization, handoff verification, bounds, and output validation. | Runtime-ready in Dev; the default registry is empty unless every autonomous runtime gate is explicitly configured |
| Private commerce admission runtime | The private `agentic-os-admission.internal` provider accepts a fenced active registration only after caller HMAC and a signed, expiring local configuration envelope bound to exact intent, inputs, and permit; its Durable Object persists a non-secret projection for replay and rehydration. | `commerce-admission-{authority,contract,provider}.js`, `worker/agent-state.js`, the existing `AGENT_STATE` binding, and `npm run commerce-admission-provider:check` prove malformed, wrong-owner, tampered, expired, unconfigured, and intent-mismatched configurations make zero admission writes. | Source-ready in Dev; the legacy agentic-commerce-os consumer has not migrated or accepted this contract, and authenticated agentic-graph issuance, Graph service binding acceptance, production deployment, live verification, rollback, and Cloudflare mutation remain gated |
...
| Agent Toolkit runtime | Application-authorized caller-declared target, candidate, adapter, evaluator, dataset, and metric revision digests can be observed, profiled, evaluated, compared as unique trusted evidence, and ranked across exact candidates without moving execution ownership. | `agent-api/src/agent-toolkit*.js`, `AGENT_STATE` atomic admission/claims/expiry alarms, authenticated Worker profile and optimize routes, structured metadata telemetry, `AGENT-TOOLKIT.md`, sanitized readiness, and the combined Toolkit/runtime/app/Worker/durable-state suite prove server timing, payload redaction, request/run/cohort quotas, percentile and bottleneck profiling, lifecycle fencing, stable evaluator idempotency, one-owner cost, principal-key isolation, remote-evidence exclusion, deterministic quality/latency/cost recommendations, physical durable cleanup, and no apply method. | Runtime-ready production contract; `productionReady` remains false until durable state, telemetry, an application authorizer, and an evaluator are configured. Refreshed session principals cannot resume earlier cohorts without a stable application principal; measured improvement, Prod release, and Cloudflare deployment remain gated |
| Agent runtime composition | One source-backed definition passes through exact model and transport selection, one Running Agents lifecycle, final-output validation, and the resolver/runner interface used by orchestration. | `agent-api/src/agent-runtime-composition.js`, a repository-backed definition fixture, `AGENT-RUNTIME-COMPOSITION.md`, app and Worker wiring, sanitized readiness, and twelve focused offline tests prove real source hashing, revision and identity fencing, feature matching, continuation, delegation, handoff, output rejection, hidden intermediates, and honest cost. | Runtime-ready in Dev with one bounded live composed proof; shipped default remains unconfigured and the explicit autonomous route is opt-in |
| Opt-in autonomous runtime | An operator may connect one exact server-side source definition to the OpenAI Responses adapter and authenticated composed-agent Worker route without enabling it by default. | `agent-api/src/{autonomous-runtime-config.js,agent-runtime-handler.js,app.js}`, `worker/index.js`, `AUTONOMOUS-RUNTIME.md`, sanitized readiness, and focused offline tests prove all-gate admission, source re-hashing, aligned model ownership, spend refusal, principal-scoped continuation, strict HTTP fields, bounds, and zero provider requests while unconfigured. | Runtime-ready offline in Dev; disabled by default, provider behavior unverified, no tools or deployment authorized |
| Application composition contract | Exact host invocation aliases compile immutable version- and schema-fenced component interfaces into a deterministic dependency plan; optional execution sequences ready work through existing owners only. | `APPLICATION-COMPOSITION.md`, `FACTS.md`, all three dictionaries, `HARNESS-CONTRACTS.md`, `MCP-GATEWAY.md`, and focused tests prove the exact alias tuple, three MCP tools, connection-negotiated capabilities, opaque integration profiles, owner boundaries, explicit migration diagnostics, and no silent upgrade, retry, fallback, or deploy. | Runtime-ready for ACOS contract; agentic-graph local MCP fixture and protected integration proof remain required for combined execution readiness |
| Deterministic agentic graph invocation | One exact `/`, `#`, and `@` contract forwards typed ingest, query, and edge-explanation requests to the agentic-graph executable owner without exposing artifact paths or duplicating its parser and store. | `AGENTIC-GRAPH.md`, facts, all three dictionaries, `MCP-GATEWAY.md`, `src/agentic-graph-mcp-client.js`, and `npm run agentic-graph-contract:check` prove exact tool names, opaque graph identity, snapshot-digest fencing, ingest projection fields, and duplicate-runtime removal. | Contract/client-ready in Dev; exact protected agentic-graph runtime and Canvas proof remain required for combined runtime readiness |
| Repository packing contract | One exact local Git worktree becomes one deterministic bounded content-addressed Markdown artifact through `/repository.pack #repository-packing @repository-root @runtime-proof` and `agentic-graph.repository.pack`. | `REPOSITORY-PACKING.md`, facts, dictionaries, skills, harness, MCP gateway, focused contract and clean-room checks, plus exact agentic-graph contract/runtime/stdio/independence tests prove containment, ignores, ordering, hashes, omissions, secrets, bounds, atomic replay, unchanged source, and zero network/model/token/cost/deploy. | Runtime-ready for ACOS contract; combined readiness requires the exact integrated agentic-graph revision and docs pin |
| Repository runtime-readiness source audit | One exact local Git worktree is inspected through the existing `/runtime-ready.check #runtime-ready #harness #vcc @repository-root @local-harness @runtime-proof` route without executing target code or reading secret values. | `REPOSITORY-RUNTIME-READINESS.md`, the evaluator, focused fixtures, dictionaries, skills, and proof ledger establish exact revision fences, bounded metadata-first discovery, stable findings, layer separation, zero model/provider/paid/token cost, and unchanged external boundaries. | Runtime-ready for read-only source admission; the Builders Hub reference subject is blocked and every higher proof layer remains unverified |
| AI Voice Studio contract | One `/voice.studio` command selects exact `clone`, `dictate`, or `create` metadata routes while consent, rights, revocation, disclosure, provenance, approval, cost, and execution remain separately owned. | `VOICE-STUDIO.md`, `FACTS.md`, all three dictionaries, `MCP-GATEWAY.md`, and `npm run voice-studio-contract:check` prove the model-free clean-room ACOS contract and one `agentic-graph.voice.studio` wire identity. | Spec-complete; exact integrated agentic-graph local stdio MCP, deterministic adapter, idempotency, cancellation, read-back, cost, and fail-before-spend proof required |
...
| Soul identity layer | Durable identity, prompt slot 1, and personality overlay contracts are source-backed. | `SOUL.md`, `FACTS.md`, dictionaries, `MEMORY.md`, `SKILLS.md`, `HARNESS-CONTRACTS.md`, `MCP-GATEWAY.md`, and `PRD-TAD-ADR-MVP-GTM.md` expose matching routes, tags, bindings, guards, and VCCs. | Runtime-ready for docs |
| Persistent memory contracts | Bounded agent notes, explicit user profile, append-only monthly history, frozen snapshots, writes, compaction, and session search are documented as no-copy contracts. | `MEMORY.md`, `MEMORY-LOG.md`, `memory/YYYY-MM.md`, the pinned ADLC start/release workflows, and `VALIDATION-RUNBOOK.md` enforce hybrid structure at startup and byte-prefix preservation before release. | Runtime-ready for docs |
| Skills system contracts | Skill discovery, on-demand load, progressive disclosure, bundles, managed writes, and open-standard compatibility are documented as no-copy contracts. | `FACTS.md`, dictionaries, `SKILLS.md`, `MEMORY.md`, `AGENTS.md`, `HARNESS-CONTRACTS.md`, `MCP-GATEWAY.md`, and `PRD-TAD-ADR-MVP-GTM.md` expose matching routes, tags, bindings, guards, and VCCs. | Runtime-ready for docs |
...
| Kanban collaboration contracts | Durable task and handoff rows coordinate named profiles and full OS worker processes. | `huijoohwee/.workspace/.todo/docs/kanban.md`, `FACTS.md`, dictionaries, `SKILLS.md`, `MEMORY.md`, `AGENTS.md`, `HARNESS-CONTRACTS.md`, `MCP-GATEWAY.md`, and `PRD-TAD-ADR-MVP-GTM.md` expose matching routes, tags, bindings, guards, and VCCs. | Runtime-ready for docs |
| Context-sharded planning | Cross-repository planning loads one bounded index plus an exact immutable Context record instead of contending on a shared monthly file. | `huijoohwee/.workspace/.todo/docs/TODO.md`, `huijoohwee/.workspace/.todo/todo/YYYY-MM/<context>.md`, and the validation contract enforce identity, unique ownership, deterministic projection, and immutable legacy history. | Runtime-ready for docs |
| Todo planning-ledger compliance | Every release carries one attributable, base-absent Context record without rewriting historical rows or other task records. | The pinned ADLC start workflow records the exact base, Context, and path; the private owner validates one complete current-schema Context record and rejects legacy or extra path changes. | Runtime-ready for docs |
...
| Invocation dictionaries | `/`, `#`, and `@` dictionaries are source-backed. | `DICTIONARY-COMMAND.md`, `DICTIONARY-SEMANTIC.md`, `DICTIONARY-BINDING.md`, and the count-and-digest contract parse and fence 409 entries. | Contract-tested; no ACOS runtime route consumes the resolver directly |
...
| Mixture-of-agents contracts | `/moa` is documented as one-shot, bounded reference fan-out plus aggregator-owned response. | `FACTS.md`, dictionaries, `MEMORY.md`, `SKILLS.md`, `HARNESS-CONTRACTS.md`, `MCP-GATEWAY.md`, and `PRD-TAD-ADR-MVP-GTM.md` expose matching routes, tags, bindings, guards, and VCCs. | Runtime-ready for docs |
...
| Computing-flow contract | `/computing-flow` routes to Agentic OS/frontmatter, not a separate flow engine. | `/computing-flow`, `#computing-flow`, and `flow.computing` are present; focused Agentic OS computing-flow tests provide external supporting proof. | Runtime-ready for docs |
...
| Deploy boundary | Docs do not mutate Prod mirror or Cloudflare. | Git scoped status confirms `content/agentic-graph` is untouched; no deploy command is required or run. | Runtime-ready |

## External Runtime Gates

These capabilities remain outside the docs-only runtime-ready claim. Promote them only after current focused proof from `$AGENTIC_GRAPH_ROOT` or an explicitly approved deploy lane.

| Capability | Required proof | Status |
|---|---|---|
| Capability discovery | MCP/local catalog check exits 0; response includes deduplicated tool ids and zero cost. | Gated by focused `agentic-graph` proof |
| OS process view | Status call returns normalized entries and `unavailableSources[]` without mutation. | Gated by focused `agentic-graph` proof |
| Cost summary | Cost logs validate against schema; model-free views report exact zero. | Gated by focused `agentic-graph` proof |
| Gate catalog | Canonical gates are listed; reads do not issue or consume tokens. | Gated by focused `agentic-graph` proof |
| Circuit breakers | Registry returns each bounded loop; no loop lacks max iteration and circuit breaker. | Gated by focused `agentic-graph` proof |
| Soul prompt runtime | Prompt assembly loads scanned `SOUL.md` into slot 1 or returns typed fallback with no silent hardcoded default identity. | Gated by focused `agentic-graph` proof |
| Persistent memory runtime | Memory/profile write, compact, frozen snapshot, and session search return typed outputs with scan, capacity, and target-separation proof. | Gated by focused `agentic-graph` proof |
| Skills system runtime | Skill discovery, selected source loading, resource loading, bundle resolution, and managed writes return typed outputs with scan, validation, and approval policy proof. | Gated by focused `agentic-graph` proof |
| Context files runtime | Context discovery, scanned load, truncation, progressive hints, and audit return typed outputs with stronger facts/identity boundaries. | Gated by focused `agentic-graph` proof |
| Context references runtime | Reference expansion and audit return typed attached-context packets, warnings, refusals, source metadata, and unsupported-surface behavior. | Gated by focused `agentic-graph` proof |
| Kanban collaboration runtime | Task, handoff, and sync commands return typed row writes, conflict ledgers, profile bindings, and worker-process proof. | Gated by focused `agentic-graph` proof |
| Tools and toolsets runtime | Tool catalog reports functions, schemas, risk classes, toolsets, and platform-scoped enable/disable state with approval proof. | Gated by focused `agentic-graph` proof |
| Tool Gateway runtime | Tool catalog, provider selection, web search, image generation, TTS, and cloud browser route through existing infrastructure with typed outputs, approval, egress, and cost proof. | Gated by focused `agentic-graph` proof |
| Tool Search gateway execution | Loaded definitions dispatch through the real tool identity with schema validation, approval, hooks, audit, and execution cost proof. | Gated by focused `agentic-graph` proof |
...
| Sandbox Agents hosted containment | Deploy the Podman adapter or another provider with durable state, approved image lifecycle, host monitoring, retention, multi-tenant controls, and independently repeated containment evidence. | Local Podman proof passed; hosted deployment and formal third-party certification remain gated |
...
| Mixture-of-agents runtime | `/moa` preset resolution, all-settled fail-soft reference fan-out, typed branch audit and failure totals, aggregator action, cost separation, and no-recursion guard return typed outputs. | Gated by focused `agentic-graph` proof |
| Stateful orchestration runtime | Graph validation, checkpoint resume, human review, and stream trace return typed outputs, cost logs, and no-copy validation. | Gated by focused `agentic-graph` proof |
| Learning-loop runtime | Memory search, experience capture, skill proposal, skill evolution, and identity reflection return typed outputs, cost logs, review gates, and no-copy validation. | Gated by focused `agentic-graph` proof |
| Skill Evolution runtime | agentic-graph `agentic-graph.skill.evolve` must prove source-fenced plan/start/step/status/cancel, isolated evaluator-owned held-out rollouts, deterministic mini-batches, textual learning-rate bounds, strict directional promotion, revision-safe resume, phase-separated cost logs, and review-pending-only handoff. | Gated until `RUNTIME-PROOF.md` cites exact integrated revisions and exact passing agentic-graph test commands |
| Long-horizon SuperAgent runtime | `/superagent.run` returns typed plan, workspace scope, message ledger, checkpoints, artifacts, verification state, cost log, and stop condition without copied external runtime layout. | Gated by focused `agentic-graph` proof |
| Video Remix Director | Native live planning returns exact-span long-script units, audience-conditioned cinematography, scene camera rigs, stable blocking/backgrounds, intelligent first-frame character/environment/prior-timeline references, automatically generated spatial image prompts, bounded parallel image candidates, deterministic VLM consistency selection, dependency-aware same-camera parallel shot batches, story hierarchy, temporal continuity, bounded specialist negotiation, multimodal review, and one inspectable nine-stage agent DAG with typed handoffs, semantic asset indexing, resource accounting, checkpoint reuse, and dependency-propagated status; gates, Cost Logs, budgets, and retries fail closed. | Runtime-ready in Dev from focused `agentic-graph` proof; Prod mirror and Cloudflare gated |
| Canvas dashboard | Markdown/frontmatter/Agentic OS document renders without dashboard-only graph store. | Gated by local Canvas proof |
