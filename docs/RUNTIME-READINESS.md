...
| Running agents runtime | One application turn advances bounded model, tool, and handoff transitions; uses one continuation strategy per conversation; resumes pauses in the same turn; and streams through the same settlement loop. | `agent-api/src/{running-agents.js,running-agent-contract.js}`, isolate-scoped Worker injection, `RUNNING-AGENTS.md`, sanitized readiness, and seven focused offline tests prove transitions, exclusive history/session/conversation/previous-response state, incremental streaming, pause and resume, serialization, replay fencing, bounds, timeout, and honest cost. | Runtime-ready in Dev; previous-response lifecycle has bounded live proof; streaming remains offline-only |
| Guardrails and human review runtime | Source-referenced input and output checks run automatically around composed agent execution; tool checks stay beside the gateway; sensitive actions pause for one approve, reject, or edit decision. | `agent-api/src/guardrails-human-review.js`, composition wiring, `GUARDRAILS-HUMAN-REVIEW.md`, sanitized readiness, and five focused tests prove ordered validation, transformation, tool stages, same-turn pause/resume, review inspection, single consumption, edit revalidation, expiry, capacity, and replay rejection. | Runtime-ready in Dev for same-process review; application evaluator, restart-safe state, and live provider/tool proof remain gated |
...
| Durable human-review recovery | An atomic application store and paused-turn store survive restart, consume one exact decision, and resume the original run without replay, identity drift, or duplicate side effects. | Same-process state is proven; cross-process and multi-region recovery remain gated |
