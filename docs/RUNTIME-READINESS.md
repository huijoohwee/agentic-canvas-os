...
| Context-sharded planning | Cross-repository planning loads one bounded index plus an exact immutable Context record instead of contending on a shared monthly file. | `TODO.md`, `todo/YYYY-MM/<context>.md`, and the validation contract enforce identity, unique ownership, deterministic projection, and immutable legacy history. | Runtime-ready for docs |
