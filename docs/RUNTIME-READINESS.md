...
| Function calling runtime | Models may request strict application functions while exact call identity, guarded gateway execution, signed review, reasoning continuity, and separate costs remain application-owned. | `agent-api/src/{function-calling.js,openai-responses-function-adapter.js,knowgrph-function-gateway.js,function-calling-handler.js}`, Worker route wiring, `FUNCTION-CALLING.md`, sanitized readiness, and eighteen focused tests prove strict schemas, OpenAI translation, prior-response and same-id continuation, guardrails around the concrete Knowgrph status call, caller-approval rejection, exact reviewer evidence, usage, bounds, redaction, and honest costs. | Runtime-ready in Dev; reviewed gateway path proven offline and live provider proof gated |
