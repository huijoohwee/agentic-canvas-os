...
| Function calling runtime | Models may request strict application functions while exact call identity, guarded gateway execution, signed review, reasoning continuity, durable long-delay continuation, and separate costs remain application-owned. | `agent-api/src/{function-calling.js,function-calling-manager.js,function-calling-continuation.js,openai-responses-function-adapter.js,knowgrph-function-gateway.js,function-calling-handler.js,durable-object-state-store.js}`, Worker start/resume route wiring, `FUNCTION-CALLING.md`, sanitized readiness, and 21 focused tests prove strict schemas, OpenAI translation, exact prior-response, reasoning, and same-id continuation across a fresh manager, atomic claims, review retry without replay, definition drift fencing, guardrails around the concrete Knowgrph status call, raw-approval rejection, exact reviewer evidence, usage, bounds, redaction, and honest costs. | Runtime-ready in Dev; durable reviewed gateway continuation proven offline and live provider proof gated |
...
| Durable human-review recovery | Atomic review, paused-turn, and Function Calling continuation stores survive isolate restart, consume one exact decision, and resume the original run or provider response chain without replay, identity drift, or duplicate tool execution. | Cross-isolate behavior is proven with one local Durable Object implementation; deployed multi-region behavior and live reviewed side effects remain gated |
