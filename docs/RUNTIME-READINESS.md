...
| Deterministic knowledge graph contract | Exact host invocations ingest a bounded local codebase graph, query one digest-bound snapshot, and explain one stored source-backed edge through three explicit Knowgrph local MCP tools. | `KNOWLEDGE-GRAPH.md`, `FACTS.md`, all three dictionaries, `MEMORY.md`, `MCP-GATEWAY.md`, `RUNTIME-PROOF.md`, and focused tests prove exact dispatch, metadata-only lookup, deterministic parser and digest evidence, explanation for every edge, typed omissions, vector-free behavior, and Graphify clean-room independence. | Runtime-ready for ACOS contract; Knowgrph local MCP fixture and protected integration proof remain required for combined execution readiness |
