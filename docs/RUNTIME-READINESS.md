...
| Function calling runtime | Models may request strict application functions through explicit selection and bounded parallel policy while exact call identity, active reasoning continuity, gateway authorization, and separate costs remain application-owned. | `agent-api/src/{function-calling.js,openai-responses-function-adapter.js,knowgrph-function-gateway.js,function-calling-handler.js}`, Worker route wiring, `FUNCTION-CALLING.md`, sanitized readiness, and seventeen focused tests prove strict schemas, OpenAI Responses translation, one-shot forced selection, prior-response and same-id continuation, opaque reasoning replay, usage accounting, application allowlisting, Knowgrph MCP policy, bounds, redaction, and honest costs. | Runtime-ready in Dev; bounded live provider proof gated by server credentials |
| Agent definitions runtime | Each specialist packages an exact model route, ordered instructions, and optional reference-only tools, guardrails, MCP servers, handoffs, and output contract without acquiring execution authority. | `agent-api/src/agent-definitions.js`, isolate-scoped Worker injection, `AGENT-DEFINITIONS.md`, sanitized readiness, and seven focused offline tests prove immutable registration, revision fencing, capability authorization, handoff verification, bounds, and text or structured output validation. | Runtime-ready in Dev; downstream execution and live provider proof gated |
| Models and providers runtime | Prepared agents resolve one revision-bound provider, explicit model, and compatible transport through agent, run, process, then provider-local defaults without calling an adapter. | `agent-api/src/{model-providers.js,model-config.js}`, Agent Definition alignment, isolate-scoped Worker injection, `MODELS-AND-PROVIDERS.md`, sanitized readiness, and seven focused offline tests prove precedence, revision fencing, exact features, transport behavior, strict environment input, bounds, and secret redaction. | Runtime-ready in Dev; adapter execution and live provider proof gated |
| Running agents runtime | One application turn advances bounded model, tool, and handoff transitions; uses one continuation strategy per conversation; resumes pauses in the same turn; and streams through the same settlement loop. | `agent-api/src/{running-agents.js,running-agent-contract.js}`, isolate-scoped Worker injection, `RUNNING-AGENTS.md`, sanitized readiness, and seven focused offline tests prove transitions, exclusive history/session/conversation/previous-response state, incremental streaming, pause and resume, serialization, replay fencing, bounds, timeout, and honest cost. | Runtime-ready in Dev; downstream adapter and live provider proof gated |
| Sandbox Agents runtime | Application-approved agent work can use one provider-neutral container workspace for bounded files, argument-vector commands, packages, private or preview ports, opaque snapshots, pause, cross-controller resume, and cleanup. | `agent-api/src/{sandbox-agents.js,sandbox-agent-contract.js}`, isolate-scoped Worker injection, `SANDBOX-AGENTS.md`, sanitized readiness, and seven focused offline tests prove provider revision fencing, capability and approval gates, safe workspace inputs, serialization, secret rejection, snapshot seeding, external resume state, cost, and honest containment status. | Runtime-ready in Dev; real container adapter and independent containment proof gated |
...
| Models and providers live execution | A registered adapter executes the resolved provider revision, model, and transport and returns supported-feature, delivery, connection, usage, and cost evidence. | Gated by adapter wiring plus approved bounded live proof |
| Sandbox Agents live containment | One injected adapter, application authorizer, and external state store run the bounded suite in a real container and return provider plus independent host evidence for filesystem, process, package, port, snapshot, resume, secret, and cleanup boundaries. | Gated by downstream container integration and approved live proof |
| Running agents live execution | One downstream adapter returns real provider continuation, model/tool/handoff events, pause or final settlement, and actual usage without mixing continuation strategies or duplicating gateway policy. | Gated by adapter wiring plus approved bounded live proof |
