...
| Function calling runtime | Models may request strict application functions through explicit selection and bounded parallel policy while exact call identity, active reasoning continuity, gateway authorization, and separate costs remain application-owned. | `agent-api/src/function-calling.js`, isolate-scoped Worker injection, `FUNCTION-CALLING.md`, sanitized readiness, and eleven focused offline tests prove strict nested schemas, selection modes, same-id continuation, reasoning replay, gateway policy, bounds, serialization, timeouts, and honest costs. | Runtime-ready in Dev; live model adapter and tool gateway gated |
