...
| Commerce adapter admission runtime | One private `agentic-graph` registration intent crosses into `agentic-canvas-os` only under an exact authoring permit and becomes a permanent, replayable native registration without isolate-local authority. | `agent-api/src/commerce-admission-{contract,provider}.js`, the existing `AGENT_STATE` binding, pure native preflight, revisioned projection rehydration, the content-addressed Commerce fixture, the scoped 2026-09-03 operator instruction, and focused tests prove service HMAC before parsing, all twelve permit headers, full-intent digest recomputation, atomic fence/outcome/registration/allowlist-ownership writes, A/A and A/B/A replay, warm-isolate convergence, concurrency, restart, byte-identical receipts, bounded bodies, private routing, and projection-failure recovery. | Runtime-ready offline and configuration-ready in the existing root and Dev services; Cloudflare deployment, any separately owned staging environment, and cross-repository protected release remain gated |
...
| Deterministic agentic graph invocation | One exact `/`, `#`, and `@` contract forwards typed ingest, query, and edge-explanation requests to the Knowgrph executable owner without exposing artifact paths or duplicating its parser and store. | `AGENTIC-GRAPH.md`, facts, all three dictionaries, `MCP-GATEWAY.md`, `src/knowgrph-mcp-client.js`, and `npm run agentic-graph-contract:check` prove exact tool names, opaque graph identity, snapshot-digest fencing, ingest projection fields, and duplicate-runtime removal. | Contract/client-ready in Dev; exact protected Knowgrph runtime and Canvas proof remain required for combined runtime readiness |
| Repository packing contract | One exact local Git worktree becomes one deterministic bounded content-addressed Markdown artifact through `/repository.pack #repository-packing @repository-root @runtime-proof` and `agenticgraph.repository.pack`. | `REPOSITORY-PACKING.md`, facts, dictionaries, skills, harness, MCP gateway, focused contract and clean-room checks, plus exact Knowgrph contract/runtime/stdio/independence tests prove containment, ignores, ordering, hashes, omissions, secrets, bounds, atomic replay, unchanged source, and zero network/model/token/cost/deploy. | Runtime-ready for ACOS contract; combined readiness requires the exact integrated Knowgrph revision and docs pin |
...
These capabilities remain outside the docs-only runtime-ready claim. Promote them only after current focused proof from `$AGENTICGRAPH_ROOT` or an explicitly approved deploy lane.
...
| Mixture-of-agents runtime | `/moa` preset resolution, all-settled fail-soft reference fan-out, typed branch audit and failure totals, aggregator action, cost separation, and no-recursion guard return typed outputs. | Gated by focused `knowgrph` proof |
...
| Protected ACOS Worker release | Exact protected-main candidate, first-attempt GitHub production-environment human approval, inactive strict Wrangler upload, full managed topology plus preserved-baseline binding readback, baseline compare, public plus owned loopback service-bound readiness, and storage-safe recovery produce typed receipts. | Source-ready and adversarially tested; external environment, credentials, and live deployment evidence remain absent |
