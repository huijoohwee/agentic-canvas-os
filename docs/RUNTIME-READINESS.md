...
| Native deterministic knowledge graph | One explicit local workspace becomes a digest-fenced queryable graph through inert generated parser artifacts and built-in code, document, SQL, config, and PDF adapters; every edge stores exact explanation and evidence. | `KNOWLEDGE-GRAPH.md`, facts, all three dictionaries, `MCP-GATEWAY.md`, native source modules, the stdio entry point, and `npm run knowledge-graph:check` prove 61/61 checks for deterministic ingestion, parser/work limits, shared token limits, source binding, snapshot revalidation, query bounds, stale reads, closed tools, zero external identity mentions, and zero model/network/embedding/vector behavior. | Runtime-ready locally in Dev; no source mutation, external parser or graph service, provider execution, Prod, or Cloudflare |
