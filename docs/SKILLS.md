...
skill_contracts:
  ...
  - "superagent.run"
  - "sme.risk.profile"
skill_variants: ["agent.moa", "agent.investment-research", "agent.pmf", "agent.sme-care", "agent.video", "agent.docs", "agent.code", "agent.cost", "agent.learning", "agent.orchestrator"]
...
| `agent.pmf` | `/pmf-agent` | Evidence-led product-market-fit hypotheses, problem-to-solution gap mapping, and falsifiable experiment planning. | `@pmf-agent`, `@source.frontmatter`, `@source.body`, `@cost-log`, `@runtime-proof`, `#pmf-agent` | `agent.pmf` definition, centralized prompt, evidence and signal ledgers, confidence and freshness fields, ranked gaps, experiment thresholds, stop conditions, max iteration, insufficient-evidence fallback, and approval gates before outreach, spend, persistence, or mutation. | Runtime-ready |
| `agent.sme-care` | `/sme-care-agent` | Invoke internal skill variant `agent.sme` / `sme.risk.profile` for deterministic SME exposure, coverage-gap, unknown-risk, growth-change, and provider-neutral guidance. | `@operator`, `@source.frontmatter`, `@source.body`, `@local-harness`, `@cost-log`, `@runtime-proof` | Shared `agent.sme-care` definition and kernel; `knowgrph-sme-profile/v1` -> `knowgrph-sme-risk-run/v1`; cyber/supply-chain/asset fan-out/fan-in; max iteration 1; timeout <=300s; token budget <=100000; exact zero-cost model-free path; typed schema/missing-binding/approval/provider/budget fallbacks; atomic Source Files writes; purchase, bind, contact, paid, Prod, and Cloudflare actions remain gated. | Runtime-ready |
| `agent.video` | `/video-agent` | Storyboard, eight-sheet text package, source-consistent media generation, persistence/read-back, render handoff, and approval-gated video workflow. | `@operator`, `@video-generation-demo-script`, one `@provider.*`, `@text`, `@image`, `@audio`, `@video`, `@local-harness`, `@cost-log`, `@runtime-proof`, one `#spec.*`, one `#thinking.type.*`, one `#token-cap.*` | `agent.video` definition, typed brief, Character/Scene/Dialogue/Visual asset/Audio/Timing/Metadata/Prompt sheets, asset manifest, render approval gate, zero-spend load path, generated-artifact boundary. | Runtime-ready |
...
The `/<agent-domain>-agent` form is a naming convention, not an executable wildcard. Each token must resolve to exactly one registered `agent.<agent-domain>` definition; `/research-agent`, `/care-agent`, `/product-market-fit-agent`, and unknown `/*-agent` forms fail as unsupported variants rather than acting as compatibility aliases. Knowgrph owns the reusable execution registry and kernel; this catalog owns only invocation authority and routing semantics.
