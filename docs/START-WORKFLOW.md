...
For v3, the published fence-to-remote/PR prefix is a separate evidence subject from the complete fence-to-local-HEAD range. Its exact remote tree, changed-path and declared/protected partition digests, and binary range digest are schema-bound. Each out-of-scope remote-prefix path must match the mode and blob at the one exact shared ancestor of that remote head and fetched protected main; the evidence binds the fetched main SHA/tree and shared-ancestor SHA/tree and proves `source base <= shared ancestor <= protected main` plus `shared ancestor <= remote head`. Out-of-scope paths in the complete fence-to-local-HEAD range remain independently bound to the current fetched protected-main mode/blob. An authorized shared-history prefix therefore remains recoverable after protected main advances, while arbitrary published-prefix bytes cannot be hidden by reverting them only in the later local suffix. Both subjects are revalidated around cloud/local CAS and marker publication. Before local CAS, recovery reads the exact source pull-request body, recaptures the complete snapshot, projects the renewed lease at that final instant, and validates the exact updated body against GitHub's 65,536-byte limit immediately before CAS, so a valid maximum-size proof cannot strand an unpublishable recovered marker or widen the pre-CAS drift window.
...
For every implementation lane, `turn:end` must finish ready against the canonical protected SHAs with no runtime-blocking residue, or the final response must report the missing integration or runtime proof. Task heads never replace canonical runtime ownership.
