...
Protected-branch policy, remotely addressable task refs, provider review, required checks, and integration receipts form the upstream publication contract. The authenticated cloud collaboration ledger is the sole authority for writer ownership, integration authorization, and retirement. Git refs, provider review requests, local leases, worktree registration, lifecycle reports, and runtime reports are required evidence projections; none can transfer authority. Reconcile each observation as `aligned`, `stale`, `missing`, or `contradictory` against an authenticated ledger readback. Only an accepted ledger compare-and-swap transition changes authority. A stale projection is a typed diagnostic, never grounds for a new recovery lane, ownership transfer, or cleanup. Protected auto-merge arming is readback-closed: operation-owned required-check evidence and exact delivery authorization block provider mutation; the GitHub adapter reads the authenticated user and exact same-repository, non-draft OPEN pull request, binding repository, node identity, URL, `main` base/SHA, task branch/head SHA, and null or exact pre-existing auto-merge. A new arm requires an exact null pre-snapshot and, whether its command succeeds or fails, an exact readback of the actor database ID/node ID/login/type, SQUASH method, deterministic headline, and a non-null byte-exact body from the bounded original/final-protected-refresh set. An exact pre-existing request is adopted only after the same current-authority verification; a non-exact pre-existing request is preserved for explicit owner reconciliation. The bounded post-arm outcomes are: accept an exact request plus fresh authority, including response loss; for only a freshly armed, stable exact subject that began null and remains same-actor/SQUASH/headline attributable with a null or drifted body, reverify delivery authority and then attempt one provider cancellation, rereading exact null and adopting cancellation response loss only from that null; or fail without cancellation/re-arm when identity, base/head, attribution, or delivery authority drifts. GitHub exposes no expected-head or request-identity compare-and-swap for `disablePullRequestAutoMerge`, so this cancellation is a single-writer, fresh-authority-fenced compensation, not an atomic or head-matched provider operation. An immediate merge is accepted only for the exact reviewed base/head, full authenticated `merged_by`, exact merge SHA, provider-canonical Git bytes, and null or exact retained `auto_merge`. Every terminal completion, replay, and proven protected-head-refresh successor must prove the approved squash subject and either byte-exact managed-lane body or a bounded deterministic refresh/device-arm body before any mutation-capable terminal authority verifier, runtime readiness, or cleanup; rewritten bodies or final attribution trailers fail closed even with a valid merge SHA.
...
- Pursue the goal autonomously within these rules. `AUTONOMOUS-GOAL-PURSUIT.md` owns one-shot input completeness, internal operand derivation, earliest-point validation, late binding of volatile identity, environment-only remediation before a verdict, decision-only escalation, and deadlock avoidance; it relaxes no ownership, fence, approval, or deploy gate in this document. Classify every rejection contended or deterministic, never retry a deterministic rejection unchanged, correct values at their owning source rather than at a lease, marker, or report projection, and treat a cascade of gates each broken by the previous fix as proof the defect is upstream of all of them.
...
Before cleanup, inventory every value-bearing commit unique to the task ref and prove each commit is included by the canonical integration or retained through an immutable recovery handle. Worktree-registration pruning, remote-tracking-ref pruning, local or remote branch deletion, and unreachable-object pruning are four separate effects; authority for one never implies another, and object pruning remains forbidden while any unique value lacks canonical inclusion or immutable recovery.
