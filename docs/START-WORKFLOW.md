...
For v3, the published fence-to-remote/PR prefix is a separate evidence subject from the complete fence-to-local-HEAD range. Its exact remote tree, changed-path and declared/protected partition digests, binary range digest, and protected-main mode/blob equivalence are schema-bound and revalidated around cloud/local CAS and marker publication. An unauthorized published-prefix path therefore cannot be hidden by reverting it only in the later local suffix. Before local CAS, recovery reads the exact source pull-request body, recaptures the complete snapshot, projects the renewed lease at that final instant, and validates the exact updated body against GitHub's 65,536-byte limit immediately before CAS, so a valid maximum-size proof cannot strand an unpublishable recovered marker or widen the pre-CAS drift window.
