...
- Dev deployment reads dynamic model and pricing values from an operator-owned offline env file outside the repository. Load the file only into the deployment shell, confirm the required `OPENAI_FUNCTION_CALLING_*` names are present without printing values, then run `npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run function-gateway:deploy:dev`; secrets remain Worker secrets and the command authorizes only the Dev Worker environment.
