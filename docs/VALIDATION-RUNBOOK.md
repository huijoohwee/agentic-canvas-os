...
EXTERNAL_COPY_PATTERN='hermes-agent/src|agentskills\.io/skills|You are Hermes Agent|kawaii Cute expressions|catgirl Neko|hermes moa preset example|MoA provider config example|GEPA optimizer code|DSPy optimizer code|langgraph/graph\.py|StateGraph example|MessagesState example|deer-flow/backend|deerflow/models|deerflow/sandbox|deerflow config example|kimi-agent-swarm/src|moonshotai/kimi-agent-swarm|agent_swarm\.py|kimi swarm provider config example|nemo_agent_toolkit|nvidia_nat|nat/cli|nat/profiler|nat/optimizer|nat/evaluator|tools/tool_search\.py|tests/tools/test_tool_search\.py|openclaw-tool-search-report|tool_search\.py|test_tool_search\.py|prompt_builder\.py|subdirectory_hints\.py|context_references\.py|reference_expander\.py|test_context_references\.py|kanban_runtime\.py|test_kanban\.py|InputGuardrailTripwireTriggered|GuardrailFunctionOutput|needsApproval|needs_approval|function_tool'
! (rg -n "$EXTERNAL_COPY_PATTERN" "$DOCS_ROOT" "$AGENTIC_CANVAS_OS_ROOT"/agent-api/src/agent-swarm*.js "$AGENTIC_CANVAS_OS_ROOT"/agent-api/src/agent-toolkit*.js "$AGENTIC_CANVAS_OS_ROOT"/__tests__/agent-swarm.test.mjs "$AGENTIC_CANVAS_OS_ROOT"/__tests__/agent-toolkit.test.mjs | rg -v 'VALIDATION-RUNBOOK\.md:[0-9]+:EXTERNAL_COPY_PATTERN=') && ! (git -C "$AGENTIC_CANVAS_OS_ROOT" diff --unified=0 -- agent-api/src/app.js agent-api/src/durable-object-state-store.js worker/agent-state.js worker/index.js __tests__/agent-api-app.test.mjs __tests__/cloudflare-worker.test.mjs __tests__/durable-agent-state.test.mjs | rg '^\+[^+]' | rg "$EXTERNAL_COPY_PATTERN") && git -C "$AGENTIC_CANVAS_OS_ROOT" diff --exit-code -- package-lock.json npm-shrinkwrap.json && ! (git -C "$AGENTIC_CANVAS_OS_ROOT" diff --unified=0 -- package.json | rg '^[+-][[:space:]]*"(dependencies|devDependencies|peerDependencies|optionalDependencies)"')
```
Expected:
...
Planning moved to `huijoohwee.github.io`. Select that checkout explicitly; do not create a local ledger.
Run `npm run planning:check` there to validate the TODO contract, immutable source hashes and Kanban
projection. Its `docs/TODO.md` owns exact record creation and release arguments. Canvas runs only:

```bash
npm run kanban:check
```

This rejects stale routing documents and any recreated local `todo/` or planning projector.
The central migration manifest binds all imported source bytes; Canvas Git history remains intact.
...
```bash
npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run running-agents:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run durable-agent-state:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run guardrails-human-review:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-runtime-composition:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-orchestration:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-swarm:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-toolkit:check
node --test \
  "$AGENTIC_CANVAS_OS_ROOT/__tests__/agent-api-app.test.mjs" \
  "$AGENTIC_CANVAS_OS_ROOT/__tests__/cloudflare-worker.test.mjs"
```
...
- Pause state stays opaque; exact signed approve, reject, or edit review continues the same turn once, invalid evidence is non-consuming, edits require validation, and replay or expiry fails closed. A fresh instance claims durable state once, then commits, replaces, or releases it while competing claims block.
...
- Readiness exposes sanitized policy, limits, and counters while provider execution stays `unverified`; Agent Swarm proves dynamic durable coordination, while Agent Toolkit proves digest-bound application authorization, atomic per-principal admission, metadata-only trust labels and exporter events, percentile/bottleneck profiling, remote-evidence exclusion, bounded deterministic evaluation and multi-candidate recommendation, durable alarm recovery, honest metrics and cost, and proposal-only learning.
...
npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run model-providers:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-runtime-composition:check
node --test \
  "$AGENTIC_CANVAS_OS_ROOT/__tests__/agent-api-app.test.mjs" \
  "$AGENTIC_CANVAS_OS_ROOT/__tests__/cloudflare-worker.test.mjs"
```
...
- Minimal definitions require an exact application-verified source URI and SHA-256 digest, preserve provider and model identity, and resolve revision-bound model and transport packets without adapter execution.
...
ruby -ryaml -e 'root=ENV.fetch("DOCS_ROOT"); parse=->(name){text=File.read(File.join(root,name)); YAML.safe_load(text.match(/\A---\n(.*?)\n---\n/m)[1], aliases: true)}; command=parse.call("DICTIONARY-COMMAND.md").fetch("dictionary_entries"); semantic=parse.call("DICTIONARY-SEMANTIC.md").fetch("dictionary_entries"); binding=parse.call("DICTIONARY-BINDING.md").fetch("dictionary_entries"); skills=parse.call("SKILLS.md"); memory=parse.call("MEMORY.md"); required_commands=%w[/kanban.task /kanban.handoff /kanban.sync]; required_semantics=%w[#kanban-board #task-row #profile-handoff #worker-process #multi-agent-collaboration]; required_bindings=%w[@kanban-board @task-row @handoff-row @agent-profile @worker-process]; required_skills=%w[kanban.collaborate]; missing=required_commands.reject{|x| command.include?(x)}+required_semantics.reject{|x| semantic.include?(x)}+required_bindings.reject{|x| binding.include?(x)}+required_skills.reject{|x| skills.fetch("skill_contracts").include?(x)}; abort("missing kanban route coverage: #{missing.join(", ")}") unless missing.empty?; abort("kanban memory missing") unless memory.dig("agentic_os_memory","kanban_collaboration","commands").is_a?(Array); puts "kanban route consistency ok"'
...
- In `huijoohwee.github.io`, `docs/kanban.md` `## Ledger Projection` regenerates byte-identically from the `active_period` context records, its declared row count, period, and digest match, and no projected id collides with an authored `KANBAN-` row; a hand edit inside the fence fails closed.
...
| Private commerce admission | `npm run commerce-admission-provider:check` exits zero; validate caller HMAC, signed local configuration binding for exact intent, inputs, and permit, expiry/tamper/wrong-owner failure, private-route-only access, Durable Object fence/replay, warm-isolate rehydration, and zero writes before configuration acceptance. Do not run a Cloudflare deploy from this repository. |
