...
- Readiness exposes sanitized policy, limits, and counters while provider execution stays `unverified`; Agent Swarm proves dynamic durable coordination, while Agent Toolkit proves digest-bound application authorization, metadata-only trust labels, remote-evidence exclusion, bounded deterministic evaluation, durable alarm recovery, honest metrics and cost, and proposal-only learning.
...
```
The proof must show one atomic Git-common-directory lease registry, parallel claims for distinct registered worktrees and scopes, same-worktree exclusion, increasing epochs after expiry, bounded heartbeat renewal, session/branch/worktree fencing, different-scope pull-request coexistence, duplicate-scope rejection, and a draft ownership pull request created before normal authoring. `node --test __tests__/worktree-lifecycle.test.mjs` must additionally prove that attributed post-baseline untracked paths remain `owned-untracked` in their physical task worktree and pull request, report path/size/object-id/time metadata without copying content, block only that semantic scope, and reject cleanup, while canonical or unattributed dirt remains fail-closed.
...
| Multi-worktree writer coordination and Agentic SDLC conformance | Focused writer-lease, repository-guard, device-branch, and worktree-lifecycle tests exit zero; same-worktree competing sessions, branch reuse, and duplicate active scopes fail, while distinct registered worktrees and scopes coexist, heartbeat extends only the owner, and `owned-untracked` state stays in place under its worktree-bound lease and pull request without delete, stash, mask, relocation, adoption, completion, or cleanup. `npm run agentic-sdlc:check` proves the canonical run evaluator, distinct authoring/Operator/persistence trust roots, whole-ledger signatures, structured check receipts, exact cap/recovery joins, and stage-load accounting; `npm run agentic-sdlc:source:check` binds the actual guideline revision/digests/Rule IDs, and `npm run agentic-sdlc:verify -- --run <explicit-run.json>` exits zero only for a runtime-ready artifact. Every command keeps the Deploy Boundary closed. |
