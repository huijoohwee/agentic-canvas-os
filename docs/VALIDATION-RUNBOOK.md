...
| Function Calling | `npm run function-gateway:check` exits zero with 33 focused checks; controller, durable manager, execution-receipt, MCP transport, OpenAI adapter, guarded gateway, app, and Worker tests confirm exact continuation, one execution claimant, stable-key uncertain retry, required native mutation receipt echo, fresh-isolate terminal replay, signed review, strict policy, redaction, and sanitized state. Live provider and mutating-tool execution remain gated. |
