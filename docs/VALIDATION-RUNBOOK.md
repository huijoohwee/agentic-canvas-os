...
- Reasoning items and exact call ids continue only inside the active previous-response loop and never enter final output.
- Callers cannot supply schemas, MCP names, or approval arrays; the application allowlist maps only `read_agentic_os_status` to `knowgrph.os.status`, where tool-input and tool-output guardrails bracket ordinary or exact signed-review execution.
...
| Function Calling | `npm run function-gateway:check` exits zero; controller, OpenAI adapter, guarded Knowgrph gateway, app, and Worker tests confirm strict schemas, call-id and prior-response continuation, reasoning replay, caller-approval rejection, signed review, usage, policy, redaction, and sanitized state. Live provider execution remains gated. |
