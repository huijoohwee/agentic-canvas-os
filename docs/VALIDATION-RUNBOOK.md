...
- Callers cannot supply schemas, MCP names, review ids, stored continuation, or approval arrays; the application allowlist maps only `read_agentic_os_status` to `knowgrph.os.status`, where tool guardrails bracket ordinary or exact signed-review execution. Invalid reviewer authentication preserves retryable state without MCP or model work; concurrent claims, expiry, multiple calls beside review, non-idempotent review, and current-definition drift fail closed.
...
```bash
npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run running-agents:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run durable-agent-state:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run guardrails-human-review:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-runtime-composition:check && npm --prefix "$AGENTIC_CANVAS_OS_ROOT" run agent-orchestration:check
node --test \
  "$AGENTIC_CANVAS_OS_ROOT/__tests__/agent-api-app.test.mjs" \
  "$AGENTIC_CANVAS_OS_ROOT/__tests__/cloudflare-worker.test.mjs"
```
...
| Function Calling | `npm run function-gateway:check` exits zero; controller, durable manager, OpenAI adapter, guarded Knowgrph gateway, app, and Worker tests confirm strict schemas, exact call and prior-response continuation across a fresh manager, private reasoning replay, atomic claims, invalid-review retry without execution, definition drift fencing, raw-approval rejection, signed review, usage, policy, redaction, and sanitized state. Live provider execution remains gated. |
...
| Guardrails, Progressive Agents, Runtime Composition, and Orchestration | `npm run guardrails-human-review:check`, `npm run progressive-agents:check`, `npm run agent-runtime-composition:check`, and `npm run agent-orchestration:check` exit zero; affected gateway, definition, app, and Worker tests confirm automatic checks, signed reviewer identity, atomic review state, source and provider fencing, owner reuse, honest costs, and sanitized readiness. |
