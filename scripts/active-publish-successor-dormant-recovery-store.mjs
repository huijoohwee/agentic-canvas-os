// Responsibility: Persist one replay-safe recovery intent behind a process-safe operation fence.
import {
...
export const ACTIVE_PUBLISH_SUCCESSOR_DORMANT_RECOVERY_JOURNAL_SCHEMA =
  "agentic-active-publish-successor-dormant-recovery-journal/v1";

export function createActivePublishSuccessorDormantRecoveryStore({
  gitCommonDir,
  statePath = null,
  journalFile = null,
  operationId = null,
  ...
  function readIntent() {
    ...
    const intent = normalizeActivePublishSuccessorDormantRecoveryIntent(journal.intent);
    if (journal.schema !== ACTIVE_PUBLISH_SUCCESSOR_DORMANT_RECOVERY_JOURNAL_SCHEMA
  ...
  function writeIntent({ expectedIntent = null, nextIntent } = {}) {
    ...
    return withFileLock(intentLockPath, "intent-cas", () => {
      ...
      const intent = normalizeActivePublishSuccessorDormantRecoveryIntent(nextIntent);
      writeAtomic(filePath, {
        schema: ACTIVE_PUBLISH_SUCCESSOR_DORMANT_RECOVERY_JOURNAL_SCHEMA,
        intent,
...
function resolveStatePath({
  ...
  const key = digestValue({ schema: "agentic-active-publish-successor-dormant-recovery-store-key/v1", ...subject });
  return path.join(
    common, "agentic-canvas-os", "active-publish-successor-dormant-recovery", `${key}.json`,
  );
}
...
function nullableIntentDigest(value) {
  ...
  return normalizeActivePublishSuccessorDormantRecoveryIntent(value).intentDigest;
}
...
function currentInstant(now) {
  ...
  if (!validInstant(instant)) throw new Error("Recovery journal clock is invalid.");
  return instant;
}
