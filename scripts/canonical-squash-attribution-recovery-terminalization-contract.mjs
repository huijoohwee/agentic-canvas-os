...
function normalizeEvidence(value) {
  ...
  if (value.recovery.controllerRevision !== value.subject.pinTransition.newRevision) {
  ...
  if (value.recovery.changedEntries.length !== 1
  ...
  if (!Array.isArray(value.recovery.changedPaths) || value.recovery.changedPaths.length !== 1
...
function normalizeRunPair(value, { label, sourceSha, sourceBranch, mergeSha }) {
  ...
  normalizeRun(value[1], `${label} protected`, "push", "main", mergeSha);
}
function normalizeRun(value, label, event, branch, revision) {
  object(value, label);
  if (!Number.isSafeInteger(value.databaseId) || value.databaseId < 1
    ...
    || value.workflowName !== "Integration" || value.conclusion !== "success") invalid(label);
}
...
function normalizeRecoveryTerminal(value, recovery) {
  ...
  if (value.status !== "completed-and-cleaned" || value.branch !== recovery.pullRequest.headBranch
    ...
    || value.completion?.mainSha !== recovery.mergeSha
