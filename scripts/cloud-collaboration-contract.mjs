import { CLOUD_COLLABORATION_BOUNDS, ENTRY_SCHEMA, LEGACY_ENTRY_SCHEMA, LEDGER_SCHEMA,
  ...
  claimOnlySuccessorBaseRolloverPredecessor, normalizeActor, normalizeCanonicalDescendantProof,
  normalizeRepository, normalizeRootIntent, normalizeWriteSet, text, validateLedger, writeSetsOverlap,
...
const SCOPE_RESERVATION_STATES = new Set([ ...WRITE_AUTHORITY_STATES, "reviewed",
  "integrated-preserved", "dormant-preserved", ]);
export const CLAIM_CONFLICT_DECISION_SCHEMA = "agentic-cloud-claim-conflict-decision/v1";
const OPERATION_RECEIPT_SCHEMAS = Object.freeze({ claim: "agentic-collaboration-claim-receipt/v1",
...
function hydrate(entry, evaluationTime = null) { if (!entry) return null; const claim = {
  ...
  return claim; }
function projectAuthorityState(state) { if (state === "active") return "current";
  if (["review-ready", "delivery-authorized"].includes(state)) return "reviewed";
  if (state === "parked" || state === "expired") return "dormant-preserved";
  if (state === "released") return "retired"; return state; }
function currentClaimEntries(ledger) { const latest = new Map();
  for (const entry of ledger.entries) latest.set(entry.claimId, entry); return latest; }
function currentClaims(ledger, evaluationTime) { const latest = currentClaimEntries(ledger);
  const legacyLineages = new Set(ledger.entries
    .filter((entry) => entry.schema === LEGACY_ENTRY_SCHEMA && entry.action === "claim")
    .map((entry) => entry.claimId)); const legacyConsumed = new Set([...latest.values()]
    .filter((entry) => legacyLineages.has(entry.claimId))
    .map((entry) => entry.claimCore.predecessorClaimId) .filter(Boolean));
  return [...latest.values()] .filter((entry) => !legacyConsumed.has(entry.claimId))
...
export function listCurrentClaims(ledger, evaluationTime, { repositoryId = null } = {}) {
  ...
  return currentClaims(ledger, evaluatedAt) .filter((claim) => !TERMINAL_STATES.has(claim.state))
    .filter((claim) => !targetRepositoryId || claim.repositoryId === targetRepositoryId)
    .sort((left, right) => left.claimId.localeCompare(right.claimId)); }
function findClaimEntry(ledger, claimId) { return currentClaimEntries(ledger).get(claimId) ?? null;
}
...
function hydrateWithLedger(ledger, entry, evaluationTime) {
  const claim = hydrate(entry, evaluationTime); if (!claim) return null;
  ...
  return claim; }
function receiptForEntry(entry, status) { if (entry.schema === LEGACY_ENTRY_SCHEMA) {
    const receipt = { schema: RECEIPT_SCHEMA, action: entry.action,
      repositoryId: entry.repositoryId, claimId: entry.claimId, claimDigest: entry.claimDigest,
      fenceRevision: entry.claimDigest, ledgerRevision: entry.digest,
    ...
    return { ...receipt, receiptDigest: digestValue(receipt) }; } const receipt = {
    schema: OPERATION_RECEIPT_SCHEMAS[entry.action], operation: entry.action, status,
    repositoryId: entry.repositoryId, claimId: entry.claimId, claimDigest: entry.claimDigest,
    fenceRevision: entry.claimDigest, ledgerRevision: entry.digest, ledgerSequence: entry.sequence,
...
function mutationResult(ledger, entry, replayed, { conflictSetDigest = null,
  claimConflictDecision = null } = {}) { return { ledger,
    ...
    claimConflictDecisionSchema: claimConflictDecision?.schema ?? null,
    claimConflictDisposition: claimConflictDecision?.disposition ?? null,
    claimConflictDecisionDigest: claimConflictDecision?.decisionDigest ?? null,
...
function requireOwnedClaim(ledger, intent, evaluationTime) {
  ...
  const leaseIndependent = ( claim.state === "dormant-preserved"
    && (intent.mode === "recovery" || intent.reason) ) || ( claim.state === "waiting-successor"
    && Boolean(intent.reason) ); if (claim.actorId !== intent.actorId || (!leaseIndependent && (
  ...
  if (intent.expectedLedgerDigest && intent.expectedLedgerDigest !== ledger.headDigest) {
    fail("stale_ledger_digest", "expectedLedgerDigest is stale"); } return claim; }
...
function continueClaim({ base, previous, intent, ledger, evaluationTime }) {
  if (intent.mode === "projection") {
    ...
    const reservedOverlap = currentClaims(ledger, evaluationTime).find((claim) => (
      claim.claimId !== previous.claimId && claim.repositoryId === previous.repositoryId
      && claim.scopeReserved
    ...
    return { ...base, state: ["reviewed", "integrated-preserved"].includes(previous.recordedState)
        ? previous.recordedState : "current", deviceId: intent.deviceId,
      sessionId: intent.sessionId, expiresAt: intent.expiresAt,
  ...
  const reservedOverlap = claims.find((claim) => ( claim.claimId !== previous.claimId
    && claim.repositoryId === previous.repositoryId && claim.scopeReserved
    && writeSetsOverlap(claim.declaredWriteScope, previous.declaredWriteScope) ));
  ...
  const eligible = claims .filter((claim) => ( claim.state === "waiting-successor"
      && claim.repositoryId === previous.repositoryId
      && writeSetsOverlap(claim.declaredWriteScope, previous.declaredWriteScope) ))
    .sort(compareWaitingSuccessors); if (eligible[0]?.claimId !== previous.claimId) {
...
function integrateClaim({ base, previous, intent, evaluationTime }) {
  ...
  if (intent.candidateRevision !== previous.laneRevision
    ...
    fail("candidate_identity_mismatch", "integrate must consume the immutable reviewed candidate");
  } return { ...base, state: "integrated-preserved", integration: {
      candidateRevision: intent.candidateRevision, reviewRequestId: intent.reviewRequestId,
      ...
      integrationIntentDigest: intent.integrationIntentDigest, integratedAt: evaluationTime, }, }; }
function retireClaim({ base, previous, intent, ledger, evaluationTime }) {
  ...
  } if (integrated) { const integrationEntry = ledger.entries.findLast((entry) => (
  ...
  } return { ...base, state: "retired", retirement: { reason: intent.reason,
      finalRevision: intent.finalRevision, reviewRequestId: intent.reviewRequestId,
      bytesDigest: intent.bytesDigest, namedChecksDigest: intent.namedChecksDigest,
      handoffEvidenceDigest: intent.handoffEvidenceDigest,
      integrationReceiptDigest: intent.integrationReceiptDigest, retiredAt: evaluationTime, }, }; }
function buildClaimCore(intent, ledger, evaluationTime) {
  if (Date.parse(intent.expiresAt) <= Date.parse(evaluationTime)) {
    fail("invalid_expiry", "expiresAt must be later than evaluationTime"); }
  const latest = currentClaimEntries(ledger); const matchingEpochs = [...latest.values()]
    .map((entry) => entry.claimCore) .filter((claim) => ( claim.repositoryId === intent.repositoryId
      && claim.workItemId === intent.workItemId && claim.writeSetDigest === intent.writeSetDigest ))
  ...
  for (const current of currentClaims(ledger, evaluationTime)) { if (
      current.repositoryId !== intent.repositoryId
      || !writeSetsOverlap(current.declaredWriteScope, intent.declaredWriteScope) ) continue;
  ...
  } return { claimId: intent.claimId, actorId: intent.actorId, deviceId: intent.deviceId,
    sessionId: intent.sessionId, repositoryId: intent.repositoryId, workItemId: intent.workItemId,
    canonicalBaseRevision: intent.canonicalBaseRevision,
    declaredWriteScope: intent.declaredWriteScope, writeSetDigest: intent.writeSetDigest,
    laneRevision: intent.laneRevision, leaseEpoch: intent.leaseEpoch, transitionCounter: 1,
    heartbeatCounter: 0,
    state: overlapping.length + queued.length === 0 ? "current" : "waiting-successor",
    expiresAt: intent.expiresAt, evidenceDigest: null, reviewRequestId: null,
    predecessorClaimId: predecessorClaimId ?? null, ...(intent.canonicalDescendantProof
      ? { canonicalDescendantProof: intent.canonicalDescendantProof } : {}),
    eligibleSince: overlapping.length + queued.length === 0 ? null : evaluationTime, handoff: null,
    release: null, }; }
function allowsPredecessorBaseContinuation({ ledger, intent, evaluationTime, protectedRevision = null }) {
  ...
  const claimOnlyRollover = claimOnlySuccessorBaseRolloverPredecessor({ ledger, intent,
    evaluationTime, protectedRevision, }); if (claimOnlyRollover) {
    return Boolean(normalizeCanonicalDescendantProof({ value: intent.canonicalDescendantProof,
      ...
      protectedRevision: protectedRevision ?? intent.canonicalBaseRevision, })); }
...
export function claimConflictSetDigest(ledger, intent) {
  const conflictSet = {
    ...
    policyRevision: "agentic-cloud-collaboration/v1@1.2.0",
...
function claimConflictDecision({ disposition, observedLedgerDigest, currentLedgerDigest,
  observedConflictSetDigest, currentConflictSetDigest }) {
  const decision = {
    schema: CLAIM_CONFLICT_DECISION_SCHEMA,
    disposition,
    observedLedgerDigest,
    currentLedgerDigest,
    observedConflictSetDigest,
    currentConflictSetDigest,
    semanticTransitionAllowed: ["current", "disjoint-rebase", "overlapping",
      "idempotent-replay"].includes(disposition),
    reparentAllowed: disposition === "disjoint-rebase",
    requiresReplan: ["semantic-conflict", "unknown-observation"].includes(disposition),
  };
  return Object.freeze({ ...decision, decisionDigest: digestValue(decision) });
}
export function classifyClaimObservation(ledger, intent) {
  const currentConflictSetDigest = claimConflictSetDigest(ledger, intent);
  if (intent.expectedLedgerDigest === ledger.headDigest) {
    return claimConflictDecision({
      disposition: "current",
      observedLedgerDigest: intent.expectedLedgerDigest,
      currentLedgerDigest: ledger.headDigest,
      observedConflictSetDigest: currentConflictSetDigest,
      currentConflictSetDigest,
    });
  }
  let observedLedger;
  if (intent.expectedLedgerDigest === null) {
    observedLedger = { ...ledger, sequence: 0, headDigest: null, entries: [] };
  } else {
    const index = ledger.entries.findIndex((entry) => entry.digest === intent.expectedLedgerDigest);
    if (index < 0) return claimConflictDecision({
      disposition: "unknown-observation",
      observedLedgerDigest: intent.expectedLedgerDigest,
      currentLedgerDigest: ledger.headDigest,
      observedConflictSetDigest: null,
      currentConflictSetDigest,
    });
    observedLedger = { ...ledger, sequence: index + 1,
      headDigest: intent.expectedLedgerDigest, entries: ledger.entries.slice(0, index + 1) };
  }
  const observedConflictSetDigest = claimConflictSetDigest(observedLedger, intent);
  return claimConflictDecision({
    disposition: observedConflictSetDigest === currentConflictSetDigest
      ? "disjoint-rebase" : "semantic-conflict",
    observedLedgerDigest: intent.expectedLedgerDigest,
    currentLedgerDigest: ledger.headDigest,
    observedConflictSetDigest,
    currentConflictSetDigest,
...
function requireCompatibleClaimObservation(ledger, intent) {
  const decision = classifyClaimObservation(ledger, intent);
  if (decision.disposition === "unknown-observation") {
    fail("claim_observation_unknown",
      "expectedLedgerDigest is not in the current ledger ancestry (claimConflictDisposition=unknown-observation)");
  }
  if (decision.disposition === "semantic-conflict") {
    fail("claim_conflict_set_changed",
      "expectedLedgerDigest conflict set is stale (claimConflictDisposition=semantic-conflict)");
  }
  return decision;
}
function appendEntry(ledger, action, intent, requestDigest, idempotencyKey, evaluationTime) {
  ...
  const claimDigest = digestValue(claimCore); const draft = { schema: ENTRY_SCHEMA,
    sequence: ledger.sequence + 1, parentDigest: ledger.headDigest, action,
    repositoryId: claimCore.repositoryId, claimId: claimCore.claimId, idempotencyKey, requestDigest,
    evaluationTime, claimCore, claimDigest, };
  const entry = { ...draft, digest: digestValue(draft) }; return { ledger: { ...ledger,
      sequence: entry.sequence, headDigest: entry.digest, entries: [...ledger.entries, entry], },
    entry, }; }
function statusReceipt(ledger, claim, evaluationTime) { const draft = { schema: RECEIPT_SCHEMA,
    action: "status", ledgerRepositoryId: ledger.ledgerRepositoryId,
    repositoryId: claim?.repositoryId ?? null, claimId: claim?.claimId ?? null,
    claimDigest: claim?.fenceRevision ?? null, fenceRevision: claim?.fenceRevision ?? null,
    ledgerRevision: ledger.headDigest, ledgerSequence: ledger.sequence, evaluationTime, };
...
export function applyCloudTransition({ ledger, action, request = {}, actor, repository, evaluationTime }) {
  ...
  if (normalizedAction === "status") {
    ...
    return { ledger, claim, claims, claimDigest: claim?.fenceRevision ?? null,
      receipt: statusReceipt(ledger, claim, evaluatedAt), replayed: false, }; }
  if (normalizedAction === "verify") {
    const receipt = verifyCloudClaim({ ledger, request, evaluationTime: evaluatedAt }); return {
      ledger, claim: receipt.claim, claimDigest: receipt.claim?.fenceRevision ?? null, receipt,
      replayed: false, }; }
  ...
  const claimOnlyRollover = normalizedAction === "claim"
    ? claimOnlySuccessorBaseRolloverPredecessor({ ledger, intent: rawIntent,
      ...
      requireProtectedRevision: true, }) : null;
  const canonicalDescendantProof = normalizedAction === "claim" && rawIntent.canonicalDescendantProof
    ? normalizeCanonicalDescendantProof({ value: rawIntent.canonicalDescendantProof,
      sourceBaseSha: claimOnlyRollover?.canonicalBaseRevision ?? rawIntent.canonicalBaseRevision,
      protectedRevision: repositoryValue.canonicalRevision }) : null;
  ...
  const intent = canonicalDescendantProof ? { ...rawIntent, canonicalDescendantProof } : rawIntent;
  if ( repositoryValue.canonicalRevision && normalizedAction === "claim"
    && repositoryValue.canonicalRevision !== intent.canonicalBaseRevision
    && !allowsPredecessorBaseContinuation({ ledger, intent, evaluationTime: evaluatedAt,
  ...
  const claimObservationDecision = normalizedAction === "claim"
  ...
  const acceptedClaimConflictDecision = normalizedAction === "claim"
    && appended.entry.claimCore.state === "waiting-successor"
    && claimObservationDecision.disposition === "current"
    ? claimConflictDecision({ ...claimObservationDecision, disposition: "overlapping" })
    : claimObservationDecision;
  return mutationResult(appended.ledger, appended.entry, false, {
    conflictSetDigest: claimObservationDecision?.currentConflictSetDigest ?? null,
    claimConflictDecision: acceptedClaimConflictDecision,
  }); }
function exactIdempotentClaimReplay({
  ...
  const replayDecision = claimConflictDecision({
    disposition: "idempotent-replay",
    observedLedgerDigest: prior.parentDigest,
    currentLedgerDigest: prior.parentDigest,
    observedConflictSetDigest: acceptedConflictSetDigest,
    currentConflictSetDigest: acceptedConflictSetDigest,
  });
  return mutationResult(ledger, prior, true, {
    conflictSetDigest: acceptedConflictSetDigest,
    claimConflictDecision: replayDecision,
  });
}
export function verifyCloudClaim({ ledger, request = {}, evaluationTime }) {
  ...
  const currentClaim = claimId
    ? hydrateWithLedger(ledger, findClaimEntry(ledger, claimId), evaluatedAt) : null;
  const historicalClaim = recoverIntegratedPreservedLineageClaim({ ledger, currentClaim, request,
  }); const claim = historicalClaim || currentClaim; const findings = []; if (failures.length > 0) {
    ...
    findings.push(collaborationFinding("stale-collaboration-fence", ledger, claim, request, null, "obtain-current-claim"));
  } else { const stale = [ ["repositoryId", claim.repositoryId], ["workItemId", claim.workItemId],
      ...
      ["fenceRevision", claim.fenceRevision], ["ledgerRevision", claim.ledgerRevision],
    ...
    if (stale || (!historicalClaim
      ...
      other.claimId !== claim.claimId && other.repositoryId === claim.repositoryId
      && other.writeAuthority && claim.writeAuthority
    ...
    if (collisions.length > 0) {
      ...
      ? digest(request.focusedEvidenceDigest, "focusedEvidenceDigest") : null;
    if (suppliedEvidence && !claim.evidenceDigest) {
      ...
      const uncoveredPaths = findUncoveredPathScopes( claim.declaredWriteScope,
        request.observedChangedPaths, ); if (uncoveredPaths.length > 0) { findings.push({
          ...collaborationFinding( "declared-write-scope-unproven", ledger, claim, request,
            claim.evidenceDigest, "reconcile-reviewed-scope", ), scope: uncoveredPaths, }); } }
    ...
    const stateReady = requiredState === "current"
      ...
      : requiredState === "reviewed" ? ["reviewed", "integrated-preserved"].includes(claim.state)
        : claim.state === requiredState; const reviewReady = requiredState !== "reviewed" || (
      claim.reviewRequestId && claim.evidenceDigest
  ...
  const findingCounts = Object.fromEntries(FINDING_TYPES.map((type) => [ type,
    findings.filter((item) => item.type === type).length, ])); const draft = {
    schema: RECEIPT_SCHEMA, action: "verify", ok: findings.length === 0,
    verdict: findings.length === 0 ? "ready" : "blocked",
    ledgerRepositoryId: ledger.ledgerRepositoryId,
    repositoryId: claim?.repositoryId ?? request.repositoryId ?? null, claimId, claim,
    claimDigest: claim?.fenceRevision ?? null, ledgerRevision: ledger.headDigest,
    ledgerSequence: ledger.sequence, evaluationTime: evaluatedAt, findings, findingCounts, };
  return { ...draft, receiptDigest: digestValue(draft) }; }
function recoverIntegratedPreservedLineageClaim({ ledger, currentClaim, request }) { if (
    !currentClaim || request.requiredState !== "integrated-preserved" || !request.fenceRevision
    || !request.integrationReceiptDigest || !Number.isSafeInteger(request.transitionCounter)
  ) return null; const integratedCurrent = currentClaim.state === "integrated-preserved";
  ...
  const historicalIndex = lineage.findIndex( entry => entry.claimDigest === request.fenceRevision,
  ); if (historicalIndex < 0) return null; const historical = hydrateWithLedger( ledger,
    lineage[historicalIndex], null, ); if ( historical.state !== "integrated-preserved"
    ...
    || currentClaim.integrationReceiptDigest !== request.integrationReceiptDigest ) return null;
  ...
  let previousEntry = lineage[historicalIndex]; for (const entry of renewalEntries) {
    if (!isIntegratedPreservedContinuation({ previousEntry, entry })) return null;
    previousEntry = entry; } if (retiredCurrent) { const retirement = suffix.at(-1); if (
      !retirement || retirement.action !== "retire"
      || retirement.claimCore?.retirement?.integrationReceiptDigest
        !== request.integrationReceiptDigest ) return null; }
  if (currentClaim.transitionCounter !== request.transitionCounter + suffix.length) { return null; }
  return historical; }
function isIntegratedPreservedContinuation({ previousEntry, entry }) { if (
    entry.schema !== ENTRY_SCHEMA || entry.action !== "continue"
  ...
  ) return false; const stableCore = (value, { omitRecovery = false } = {}) => { const {
      expiresAt: _expiresAt, heartbeatCounter: _heartbeatCounter,
      transitionCounter: _transitionCounter, recovery: omittedRecovery, ...stable } = value;
    return omitRecovery ? stable : { ...stable,
  ...
  const renewal = (
    ...
    && canonicalJson(stableCore(entry.claimCore))
      === canonicalJson(stableCore(previousEntry.claimCore)) ); if (renewal) return true; return (
