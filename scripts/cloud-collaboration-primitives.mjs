...
export const CLOUD_COLLABORATION_BOUNDS = Object.freeze({ writeScopeItems: 128, textCharacters: 512,
});
export const ROOT_OPERATIONS = Object.freeze([ "claim", "continue", "integrate", "retire", ]);
...
const HISTORICAL_ACTIONS = new Set([ "bind", "heartbeat", "review-ready", "delivery-authorize",
  "handoff", "release", ]);
const CLAIM_STATES = new Set([ "current", "waiting-successor", "reviewed", "integrated-preserved",
  "dormant-preserved", "retired", "active", "review-ready", "delivery-authorized", "parked",
  "released", ]);
const DIGEST_PATTERN = /^[a-f0-9]{64}$/u;
export class CloudCollaborationError extends Error { constructor(code, message) { super(message);
    this.name = "CloudCollaborationError"; this.code = code; } }
export function fail(code, message) { throw new CloudCollaborationError(code, message); }
...
export function digest(value, field, { optional = false } = {}) {
  const normalized = text(value, field, { optional }); if (normalized === null) return null;
  if (!DIGEST_PATTERN.test(normalized)) fail("invalid_request", `${field} must be a lowercase SHA-256 digest`);
  return normalized; }
export function instant(value, field) { const normalized = text(value, field);
...
function normalizeCanonical(value, seen = new Set()) {
  ...
  seen.add(value); const normalized = {}; for (const key of Object.keys(value).sort()) {
    if (value[key] === undefined) fail("invalid_canonical_value", `canonical field ${key} is undefined`);
    normalized[key] = normalizeCanonical(value[key], seen); } seen.delete(value); return normalized;
}
export function canonicalJson(value) { return JSON.stringify(normalizeCanonical(value)); }
...
function normalizePathScope(rawValue) {
  ...
  if (value.startsWith("/") || /^[A-Za-z]:\//u.test(value)) {
    fail("invalid_write_scope", "path scope must be repository-relative"); } const segments = [];
  for (const segment of value.split("/")) { if (!segment || segment === ".") continue;
    if (segment === "..") fail("invalid_write_scope", "path scope must not traverse its repository");
    segments.push(segment); } value = segments.length === 0 ? "." : segments.join("/");
  return `path:${value}`; }
...
export function normalizeWriteSet(values) { if (!Array.isArray(values) || values.length === 0) {
  ...
  if (values.length > CLOUD_COLLABORATION_BOUNDS.writeScopeItems) {
    ...
    return normalizePathScope(value); }); return [...new Set(normalized)].sort(); }
function pathScopesOverlap(left, right) { const leftPath = left.slice("path:".length);
  const rightPath = right.slice("path:".length);
  if (leftPath === "." || rightPath === ".") return true; return leftPath === rightPath
    || leftPath.startsWith(`${rightPath}/`) || rightPath.startsWith(`${leftPath}/`); }
export function writeSetsOverlap(leftValues, rightValues) {
  const left = normalizeWriteSet(leftValues); const right = normalizeWriteSet(rightValues);
  return left.some((leftScope) => right.some((rightScope) => {
    if (leftScope.startsWith("path:") && rightScope.startsWith("path:")) {
      return pathScopesOverlap(leftScope, rightScope); } return leftScope === rightScope; })); }
export function normalizeCanonicalDescendantProof({ value, sourceBaseSha, protectedRevision = null }) {
  if (value == null) return null; if (!value || typeof value !== "object" || Array.isArray(value)) {
...
function normalizeProofPaths(value, label) { if (!Array.isArray(value) || value.length === 0) {
...
export function findUncoveredPathScopes(declaredWriteScope, changedPaths) {
  const declared = normalizeWriteSet(declaredWriteScope); if (!Array.isArray(changedPaths)) {
    fail("invalid_request", "observedChangedPaths must be an array"); }
  if (changedPaths.length === 0) return []; if (changedPaths.length > 100_000) {
  ...
  const observed = [...new Set(changedPaths.map((value, index) => {
    ...
    return normalized.startsWith("path:") ? normalizePathScope(normalized.slice("path:".length))
      : normalizePathScope(normalized); }))].sort();
  return observed.filter((scope) => !writeSetsOverlap(declared, [scope])); }
export function normalizeRepository(repository) { if (typeof repository === "string") {
...
export function normalizeActor(actor, request = {}) {
  ...
  const opaqueId = actor.actorId ?? actor.id; return {
...
export function normalizeRootIntent(action, request, actor, repositoryId) {
  ...
  if (action === "claim") return normalizeClaimIntent(request, common); const expected = {
...
function normalizeClaimIntent(request, common) {
  const declaredWriteScope = normalizeWriteSet(request.declaredWriteScope); const intent = {
    ...common, workItemId: text(request.workItemId, "workItemId"),
    canonicalBaseRevision: text(request.canonicalBaseRevision, "canonicalBaseRevision"),
    declaredWriteScope, writeSetDigest: digestValue(declaredWriteScope),
    ...
    expectedLedgerDigest: request.expectedLedgerDigest === null ? null
      : digest(request.expectedLedgerDigest, "expectedLedgerDigest"), };
  const claimId = digestValue({ actorId: common.actorId,
    canonicalBaseRevision: intent.canonicalBaseRevision, leaseEpoch: intent.leaseEpoch,
    repositoryId: common.repositoryId, workItemId: intent.workItemId,
...
function normalizeIntegrationIntent(request, common, expected) { return { ...common, ...expected,
...
export const FINDING_TYPES = Object.freeze([ "declared-write-scope-unproven",
  "parallel-scope-collision", "stale-collaboration-fence", "delivery-authority-unjoined",
  "evidence-without-run", "runtime-readiness-unproven", ]);
export function collaborationFinding(type, ledger, claim, expected = {}, evidenceDigest = null, remediation = "re-evaluate") {
  return { type, severity: type === "evidence-without-run" ? "major" : "blocker",
    ...
    expectedFence: expected.fenceRevision ?? null, observedFence: claim?.fenceRevision ?? null,
    affectedRevisions: [claim?.canonicalBaseRevision, claim?.laneRevision, ledger.headDigest].filter(Boolean),
    evidenceDigest, remediation, }; }
...
function validInstant(value) { const milliseconds = Date.parse(value);
...
function effectiveState(entry, evaluationTime) { const state = recordedState(entry);
...
function cachedOverlap(left, right) {
  ...
  if (DIGEST_PATTERN.test(digests[0] ?? "")) {
    ...
    OVERLAP_CACHE.set(key, result); } return result; }
...
export function claimOnlySuccessorBaseRolloverPredecessor({
  ledger, predecessorHistory = null, intent, protectedRevision = null,
  requireProtectedRevision = false, }) { if (!intent?.predecessorClaimId) return null;
  const history = predecessorHistory ?? (Array.isArray(ledger?.entries)
    ? ledger.entries.filter(entry => entry?.claimId === intent.predecessorClaimId) : null);
  if (!Array.isArray(history)) return null; if (history.length !== 2) return null;
  const [origin, terminal] = history;
  const source = origin?.claimCore, predecessor = terminal?.claimCore;
  const targetRevision = protectedRevision ?? intent.canonicalBaseRevision; let sameScope = false;
  try { sameScope = canonicalJson(intent.declaredWriteScope)
      === canonicalJson(predecessor?.declaredWriteScope); } catch { return null; }
  const sourceOptionalEvidence = ["canonicalDescendantProof", "handoffEvidenceDigest", "promotedAt",
    "recovery", "integration", "retirement"];
  if ((requireProtectedRevision && !protectedRevision) || !source || !predecessor
    || origin.schema !== ENTRY_SCHEMA || terminal.schema !== ENTRY_SCHEMA
    || origin.action !== "claim" || terminal.action !== "retire"
    || source.state !== "waiting-successor" || !source.predecessorClaimId
    || source.canonicalBaseRevision !== source.laneRevision
    || source.transitionCounter !== 1 || source.heartbeatCounter !== 0
    || source.evidenceDigest !== null || source.reviewRequestId !== null
    || sourceOptionalEvidence.some(field => Object.hasOwn(source, field))
    || predecessor.state !== "retired" || predecessor.transitionCounter !== 2
    || predecessor.heartbeatCounter !== 0 || predecessor.evidenceDigest !== null
    || predecessor.reviewRequestId !== null || predecessor.integration !== undefined
    || predecessor.recovery !== undefined || predecessor.retirement?.reason !== "superseded"
    || predecessor.retirement.finalRevision !== source.canonicalBaseRevision
    || predecessor.retirement.reviewRequestId !== null
    || predecessor.retirement.integrationReceiptDigest !== null
    || intent.actorId !== predecessor.actorId || intent.deviceId !== predecessor.deviceId
    || intent.sessionId !== predecessor.sessionId || intent.repositoryId !== predecessor.repositoryId
    || intent.workItemId !== predecessor.workItemId || intent.writeSetDigest !== predecessor.writeSetDigest
    || !sameScope || intent.leaseEpoch !== predecessor.leaseEpoch + 1
    || intent.canonicalBaseRevision !== targetRevision || intent.laneRevision !== targetRevision
    || targetRevision === predecessor.canonicalBaseRevision
    || intent.canonicalDescendantProof?.sourceBaseSha !== predecessor.canonicalBaseRevision
    || intent.canonicalDescendantProof?.targetBaseSha !== targetRevision
    || intent.canonicalDescendantProof?.protectedMainSha !== targetRevision) return null;
  return predecessor; }
function checkClaim(entry, records, latest, histories, failures, label) {
  ...
  if (!queued && core.predecessorClaimId) {
    ...
    const rolloverPredecessor = divergentBase && claimOnlySuccessorBaseRolloverPredecessor({
      predecessorHistory: histories.get(core.predecessorClaimId), intent: core,
      protectedRevision: core.canonicalBaseRevision, }); let validDescendantProof = false;
    if (divergentBase && core.canonicalDescendantProof) { try {
        validDescendantProof = Boolean(normalizeCanonicalDescendantProof({
          value: core.canonicalDescendantProof,
          sourceBaseSha: rolloverPredecessor?.canonicalBaseRevision ?? core.canonicalBaseRevision,
          protectedRevision: rolloverPredecessor ? core.canonicalBaseRevision : null, }));
      } catch {} }
    const ordinaryInvalid = !prior || !["dormant-preserved", "retired"].includes(effectiveState(prior, entry.evaluationTime))
    ...
    if (ordinaryInvalid && !(rolloverPredecessor && validDescendantProof)) {
...
function checkPromotion(entry, previous, records, latest, failures, label) {
  ...
  const eligible = peers .filter(({ state }) => state === "waiting-successor").sort(waitingOrder);
...
function checkContinue(entry, previous, records, latest, failures, label) {
  ...
  if (state === "dormant-preserved") {
    ...
    if (core.state !== restored || !core.recovery || core.recovery.recoveredAt !== entry.evaluationTime
      ...
      failures.push(`${label} dormant recovery is invalid`); } return; }
  if (state === "current" && core.state === "current") {
    ...
    if (!projection && !renewal) failures.push(`${label} current continuation is invalid`); return;
    ...
    return; } if (state === "current" && core.state === "dormant-preserved") {
    if (!core.handoffEvidenceDigest || heartbeatDelta !== 0 || core.expiresAt !== prior.expiresAt
      ...
      failures.push(`${label} preservation evidence is invalid`); } return; }
  if (state === "reviewed" && core.state === "reviewed") {
    ...
    return; } if (state === "integrated-preserved" && core.state === "integrated-preserved") {
    ...
    return; } failures.push(`${label} continue state is invalid`); }
...
export function validateLedger(ledger) { const failures = [];
  ...
  const latest = new Map(), histories = new Map(), keys = new Set(), epochs = new Map();
  const integrationReceipts = new Map(), legacyLineages = new Set();
  for (let index = 0; index < ledger.entries.length; index += 1) {
    ...
    keys.add(entry.idempotencyKey); const evaluation = Date.parse(entry.evaluationTime);
    ...
    if (!core || typeof core !== "object") failures.push(`${label}.claimCore is invalid`); else {
      ...
      try { const scope = normalizeWriteSet(core.declaredWriteScope);
      ...
      else if (entry.action === "claim") checkClaim(entry, records, latest, histories, failures, label);
      ...
      latest.set(entry.claimId, entry); } if (typeof entry.claimId === "string") {
      const history = histories.get(entry.claimId); if (history) history.push(entry);
      else histories.set(entry.claimId, [entry]); }
    const { digest: observedDigest, ...draft } = entry; try {
    ...
    } catch { failures.push(`${label} is not canonical JSON`); } parentDigest = observedDigest; }
