...
function normalizeCloud(value) { exact(value, ["claimId", "matches", "state", "writeAuthority", "scopeReserved", "leaseEpoch", "canonicalBaseSha", "laneRevision", "writeSetDigest", "reviewRequestId", "integrationState", "claimDigest", "transitionCounter", "operationReceiptDigest"], "cloud"); if (value.matches !== 1 || !new Set(["dormant-preserved", "current"]).has(value.state) || value.scopeReserved !== true || (value.state === "dormant-preserved" ? value.writeAuthority !== false : value.writeAuthority !== true) || value.integrationState !== "not-integrated") invalid("same-claim cloud state"); return Object.freeze({ claimId: digest(value.claimId, "cloud claim"), matches: 1, state: value.state, writeAuthority: value.writeAuthority, scopeReserved: true, leaseEpoch: positive(value.leaseEpoch, "cloud lease epoch"), canonicalBaseSha: sha(value.canonicalBaseSha, "cloud base"), laneRevision: sha(value.laneRevision, "cloud lane revision"), writeSetDigest: digest(value.writeSetDigest, "cloud write set"), reviewRequestId: text(value.reviewRequestId, "cloud review request"), integrationState: value.integrationState, claimDigest: digest(value.claimDigest, "cloud claim digest"), transitionCounter: positive(value.transitionCounter, "cloud transition"), operationReceiptDigest: digest(value.operationReceiptDigest, "cloud operation receipt") }); }
