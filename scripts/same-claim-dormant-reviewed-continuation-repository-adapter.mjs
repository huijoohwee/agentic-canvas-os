...
export function createRepositorySameClaimDormantReviewedContinuationAdapter(options = {}, dependencies = {}) {
  ...
  function inspectFrame() {
    ...
    const stateShape = claim.state === "dormant-preserved" ? claim.writeAuthority === false && claim.scopeReserved === true : claim.state === "current" && claim.writeAuthority === true && claim.scopeReserved === true;
...
export function projectRenewedReviewedLeaseSubject(lease, authority) {
  if (lease?.status !== "review_ready" || authority?.state !== "active" || lease.cloudAuthority?.claimId !== authority.claimId || lease.cloudAuthority?.reviewRequestId !== authority.reviewRequestId) fail("renewed reviewed lease subject");
