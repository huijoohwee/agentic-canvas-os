...
const WRITE_CALL = /\b(?:writeFile|writeFileSync|appendFile|appendFileSync|mkdir|mkdirSync|rename|renameSync|copyFile|copyFileSync|cp|cpSync|rm|rmSync)\s*\(\s*([^,\n]+)/gu;
...
export function analyzeSource({ source, file, repositoryRoot: root }) {
  ...
  if (file.startsWith(".githooks/")) {
    source.split("\n").forEach((line, index) => {
      const shellMatch = line.match(/(?:^|[;&|]\s*|\s)(?:mkdir|cp|mv|tee)\s+(?:-[^\s]+\s+)*(["']?[^\s"']+["']?)/u)
        || line.match(/(?:>|>>)\s*(["']?[^\s"']+["']?)/u);
      if (!shellMatch) return;
      const target = resolveExpression(shellMatch[1], { constants, root });
      if (target && escapes(root, target)) findings.push({ file, line: index + 1, target, expression: shellMatch[1] });
    });
  }
  for (const match of source.matchAll(WRITE_CALL)) {
    const target = resolveExpression(match[1], { constants, root });
    if (target && escapes(root, target)) findings.push({
      file, line: source.slice(0, match.index).split("\n").length, target, expression: match[1].trim(),
    });
  }
...
export function resolveExpression(raw, { constants = new Map(), root }) {
  ...
  if (/^os\.homedir\(\)$/u.test(expression)) return path.parse(root).root;
