#!/usr/bin/env node
// Responsibility: Expose read-only planning and exact authorized terminal-handoff recovery.
import { readFileSync } from "node:fs";
import path from "node:path";
import { pathToFileURL } from "node:url";

import { createTerminalHandoffOwnedDirtSuccessorRecoveryController }
  from "./terminal-handoff-owned-dirt-successor-recovery-controller.mjs";
import { createTerminalHandoffOwnedDirtSuccessorRecoveryRepositoryAdapter }
  from "./terminal-handoff-owned-dirt-successor-recovery-repository-adapter.mjs";
...
export async function main(argumentsList = process.argv.slice(2), dependencies = {}) {
  ...
  const adapter = (dependencies.createAdapter
    ...
    repository, branch: options.get("branch") || null,
    taskAuthorityFile: path.resolve(required(options, "task-authority")),
  });
  const controller = (dependencies.createController
    || createTerminalHandoffOwnedDirtSuccessorRecoveryController)(adapter);
  if (command === "plan") {
  ...
  const plan = readJson(path.resolve(required(options, "plan")), "recovery plan");
  return controller.run({ plan, operatorSessionId,
...
export async function runCli(argumentsList = process.argv.slice(2)) {
  try { console.log(JSON.stringify(await main(argumentsList))); return 0; }
    ...
    schema: "agentic-terminal-handoff-owned-dirt-successor-recovery-result/v1",
    ok: false, status: "blocked", error: String(error?.message || error).slice(0, 1_000),
  })); return 1; }
}

function parse(argumentsList) { const options = new Map();
  for (const argument of argumentsList) { const match = argument.match(/^--([a-z0-9-]+)=(.*)$/u);
    if (!match || !OPTIONS.has(match[1]) || !match[2] || options.has(match[1]))
      throw new Error(`Unsupported or duplicate option: ${argument}`); options.set(match[1], match[2]); }
  return options; }
...
function usage() { return "Usage: terminal-handoff-owned-dirt-successor-recovery.mjs plan|run "
