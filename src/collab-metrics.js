 ...
 */
export function recordRoomFanOut(metrics, settlement = {}) {
  let attempted;
  let dispatched;
  let succeeded;
  let failed;
  ...
  try {
    ...
    dispatched = settlement.dispatched === undefined ? attempted : settlement.dispatched;
    succeeded = settlement.succeeded;
    failed = settlement.failed;
    timedOut = settlement.timedOut === undefined ? 0 : settlement.timedOut;
    canceled = settlement.canceled === undefined ? 0 : settlement.canceled;
    canceledBeforeDispatch = settlement.canceledBeforeDispatch === undefined
      ? 0
      : settlement.canceledBeforeDispatch;
    setupFailures = settlement.setupFailures === undefined ? 0 : settlement.setupFailures;
  } catch {
    return recordRoomObservabilityFailure(metrics);
  }
  if (
...
export function roomLogRecord({ room, event, level = "info", now, fields = {} } = {}) {
  ...
  // Keep the trusted envelope authoritative even when an adapter supplies a
  // colliding field name. Arbitrary non-reserved fields remain transport-neutral.
  return { ...safeFields, ts: new Date(ts).toISOString(), level, room, event };
}
