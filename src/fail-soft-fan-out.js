...
function boundedDispatch({
  branch,
  index,
  dispatch,
  onCanceledBeforeDispatch,
  onDispatched,
  signal,
  timeoutMs,
}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let preDispatchCancellationRecorded = false;
    let settled = false;
    const cleanup = () => {
      try {
        if (timer !== undefined) clearTimeout(timer);
      } catch {
        // Host timer cleanup cannot leave the aggregate waiting forever.
      }
      try {
    ...
    const onAbort = () => {
      if (!dispatchStarted && !preDispatchCancellationRecorded) {
        preDispatchCancellationRecorded = true;
        onCanceledBeforeDispatch();
      }
      abort(failSoftBranchFailure("branch_canceled", { retryable: true }));
    };
    ...
    Promise.resolve()
      .then(() => {
        // Cancellation is delivered synchronously by native AbortSignals, but
        // re-check at the actual dispatch boundary for portable signal-like
        // implementations and to close the setup-to-microtask race.
  ...
  const outcomes = await Promise.all(branchList.map(async (branch, index) => {
    ...
    try {
      const value = await boundedDispatch({
        ...
        index,
        dispatch,
        onCanceledBeforeDispatch: () => { canceledBeforeDispatch += 1; },
        onDispatched: () => { dispatched += 1; },
        signal,
        timeoutMs,
      });
      return Object.freeze({ branchId, status: "succeeded", value });
    } catch (error) {
      return Object.freeze({ branchId, status: "failed", ...failureMetadata(error) });
    }
  }));
